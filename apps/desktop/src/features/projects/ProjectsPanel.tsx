...
export function ProjectsPanel({ state, onChooseProject }: { state: DesktopState; onChooseProject: () => void }) {
  const { t } = useLocale();
  return (
    <section className="page-section" aria-labelledby="projects-title" data-codegpt-page="projects">
      ...
      {state.project ? (
        <article className="detail-card" aria-labelledby="configured-project-title">
          ...
          <button className="secondary-button" onClick={onChooseProject} disabled={Boolean(state.current_operation)} data-codegpt-action="change-project">
            {t("project.change")}
          </button>
        </article>
      ) : (
        <div className="empty-state">
          ...
          <button className="primary-button" onClick={onChooseProject} disabled={Boolean(state.current_operation)} data-codegpt-action="add-project">
            {t("project.add")}
          </button>
        </div>
      )}
