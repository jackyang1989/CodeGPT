...
impl CodeGPTAdapter {
    ...
    pub fn binaries(&self) -> DesktopResult<&ResolvedBinaries> {
        self.binaries.as_ref().ok_or_else(|| {
            DesktopError::new(
                ...
                "CodeGPT binaries have not been verified yet",
    ...
    async fn server_status_with_deadline(
        ...
        let codegpt = match deadline {
            Some(deadline) => self
                .ensure_binaries_until(cancellation, deadline)
                .await?
                .codegpt
                .clone(),
            None => self.ensure_binaries(cancellation).await?.codegpt.clone(),
        };
        ...
        let output: ServerStatusOutput = match deadline {
            Some(deadline) => {
                run_json_until(&codegpt, &args, None, false, cancellation, deadline).await?
            }
            None => run_json(&codegpt, &args, None, false, cancellation).await?,
        };
            ...
            .is_some_and(|value| value.starts_with("warning:"))
        {
            return Err(DesktopError::new(
                ...
                "The running Server does not match this Desktop CodeGPT CLI build",
    ...
    pub fn local_server_command(&self, env_file: &Path) -> DesktopResult<Command> {
        ...
        command.env("CODEGPT_ENV_FILE", env_file);
    ...
    pub fn quick_share_command(
        ...
        let mut command = Command::new(&binaries.codegpt);
        command
            .arg("share")
            .arg("--root")
            .arg(project)
    ...
    pub async fn activate_project(
        ...
        let output: ProjectActivationOutput =
            run_project_activation_json(&codegpt, &args, cancellation).await?;
    ...
    pub async fn legacy_register_project(
        ...
        let output: LegacyProjectRegisterOutput =
            run_json(&codegpt, &args, None, false, cancellation).await?;
...
pub async fn inspect_project_path(path: &str) -> DesktopResult<ProjectSelection> {
    ...
    codegpt_runner_config::paths::validate_project_path_ingress(&requested).map_err(|_| {
        DesktopError::new(
...
pub fn validate_server_url(value: &str) -> DesktopResult<String> {
    ...
    let parsed = Url::parse(value).map_err(|_| {
        DesktopError::new(
            ...
            "Enter a CodeGPT Server URL such as https://codegpt.example.com.",
        )
    })?;
        ...
        || !matches!(parsed.path(), "" | "/")
    {
        return Err(DesktopError::new(
            ...
            "Enter only the CodeGPT Server origin.",
        ));
    }
    Ok(value.to_string())
}
...
fn invalid_contract(operation: &str) -> DesktopError {
    DesktopError::new(
        ...
        format!("CodeGPT returned an incomplete {operation} identity"),
        "Verify that Desktop and the CodeGPT binaries come from the same source baseline.",
    )
}
...
mod tests {
    ...
    fn quick_share_only_inherits_control_plane_credentials_for_openai_provider() {
        let binaries = ResolvedBinaries {
            ...
            codegpt: PathBuf::from("codegpt"),
            server: PathBuf::from("codegpt-server"),
            runner: PathBuf::from("codegpt-runner"),
