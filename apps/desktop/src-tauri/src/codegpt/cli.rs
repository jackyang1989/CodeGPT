...
impl ResolvedBinarySource {
    fn label(self) -> &'static str {
        match self {
            ...
            Self::Environment => "CODEGPT_DESKTOP_BIN_DIR",
    ...
    pub async fn resolve_until(
        ...
        let (directory, source) =
            if let Some(directory) = bundled_runtime_dir.filter(|path| path.is_dir()) {
                ...
                let expected = bundled_runtime_dir
                    ...
                    .unwrap_or_else(|| "<Desktop resource directory>/codegpt-runtime".to_string());
                return Err(DesktopError::new(
                    "bundled_runtime_missing",
                    format!("The installed CodeGPT runtime is missing: {expected}"),
                    "Reinstall CodeGPT Desktop from the matching release installer.",
                ));
            } else if let Some(value) = std::env::var_os("CODEGPT_DESKTOP_BIN_DIR") {
                let directory = PathBuf::from(value);
                if directory.as_os_str().is_empty() {
                    return Err(DesktopError::new(
                        "binary_directory_invalid",
                        "CODEGPT_DESKTOP_BIN_DIR is empty",
                        "Set it to the directory containing the source-matched CodeGPT binaries.",
                    ));
                }
                ...
                let repo = manifest
                    ...
                    .ok_or_else(|| {
                        DesktopError::new(
                            "binary_directory_invalid",
                            "Could not derive the CodeGPT source root",
                            "Set CODEGPT_DESKTOP_BIN_DIR explicitly.",
                        )
                    })?;
                (
        ...
        let missing_directory_action = match source {
            ...
            ResolvedBinarySource::Environment | ResolvedBinarySource::SourceDogfoodTarget => {
                "Build `cargo build --profile dogfood -p codegpt -p codegpt-cli -p codegpt-runner` from this source baseline or set CODEGPT_DESKTOP_BIN_DIR."
            }
        };
        let directory = directory.canonicalize().map_err(|_| {
            DesktopError::new(
                ...
                format!(
                    "CodeGPT binary directory does not exist: {}",
                    directory.display()
                ),
                missing_directory_action,
            )
        })?;
        let codegpt = directory.join(executable_name("codegpt"));
        let server = directory.join(executable_name("codegpt-server"));
        let runner = directory.join(executable_name("codegpt-runner"));
        for path in [&codegpt, &server, &runner] {
            if !path.is_file() {
                return Err(DesktopError::new(
                    "binary_missing",
                    format!("Required CodeGPT binary is missing: {}", path.display()),
                    match source {
                        ResolvedBinarySource::Bundled => {
                            "Reinstall CodeGPT Desktop from the matching release installer."
                        }
                        ...
                        | ResolvedBinarySource::SourceDogfoodTarget => {
                            "Build all CodeGPT dogfood binaries from the current source baseline."
                        }
                    },
                ));
            }
        ...
        let cli_version = binary_version(&codegpt, cancellation, deadline).await?;
            ...
            || cli_version.git_commit != runner_version.git_commit
        {
            return Err(DesktopError::new(
                ...
                "Rebuild all dogfood binaries from one CodeGPT checkout.",
            ));
        }
        if cli_version.git_commit == "unknown" {
            return Err(DesktopError::new(
                "binary_version_unverifiable",
                "CodeGPT binaries do not carry a source revision",
                "Rebuild dogfood binaries with normal CodeGPT build metadata enabled.",
            ));
        }
        Ok(Self {
            directory,
...
async fn binary_version(
    ...
    if output.exit_code != Some(0) {
        return Err(DesktopError::new(
            ...
            "Rebuild the CodeGPT dogfood binaries.",
        ));
    }
    parse_version_line(&output.stdout).ok_or_else(|| {
        DesktopError::new(
            ...
            "Rebuild the CodeGPT dogfood binaries from a compatible source baseline.",
        )
    })
}
...
pub async fn run_project_activation_json<T: DeserializeOwned>(
    ...
    if output.exit_code != Some(0) {
        ...
        .unwrap_or("codegpt_command_failed");
        ...
        return Err(DesktopError::new(
            code,
            "CodeGPT could not activate the selected project on the current Runner",
            next_action,
        )
    ...
    serde_json::from_slice(&output.stdout).map_err(|_| {
        DesktopError::new(
            ...
            "CodeGPT returned invalid project activation output",
...
pub async fn run_json_until<T: DeserializeOwned>(
    ...
    if output.exit_code != Some(0) {
        return Err(DesktopError::new(
            ...
            "CodeGPT did not complete the requested operation",
    ...
    serde_json::from_slice(&output.stdout).map_err(|_| {
        DesktopError::new(
            "codegpt_contract_invalid",
            "CodeGPT returned invalid machine-readable output",
            "Verify that Desktop and CodeGPT binaries come from the same source baseline.",
        )
    })
}
...
async fn run_bounded_until(
    ...
    if stdin_payload.is_some_and(|payload| payload.len() > CLI_INPUT_BYTES) {
        return Err(DesktopError::new(
            ...
            "Protected CodeGPT command input exceeds the Desktop safety limit",
        ...
        ManagedChild::spawn_with_options(&mut command, platform::managed_spawn_options(false))
            .map_err(|error| {
                DesktopError::new(
                    ...
                    "Could not start a safely owned CodeGPT command",
    ...
    let stdout = match child.child_mut().stdout.take() {
        ...
        None => {
            ...
            return Err(DesktopError::new(
                "codegpt_command_start_failed",
                "Could not capture CodeGPT output",
                "Retry the operation.",
            ));
        }
    };
    let stderr = match child.child_mut().stderr.take() {
        ...
        None => {
            ...
            return Err(DesktopError::new(
                "codegpt_command_start_failed",
                "Could not capture CodeGPT diagnostics",
                "Retry the operation.",
            ));
        }
    };
    ...
    if let Some(payload) = stdin_payload {
        let Some(stdin) = child.child_mut().stdin.take() else {
            ...
            return Err(DesktopError::new(
                "codegpt_command_input_failed",
                "Could not open protected input for the CodeGPT command",
                "Retry the operation.",
            ));
        };
        ...
        match await_stdin_writer(&mut stdin_task, deadline.instant(), cancellation).await {
            ...
            Ok(Err(_)) => {
                ...
                return Err(DesktopError::new(
                    "codegpt_command_input_failed",
                    "Could not pass protected input to CodeGPT",
                    "Retry the operation.",
                ));
            }
    ...
    let status = match wait_for_direct_child(&mut child, deadline.instant(), cancellation).await {
        ...
        Ok(Err(_)) => {
            ...
            return Err(DesktopError::new(
                ...
                "Could not observe the CodeGPT command result",
                "Retry the operation.",
            ));
        }
        Err(WaitInterruption::Timeout) => {
    ...
    match wait_for_tree_exit(&child, deadline.instant(), cancellation).await {
        ...
        Ok(Err(_)) => {
            ...
            return Err(DesktopError::new(
                "codegpt_command_wait_failed",
                "Could not verify that the Desktop-owned CodeGPT process tree exited",
...
fn bounded_command(executable: &Path, args: &[String]) -> Command {
    ...
    if std::env::var_os("CODEGPT_DESKTOP_STUCK_OPERATION_FIXTURE").as_deref()
...
fn timeout_error() -> DesktopError {
    DesktopError::new(
        ...
        "CodeGPT did not finish within the Desktop command timeout",
...
mod tests {
    ...
    fn version_parser_requires_commit_identity() {
        let parsed =
            parse_version_line(b"codegpt 0.3.9 (commit 0123456789abcdef, dirty=false)\n").unwrap();
        assert_eq!(parsed.version, "0.3.9");
        assert_eq!(parsed.git_commit, "0123456789abcdef");
        assert!(parse_version_line(b"codegpt 0.3.9\n").is_none());
    }
    ...
    async fn blocked_stdin_uses_one_total_deadline_and_reclaims_owned_tree() {
        ...
        let args = vec![
            ...
            "codegpt-blocked-stdin".to_string(),
        ...
        .await
        .unwrap_err();
        assert_eq!(error.code, "codegpt_command_timeout");
    ...
    async fn inherited_output_pipe_cannot_extend_the_command_lifetime() {
        ...
        let args = vec![
            ...
            "codegpt-inherited-pipe".to_string(),
            marker.to_string_lossy().to_string(),
        ];
        let cancellation = CancellationContext::never();
        let started = Instant::now();
    ...
    async fn explicit_cancel_interrupts_blocked_stdin_and_keeps_cancel_classification() {
        ...
        let args = vec![
            "-c".to_string(),
            "sleep 8 & descendant=$!; printf '%s %s\\n' \"$$\" \"$descendant\" > \"$1\"; wait \"$descendant\"".to_string(),
            "codegpt-cancelled-stdin".to_string(),
            marker.to_string_lossy().to_string(),
        ];
    ...
    async fn elapsed_deadline_does_not_start_a_new_cli_command() {
        ...
        let args = vec![
            ...
            "codegpt-expired-deadline".to_string(),
            marker.to_string_lossy().to_string(),
        ];
        let error = run_bounded_until(
            Path::new("/bin/sh"),
    ...
    async fn cancellation_during_output_drain_reclaims_the_owned_tree() {
        ...
        let args = vec![
            "-c".to_string(),
            "sleep 8 & descendant=$!; printf '%s\\n' \"$descendant\" > \"$1\"; exit 0".to_string(),
            "codegpt-cancel-output-drain".to_string(),
            marker.to_string_lossy().to_string(),
        ];
        let operation = CancellationSignal::new();
        let cancellation = CancellationContext::new(operation.clone(), CancellationSignal::new());
