...
WORK_ROOT="${CODEGPT_MCP_CONFORMANCE_WORK_ROOT:-target/mcp-conformance}"
HARNESS_DIR_EXTERNAL=0
if [ "${CODEGPT_MCP_CONFORMANCE_HARNESS_DIR+x}" = x ]; then
  HARNESS_DIR="${CODEGPT_MCP_CONFORMANCE_HARNESS_DIR}"
  HARNESS_DIR_EXTERNAL=1
else
  HARNESS_DIR="$WORK_ROOT/harness"
fi
REPORT_ROOT_EXTERNAL=0
if [ "${CODEGPT_MCP_CONFORMANCE_REPORT_ROOT+x}" = x ]; then
  REPORT_ROOT="${CODEGPT_MCP_CONFORMANCE_REPORT_ROOT}"
  REPORT_ROOT_EXTERNAL=1
else
  REPORT_ROOT="$WORK_ROOT/reports"
fi
PROFILES=("$@")
HARNESS_OWNER_MARKER=".codegpt-mcp-conformance-harness"
REPORT_OWNER_MARKER=".codegpt-mcp-conformance-reports"
harness_build_dir=""
fixture_dir=""
fixture_pid=""
cleanup_started=0
...
prepare_harness_source() {
  if [ "$HARNESS_DIR_EXTERNAL" -eq 1 ]; then
    if ! is_git_worktree "$HARNESS_DIR"; then
      echo "CODEGPT_MCP_CONFORMANCE_HARNESS_DIR must name an existing Git worktree; refusing to modify it: $HARNESS_DIR" >&2
      exit 2
    fi
...
prepare_report_root() {
  ...
  if [ ! -e "$REPORT_ROOT" ]; then
    ...
    if find "$REPORT_ROOT" -mindepth 1 -maxdepth 1 -print -quit | grep -q .; then
      echo "MCP report root is non-empty and lacks the CodeGPT ownership marker; refusing to delete its contents: $REPORT_ROOT" >&2
      exit 2
    fi
    touch "$REPORT_ROOT/$REPORT_OWNER_MARKER"
  fi
...
# Compile before starting the bounded readiness clock. A cold CodeGPT test build
# can take several minutes on small builders; readiness should measure server
# startup, not Rust compilation time.
cargo test --locked -p codegpt --lib mcp_conformance_fixture_server --no-run

fixture_dir="$(mktemp -d "${TMPDIR:-/tmp}/codegpt-mcp-conformance.XXXXXX")"
...
CODEGPT_MCP_CONFORMANCE_URL_FILE="$url_file" \
CODEGPT_MCP_CONFORMANCE_STOP_FILE="$stop_file" \
  python3 -c 'import os, sys; os.setsid(); os.execvp(sys.argv[1], sys.argv[1:])' \
  cargo test --locked -p codegpt --lib mcp_conformance_fixture_server -- --ignored --nocapture \
  >"$fixture_log" 2>&1 &
fixture_pid=$!
        ...
        "params": {
            "_meta": {
                ...
                "io.modelcontextprotocol/clientInfo": {
                    "name": "codegpt-conformance-capability-probe",
                    "version": "1.0",
                },
            }
        },
