...
if [ "${CODEGPT_E2E_AGENT_RELOAD:-0}" != "1" ]; then
    printf '[agent-reload-e2e] skipped (set CODEGPT_E2E_AGENT_RELOAD=1)\n'
    exit 0
fi
...
TOKEN="codegpt-runner-reload-e2e-only"
...
program = "sh"
args = ["-lc"]
env = { CODEGPT_RELOAD_MARKER = "${marker}" }
[tool_providers]
strategy = "${strategy}"
...
assert_marker() {
    run_shell_request 'printf %s "$CODEGPT_RELOAD_MARKER"'
...
[ -x target/debug/codegpt-server ] && [ -x target/debug/codegpt-runner ] \
    || fail "debug server/agent binaries are unavailable"
TMP_ROOT="$(mktemp -d -t codegpt-runner-reload-e2e-XXXXXX)"
...
setsid env -i PATH="$PATH" LANG=C HOME="$ISOLATED_HOME" \
    ...
    TMPDIR="$RUNTIME_TMP" CODEGPT_ENV_FILE="$TMP_ROOT/empty.env" \
    CODEGPT_ADDR="127.0.0.1:${PORT}" CODEGPT_DATA="$DATA_DIR" CODEGPT_TOKEN="$TOKEN" \
    RUST_LOG=warn target/debug/codegpt-server >"$SERVER_LOG" 2>&1 &
SERVER_PID=$!
wait_for_port open || fail "isolated server port did not open"
setsid env -i PATH="$PATH" LANG=C HOME="$ISOLATED_HOME" \
    XDG_CONFIG_HOME="$ISOLATED_HOME/.config" XDG_DATA_HOME="$ISOLATED_HOME/.local/share" \
    XDG_STATE_HOME="$ISOLATED_HOME/.local/state" XDG_CACHE_HOME="$ISOLATED_HOME/.cache" \
    TMPDIR="$RUNTIME_TMP" CODEGPT_ENV_FILE="$TMP_ROOT/empty.env" RUST_LOG=warn \
    target/debug/codegpt-runner --config "$RUNNER_CONFIG" >"$RUNNER_LOG" 2>&1 &
RUNNER_PID=$!
...
assert_runner_pid
command='sleep 3; printf %s "$CODEGPT_RELOAD_MARKER"'
...
JOB_TWO="$(start_job 'printf %s "$CODEGPT_RELOAD_MARKER"')"
queued_seen=false
for _ in $(seq 1 50); do
