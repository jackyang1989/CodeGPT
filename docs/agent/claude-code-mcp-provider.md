...
`codegpt-runner` can use `claude mcp serve` for one allowlisted, single-call
search capability while CodeGPT remains the online MCP/API, authorization,
...
CodeGPT file writes or edits.

Strategies:

- `native` — existing CodeGPT execution only; this is the default.
...
- `claude_code_then_native` — use Claude first. Searches may fall back to the
  ...
  provider routing exposes only read-only search, it never routes a CodeGPT
...
does not disable CodeGPT search when using `native` or `claude_code_then_native`:
...
configuration. It does not inherit API-key or CodeGPT credential variables.
...
CodeGPT `read_files` always keeps its Native file-read implementation in ordinary
provider routing. That routing path does not discover, map, or call Claude's
`Read` tool.

Claude tools remain an agent-internal implementation detail. CodeGPT builds
its public MCP `tools/list`, runtime registry, OAuth policy, and OpenAPI from
the static CodeGPT tool definitions only. Claude `tools/list` output is never
...
any of them visible to an external CodeGPT client. Public names and input
...
```bash
sudo systemctl reload codegpt-runner
```
...
deduplicated, and capped at 64, and only the configured CodeGPT capability
...
```bash
CODEGPT_PROBE_CLAUDE_PROVIDER=1 \
cargo test -p codegpt-runner --bin codegpt-runner opt_in_real_claude_mcp_probe -- --nocapture
```
...
```bash
CODEGPT_TEST_CLAUDE_MCP=1 cargo test -p codegpt-runner --bin codegpt-runner opt_in_real_claude_mcp_smoke -- --nocapture
```
