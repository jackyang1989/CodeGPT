...
This page is the maintainer-level contract for CodeGPT tool-request tracing.
...
```bash
TRACE_ROOT=/var/lib/codegpt/tool-request-traces
...
`tool_handler_returned` proves only that CodeGPT handed a response to the HTTP
...
Full tracing is raw forensic capture. The trace path does not read the CodeGPT
