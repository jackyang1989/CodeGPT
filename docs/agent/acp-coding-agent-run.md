...
Protocol (ACP) coding agent from CodeGPT. It is deliberately narrower than a
...
not a CodeGPT Job. `CodingAgentRun` is a separate execution primitive whose
payload is ACP protocol state and structured coding-agent activity rather than a
shell command plus stdout/stderr.

This contract is based on current CodeGPT V1 architecture plus real dogfood of
...
ChatGPT / API client
  -> coding_agent_start
  -> exact CodeGPT Project + Runner + configured ACP provider
  -> CodingAgentRun R
...
CodeGPT must not turn ACP into a second implementation of CodeGPT file, shell,
...
CodeGPT Job J             independent process-execution primitive
```
...
A CodeGPT Job is stdout/stderr/process/exit oriented. A CodingAgentRun is
...
disabled. This is adapter/provider behavior: CodeGPT did not set those values in
the P0 no-override probes. It is the concrete reason that "no CodeGPT override"
...
For CodeGPT, cancellation therefore means cancellation of the active Run/turn.
...
These names and values are observations, not CodeGPT enums. P1 must validate
...
`session/request_permission` is an ACP agent-to-client request, not a CodeGPT
...
This reinforces the product boundary: CodeGPT performs one admission decision
for the Run, then the delegated agent owns its normal internal coding policy;
CodeGPT does not rerun `PermissionEvaluator` for every ACP tool action.
...
CodeGPT cannot infer success or failure from its exit code alone. The Run must
be treated as uncertain unless exact protocol recovery proves otherwise.

## 5. Configuration semantics

### `config` omitted or `{}` means no CodeGPT override

P0 corrects an important earlier assumption. For CodeGPT:
...
> CodeGPT inherits the selected Runner-owned ACP provider's effective defaults
...
The exact executable example is operator-specific; CodeGPT must not prescribe
...
Project binding gives CodeGPT three real guarantees:
...
CodeGPT file-tool path rules and may evolve independently.

For the current Codex adapter, the effective mode influences Codex sandbox and
approval behavior. CodeGPT may report the provider's sanitized advertised
configuration, but it must not translate that into a claim of CodeGPT Project
isolation unless CodeGPT separately enforces such isolation.

The P1 product description should therefore say **operator-configured delegated
local coding agent**. It must not promise parity with CodeGPT `read_files` /
`apply_text_edits` filesystem isolation.

## 8. Permission-request exceptional path

The normal CodeGPT authority decision happens once at `coding_agent_start`:

```text
caller auth + exact Project + provider fence + config override policy
  -> CodeGPT start admission decision
  -> ACP Run starts
...
CodeGPT `PermissionEvaluator`, because that would create a second per-action
...
`crates/codegpt-core/src/coding_agent.rs`) with bounded typed values for:
...
Add a focused ACP module under `crates/codegpt-runner` that:
...
4. Default CodeGPT config inheritance means **no ACP config override calls**;
...
6. ACP permission callbacks are real and exceptional; never auto-allow them and
   do not rerun CodeGPT `PermissionEvaluator` per agent action.
