...
The need is practical rather than a new inventory system. CodeGPT already knows
...
- one Runner is running on the same host as the CodeGPT Server, so Server
...
- service management on a host uses the ordinary host-local service mechanism
  rather than a special CodeGPT lifecycle.
...
```json
{
  "host_context": {
    ...
    "service": "CodeGPT Server lifecycle uses the normal host-local service mechanism.",
    "network": null,
    "architecture": "This host is the CodeGPT control/server host."
  }
}
```
...
service = "CodeGPT Server lifecycle uses the ordinary host-local service mechanism."
architecture = "This host runs the CodeGPT Server/control plane."
```
