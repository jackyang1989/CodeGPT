...
`codegpt-runner`; the CLI namespace that manages it is `codegpt runner ...`.
`codegpt` and `codegpt-runner` are separate executables. Operator lifecycle
...
a CodeGPT Server, registers the projects it is allowed to serve, and executes
...
| **CLI** | The `codegpt` operator/developer command. |
| **Runner** | The `codegpt-runner` process that executes repository work. |
...
`runner.toml` is the canonical config filename. A legacy directory containing only `agent.toml` remains readable for compatibility; if both names exist in the same config directory CodeGPT fails closed and asks the operator to resolve the ambiguity. `CODEGPT_RUNNER_CONFIG` is the current path override; the old `CODEGPT_AGENT_CONFIG` remains a compatibility alias.
...
```toml
id = "codegpt"
path = "/srv/codegpt/projects/codegpt"
name = "CodeGPT"
kind = "repo"
allow_patch = true
```
...
both old and new locations/fields are configured, CodeGPT fails closed instead
...
| Configured live Runner Skill roots | Operator-selected absolute directories on the Runner host | `operator_configured_guidance` | Live filesystem content that CodeGPT does not modify; supported scripts may execute through `run_skill_resource`; no install, activation, rollback, or package revision. |
...
Runner. CodeGPT does not modify files in configured roots or copy them into the
...
The Runner can directly host persistent stdio MCP providers for CodeGPT's built-in MCP gateway:
...
that started the Runner. CodeGPT transport/account credentials are filtered.
...
CodeGPT does not automatically source `~/.bashrc` or `~/.profile`.
...
JavaScript and TypeScript are external Node.js execution on the Runner. CodeGPT
...
such as async/await, ESM, and Node built-ins. CodeGPT does not type-check, invoke
...
properties, runtime namespaces, or import aliases. CodeGPT also does not use
...
Node's own `ExperimentalWarning` may appear on stderr. CodeGPT does not suppress
...
CodeGPT does not install or bootstrap npm dependencies, inject `node_modules`
...
```sh
CODEGPT_QUIC_ENABLED=true
CODEGPT_QUIC_LISTEN=0.0.0.0:8443
CODEGPT_QUIC_CERT=/etc/letsencrypt/live/<host>/fullchain.pem
CODEGPT_QUIC_KEY=/etc/letsencrypt/live/<host>/privkey.pem
CODEGPT_QUIC_ALPN=codegpt-runner/1
```
...
CodeGPT application `Ping`/`Pong` liveness, which remains on its own 30-second
...
`codegpt-runner` stops cleanly on `SIGINT`/`SIGTERM`. It does not daemonize
itself. For a supervised deployment, use `codegpt runner install --scope
...
`codegpt connect` or `codegpt runner start --profile <profile>`. Automatic
...
`%LOCALAPPDATA%\codegpt\runner-exit-diagnostics-v1\<runner-hash>\` (falling
back to `%USERPROFILE%\.local\state\codegpt` and then `%TEMP%\codegpt` through
...
`codegpt connect ... --oauth-local-ssh`.
...
`CODEGPT_RUST_ANALYZER` and `CODEGPT_GOPLS`. The gopls profile also forces
module/toolchain network access off and uses `-mod=readonly`; CodeGPT never
...
codegpt runner restart --profile <profile>
```
...
```bash
sudo codegpt runner install --scope system --profile <profile> \
  --user <runner-user> --working-directory /home/<runner-user>
sudo codegpt runner status --scope system --profile <profile>
```
