...
A Server launched by `codegpt share` or `codegpt run` uses the same generic Adaptive Runtime OpenAPI projection as an ordinary Server. Project-scoped authentication limits the caller to its ProjectGrant; it does not switch schema generation to a separate Connector capability registry.
...
The retired ProjectConnector Action names and host-side `codegpt task` review workflow are not projected as compatibility aliases.

## Management and safety

The OpenAPI model surface intentionally excludes users, API tokens, Runner tokens, pairing/enrollment, setup, doctor, npm, server management, and audit endpoints. Use the `codegpt` CLI for those tasks.
