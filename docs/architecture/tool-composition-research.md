...
Current CodeGPT already has strong primitive tools and several homogeneous batch
...
way to combine *heterogeneous canonical CodeGPT tools* in one model-facing MCP
...
work. CodeGPT can measure time from one inbound request through Server/Runner
processing and can see when the next request arrives. It still cannot observe the
model's private reasoning or prove why time outside CodeGPT elapsed. Performance
...
The inspected CodeGPT Next design already separates model-facing call economy
...
CodeGPT Next dogfood also found that a very large composed `Apply` schema could
...
CodeGPT should adopt the first principle before attempting generalized resource
...
That shape is more attractive for CodeGPT than a large user-facing JSON DAG:
...
A CodeGPT composition layer should follow these rules:
...
7. **The parent is not a transaction.** If one child effect succeeds and a later
   child fails, CodeGPT does not invent rollback. Existing child result truth
...
const [review, matches] = await Promise.all([
  tools.git_review_summary({ project: "agent:special:codegpt" }),
...
Long-running child execution now uses the existing CodeGPT Job path directly. E2a admits only structured `cargo_check` / `cargo_test` as consequential children and caps their nested synchronous handoff preference at five seconds without changing total `timeout_secs`. An unfinished validator returns the same canonical `job_id` and parser-ready ordinary Job continuation; E2a creates no second background-process or cell lifecycle and does not restart the child. `observe_jobs` intentionally remains outside nested Code Mode in E2a.
...
The interval between a completed CodeGPT response and the next inbound request
is outside CodeGPT. It may be model inference, host scheduling, user delay, UI
...
response handoff. CodeGPT-owned service time is `H_i - A_i`; the adjacent
...
service + outside-CodeGPT gap. Pairing uses hashed `ClientWindow` plus canonical
...
SLO targets: CodeGPT-owned service/failure SLIs and interaction-efficiency gaps
...
CodeGPT already has the canonical primitives. The composition layer should stay
...
- Window activity can still explain outer versus nested CodeGPT work without
...
3. How should nested tool schemas be made ergonomic without recreating the large
   composed-schema projection problem observed in CodeGPT Next?
