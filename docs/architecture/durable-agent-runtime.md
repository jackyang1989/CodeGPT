...
asynchronous work in CodeGPT. It builds on the implemented Agent/Conversation/Wake
foundation without turning CodeGPT into a generic swarm framework or workflow
scheduler.
...
is not the product definition. CodeGPT should first make Agent identity and work
...
CodeGPT already contains several concepts that use similar words. They are separate
...
Agent Task mutations should follow the existing CodeGPT durable patterns:
...
Only after A4a and A4b reveal repeated common machinery should CodeGPT consider a
...
prepare fence because CodeGPT can no longer prove that the Host did not receive the
...
This is a possible later product slice, not an A4b requirement and not CodeGPT's
north star.
