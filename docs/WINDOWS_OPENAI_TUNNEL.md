...
flowchart LR
    ...
    TC -->|HTTP streamable MCP\nBearer stays local| S[CodeGPT Server\n127.0.0.1:18080]
    R[CodeGPT Runner] -->|WebSocket| S
    R --> P[C:\\src\\your-repository]
```

Two boundaries matter:

1. ChatGPT selects **Connection: Tunnel** and never needs the local CodeGPT Bearer.
2. The Runner remains a standard CodeGPT Runner. The Tunnel changes only the private ChatGPT-to-MCP transport; it does not change how the Runner connects to or operates projects through CodeGPT.

## 1. Prerequisites

### CodeGPT

Install the current CodeGPT build and make sure the CLI, Server, and Runner binaries come from the same version/commit:

```powershell
codegpt --version
codegpt-server --version
codegpt-runner --version
```
...
Never print or commit the Tunnel ID, API key, CodeGPT Bearer, bootstrap token, or authorization-file contents.

CodeGPT uses a pinned and verified OpenAI `tunnel-client`; the implementation can also resolve it from `CODEGPT_TUNNEL_CLIENT_BIN` or `PATH`.

## 2. Initialize a Windows foreground Server

Use dedicated env/data paths so this topology does not overwrite another CodeGPT runtime:

```powershell
$envFile = Join-Path $HOME ".config\codegpt\openai-tunnel\codegpt.env"
$dataDir = Join-Path $HOME ".local\share\codegpt-openai-tunnel"
...
Check the returned config with `codegpt runner status --config <login-reported-runner-config>`. At this point the Server should see a normal independent Runner with `C:\src\your-repository` already registered. Add more projects later with the normal `codegpt project register --config ...` workflow when needed.
...
Keep the CodeGPT Bearer local and inject it through a file-backed `Authorization` header. Do not paste that Bearer into ChatGPT.

Before starting the long-lived daemon, run `tunnel-client doctor`. Require it to validate the Tunnel ID, Restricted control-plane API key, local CodeGPT MCP reachability, and local Bearer injection together.
...
`codegpt share --tunnel openai` automates the same categories of setup, runs `doctor`, and waits for `/readyz`. Manual operation is intended for an explicit independent Server/Runner topology or deep troubleshooting.
...
1. any descriptive name, such as `CodeGPT Windows`;
2. **Connection: Tunnel**;
3. the selected CodeGPT OpenAI Tunnel;
4. **Authentication: No authentication**;
5. acknowledge the custom MCP risk notice;
6. create/scan the app tools.

Why **No authentication**? The CodeGPT Bearer for the local MCP hop is already injected locally by `tunnel-client`. ChatGPT should not receive or store it.
...
1. confirm the Project registered during `codegpt login --project ...` is visible;
...
The project handle returned by CodeGPT is output, not setup input. Do not ask the user to invent a Runner/project runtime id.
...
### Pasting the CodeGPT Bearer into ChatGPT
...
The core transport is the same: local CodeGPT MCP, local Bearer injection, and OpenAI `tunnel-client`. `share` owns a temporary Server/Runner/session automatically; this guide keeps the Server and Runner explicit so they can behave like a normal long-lived topology and be diagnosed independently.
