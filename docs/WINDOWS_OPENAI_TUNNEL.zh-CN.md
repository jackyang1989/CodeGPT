...
本文是当前 Windows 独立前台 CodeGPT Server + Runner + OpenAI Secure MCP Tunnel 的深入配置指南。稳定的当前操作和排障步骤放在前面；文末单独保留 [2026-08-30 historical dogfood note](#historical-dogfood-note--2026-08-30)，用于保存真实验证证据，而不会让当时的版本、机器、项目、app 或 branch 看起来像当前配置要求。

## 适用场景

如果你的目标只是**正常长期使用 CodeGPT**，先看[完整使用指南](PERSONAL_SETUP.zh-CN.md)。本文是 Windows + OpenAI Tunnel 的深入配置与故障排查记录，只有在你明确选择这条私有网络入口、需要复现实操拓扑或排查 Tunnel 问题时才需要继续阅读。

你希望 Windows 机器本身承担完整的 CodeGPT runtime：

- CodeGPT Server 运行在 Windows 前台；
- CodeGPT Runner 作为独立 Runner 连接这个 Server；
- Server 不暴露公网端口；
...
1. ChatGPT 只选择 **Connection: Tunnel**，不需要拿到 CodeGPT 本地 Bearer。
2. Runner 仍然是标准 CodeGPT Runner；Tunnel 只解决 ChatGPT 到 MCP Server 的私有传输，不改变 Runner 连接 CodeGPT 和操作 Project 的方式。

## 1. 前置条件

### CodeGPT

安装当前 CodeGPT，确保三个 Windows binary 来自同一版本/commit：
...
不要打印或提交 Tunnel ID、API key、CodeGPT Bearer、bootstrap token 或 authorization-file 内容，也不要把它们放进文档、issue 或截图。

CodeGPT 会使用固定并校验的 OpenAI `tunnel-client`；当前实现也可以从 `CODEGPT_TUNNEL_CLIENT_BIN` 或 `PATH` 解析它。

## 2. 初始化 Windows 前台 Server

选择独立 env/data 路径，避免影响机器上已有的 CodeGPT runtime：
...
使用 `codegpt runner status --config <login-reported-runner-config>` 检查状态。此时 Server 应看到一个普通独立 Runner，并且 `C:\src\your-repository` 已经注册。以后需要增加更多 Project 时，再使用普通的 `codegpt project register --config ...` 流程。
...
CodeGPT Bearer 应保留在本机，通过 file-backed `Authorization` header 注入给 `tunnel-client`；不要把这个 Bearer 复制到 ChatGPT。
...
CodeGPT 的 `share --tunnel openai` 会自动完成相同类别的准备、`doctor` 和 `/readyz` 等待。手工拓扑只建议用于本文这种独立 Server/Runner 场景或排障。
...
1. 名称：任意，例如 `CodeGPT Windows`；
2. 连接选择 **隧道 / Tunnel**；
3. 从可用 Tunnel 中选择对应的 CodeGPT Tunnel；
...
因为 OpenAI Tunnel 到本机 CodeGPT MCP 的 Bearer 已由 `tunnel-client` 在本机注入。ChatGPT 不应该拿到或保存该 Bearer。

## 7. 通过 Connector 做端到端验收

Connector 创建成功后，如果当前对话还没出现新工具，先刷新 ChatGPT 窗口。然后验证真正的数据路径，而不只是 UI 创建成功：

1. 确认 `codegpt login --project ...` 阶段已经注册的 Project 可以看到；
2. 选择这个已注册 Project，并通过 Connector 读取一个已知文件；
3. 如果明确启用了写权限，用专用分支/安全小改动验证写入，并检查 Git diff。

CodeGPT 返回的 Project handle 是结果，不是设置时需要用户自己编造的输入；不要让用户手工拼 Runner/project runtime id。
...
只有最后几项也通过，才说明它真的具有和普通 CodeGPT Server + Runner 相同的开发使用体验。
...
底层核心链路相同：本地 CodeGPT MCP + 本地 Bearer 注入 + OpenAI `tunnel-client`。区别是 `share` 自己管理临时 Server/Runner/session，而本文显式拆开 Server 和 Runner，便于验证长期拓扑和排障。
