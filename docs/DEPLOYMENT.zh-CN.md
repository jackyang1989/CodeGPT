...
本文档只面向**生产与高级自托管**：长期 Server、多机器/多用户、systemd/Docker、反向代理和 operator 管理的网络配置。普通 Windows / macOS 工作站**不要从这里开始**；最推荐的路径是 [CodeGPT Desktop + 官方 OpenAI Secure Tunnel](desktop-install.zh-CN.md)。CLI 或已有 Server 再看[完整使用指南](PERSONAL_SETUP.zh-CN.md)；如果只想几分钟临时体验一个仓库，再看[快速试用](QUICK_START.zh-CN.md)。

## 组件

- `codegpt` —— 统一 CLI：项目工作流、Server/Runner 生命周期、接入与运维。
- `codegpt-server` —— Server 进程：暴露 REST、GPT Actions OpenAPI、MCP 与
  Runner endpoint。
- `codegpt-runner` —— 运行在持有仓库机器上的长驻 worker。
...
支持 Linux x64、Linux arm64、macOS x64、macOS arm64、Windows x64 与 Windows arm64。Windows 支持 CLI + Runner、显式前台 Server，以及显式本机 `codegpt share --tunnel cloudflare|openai|none`。Windows x64 支持 managed Cloudflare 获取；固定版本 upstream 没有官方 Windows ARM64 artifact，因此 ARM64 使用 Cloudflare 时需要受信任的显式/`PATH` binary。managed OpenAI `tunnel-client` 支持 Windows x64/arm64。CodeGPT 仍不支持 Windows Server/Runner service 托管生命周期；Windows 上应显式以前台方式运行。npm 包装器要求 Node.js 18 或更新。Linux x64 native artifact 以 glibc 2.17 或更新为兼容基线。
...
这会生成 `codegpt`、`codegpt-server`、`codegpt-runner`。
...
如果 Server 与 Runner 位于不同机器，把 loopback URL 替换为 Server 可访问的 HTTPS URL，并按下文配置 Server listener/public URL 与受信任的反向代理或 tunnel。不要把 Server bootstrap token 或 env 文件复制到 Runner 机器。Windows 仍不支持 `codegpt server install/start/stop/restart/logs/uninstall` 与 `codegpt runner install`；Ctrl-C 或 Ctrl-Break 会结束前台 runtime。
...
机器重启后，重新运行同一条 `connect` 或使用 `codegpt runner start --profile <profile>`。

这不是刚完成 bootstrap 的自托管 Server 的首次 enrollment。那种情况应把 bootstrap
administrator token 留在 Server，并按下文 pairing / `codegpt login` 流程接入仓库机器。
...
2. 安装 `@yyjeqhc/codegpt`，运行 `codegpt server init`，安装
   `codegpt-server` 服务。
3. 配置反向代理并把 `CODEGPT_PUBLIC_URL` 设为精确的公网 HTTPS origin。
4. 在 server 上创建短期 pairing code，并在持有仓库的机器上运行
   `codegpt login <server-url> --code <code>`。
5. 在该仓库机器上安装 `codegpt-runner` 服务。
6. 运行 `codegpt ops status --strict`；之后才导入 GPT Actions schema 或添加
   MCP connector。

### Server 设置

初始化 Server env 文件（创建 bootstrap `CODEGPT_TOKEN` 与监听/数据设置）：
...
省略 `--working-directory` 时，`server install` 会优先读取所选 env 文件中的 `CODEGPT_DATA`，再回退到平台默认值。安装前会在任何 systemd mutation 之前拒绝缺失/不可执行 binary、缺失 WorkingDirectory、不可读 env file 或不存在的显式 User/Group。`server status` 也会默认从同一 env 的 `CODEGPT_ADDR` 派生本地 HTTP probe；显式 `--url` 始终优先。

受管 Linux 部署会明确拆分职责：`codegpt.socket` 持有固定的
`CODEGPT_ADDR` listener，`codegpt.service` 只持有 Server process。该结构建立后，
...
正常替换路径不要 restart `codegpt.socket`。旧 Server drain 到新 Server 继承 listener
...
CodeGPT 会先让 process-local drain fence 生效，再要求 Salvo 停止 accept 新 connection
...
正在运行的 legacy direct-bind `codegpt.service`，第一次迁移属于单独的一次 migration
...
`CODEGPT_PUBLIC_URL`，并在 `127.0.0.1:8080` 前面配置反向代理。支持 Nginx；
named Cloudflare Tunnel 也是有效入口。同一 hostname 必须承载普通 HTTPS 请求
与 `/api/agents/ws`（Cloudflare 支持 WebSocket upgrade）。CodeGPT CLI 不会自动
配置反向代理或 tunnel。
...
`codegpt login` 是 canonical 客户端入口：它自动派生唯一设备名、兑换 pairing code，并写入客户端侧 `codegpt-user-token` 与 `runner.toml`。`--allowed-root` 只授予 Project 注册 authority，`--project` 才表示要注册的实际 existing workspace；生成的 `project_registry_dir` 是 registry 而不是 workspace root。如果 login 时没有传 `--project`，应在 project-bound 工作前执行 `codegpt project register --config <login-reported-runner-config> /path/to/repo`。需要显式设备 identity 或不同的本地 base directory 时，使用 `login` 已文档化的 `--device` 与 `--dir`；不再有单独的 compatibility enrollment 命令。
...
`codegpt runner install` 支持 user 或 system scope。非 root 用户默认 user scope；
root 默认 system scope。

**User scope** 使用 `systemctl --user`，unit 写入
`$XDG_CONFIG_HOME/systemd/user`，配置放在 `$XDG_CONFIG_HOME/codegpt`，无需
`sudo`：
...
可显式运行 `sudo loginctl enable-linger <runner-user>`；CodeGPT 不会自动修改
lingering。
...
生命周期命令使用相同 `--scope`。示例文件在 `deploy/`（`codegpt.env.example`、
`codegpt.service.example`、`codegpt-runner.toml.example`、
`codegpt-runner.service.example`、`nginx.codegpt.example.conf`）。

## Docker（仅 Server）

仓库提供 server-only Dockerfile 与 Compose 部署，运行 `codegpt-server` 与管理
CLI；有意不包含 Runner、项目仓库与工具链。正式 Release 会向
`ghcr.io/yyjeqhc/codegpt-server` 发布同时支持 `linux/amd64` 与 `linux/arm64`
的多架构镜像。
...
定义，并在本地 materialize 为 `codegpt-server-compose.yaml`。因此即使使用
...
随后通过私有 `.codegpt-bootstrap.receipt` 依次记录 `AssetsPrepared`、
...
| `[skills].roots` | 可选的 Runner 本机绝对 live Skill roots；CodeGPT 不修改其中内容，受支持脚本可经 `run_skill_resource` 执行，也不会复制进 managed Skill Store。 |
...
前台测试可运行 `codegpt-runner --profile workstation`。高级手动生成配置用
`codegpt runner init`。

## OAuth2

Server 没有公网 origin 时 OAuth2 仍默认关闭。使用 `codegpt server init --public-url https://your-domain.example` 时，初始化会写入 public URL、以该 URL 作为 issuer 启用 OAuth，并为普通 hosted connect 启用 shared-key OAuth bridge。手工维护 env 时等价配置为：
...
只有明确需要 managed-user OAuth identity 时，才使用高级 `codegpt login` 流程，再执行 `codegpt connect ... --auth managed-oauth --oauth-redirect-uri ...`；`--user` 仅用于该模式。
...
`allowed_scopes` 限制 OAuth client 最多可以请求哪些权限。CodeGPT 新增 permission 时不会静默扩大已有 client。要修改现有 client，请把期望保留的完整、非空 allow-list 提交到 `POST /api/oauth/clients/update_scopes`。真实变化会让旧 OAuth grant 失效并要求重新授权；提交相同 canonical list 是 no-op。安全模型见[认证](AUTH_MODEL.zh-CN.md#oauth2)。

如果启用 ChatGPT MCP host-file import，请把精确的 server-generated OAuth client id 配入 `CODEGPT_OAUTH2_TRUSTED_MCP_FILE_CLIENT_IDS`。重新创建 client 会生成新 id，因此应把更新这个设置作为一次显式 trust rotation。Client display name 与 redirect URI 不能替代该精确 client id。

对于绑定到 loopback、并通过 OpenAI Secure Tunnel 以本机注入 user API token 访问的 operator-controlled Server，还有一个独立的 local-only 例外。设置 `CODEGPT_MCP_TRUST_LOOPBACK_API_TOKEN_FILE_IMPORT=true` 后，可在该路径上信任 ChatGPT host-file rewrite。非 loopback bind 或非 user API credential 会忽略该 flag；network-accessible Server 应保持未设置。
...
refresh-token scope，不授予额外 CodeGPT 权限。
...
`CODEGPT_AUTHORITY_MODE` 控制有后果的 runtime 工具是自动执行还是需要人工审批：
...
job 取消语义）。`CODEGPT_PERMISSION_MODE` 支持明确映射：`dev_auto_approve` → `trusted_agent`，
...
1. `codegpt ops status ... --strict` 通过。
2. `POST /api/runtime/status` 返回 `service=codegpt` 与预期公网 URL。
