...
Use this path for CLI setup, an existing Server, or a setup that needs more control than Desktop provides. For a normal Windows or macOS personal installation, the recommended first path is [CodeGPT Desktop + the official OpenAI Secure Tunnel](desktop-install.md). In either case, the full CodeGPT model is the same: ChatGPT connects to a regular Server, and a persistent Runner uses the real projects, Git checkout, compiler, and test tools on your machine.

If you only want to try CodeGPT for a few minutes, use the [Quick Trial](QUICK_START.md) and `codegpt share` instead. `share` is a temporary, single-project launch of the ordinary Adaptive Runtime with project-scoped authentication; it ends when the command exits.
...
codegpt server init \
  ...
  --data-dir "$HOME/.local/share/codegpt" \
  --env-file "$HOME/.config/codegpt/codegpt.env"
...
A Tunnel solves reachability only. Do not switch to `codegpt share` merely because you use a Tunnel; the full setup continues to use the regular Server started above.
...
```bash
codegpt project register --config <runner-config> /path/to/another-repo
```
...
Once the Runner is online, CodeGPT has an execution path to the machine's files, Git checkout, compiler, tests, and other development tools.
...
If you chose OpenAI Secure MCP Tunnel, login does not need `--print-mcp-config` for ChatGPT. ChatGPT normally uses Tunnel + No authentication while the local tunnel client injects the CodeGPT credential. Follow the Tunnel guide instead of copying the local Bearer into ChatGPT.
...
| --- | --- | --- |
| Try one repository for a few minutes | `codegpt share` | One command, temporary, single-project, ordinary Adaptive Runtime, ends on exit |
...
```bash
codegpt server status --env-file <server-env-file>
codegpt runner status --config <runner-config>
codegpt ops status --server-url <server-url> --token-file <login-reported-codegpt-user-token>
```

See [Troubleshooting](TROUBLESHOOTING.md) for detailed failures. The [CLI reference](CLI.md) documents every command and internal configuration field, but you do not need to understand those internals before successfully using CodeGPT.
