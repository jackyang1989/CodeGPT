...
This guide is for ordinary CodeGPT coding/review work. It describes the model-facing workflow, not the internal continuity, audit, or transport protocols used to implement it.

## Normal loop

The ordinary CodeGPT coding loop is intentionally small:
...
Use `work_on_project` for both a new coding task and an explicit continuation. CodeGPT keeps bounded Workflow Session evidence so validation, review, and handoff can refer to the same unit of work, but that Session is not an authentication credential and does not widen project access.

For ordinary use you do not need to reason about CodeGPT's internal continuity or audit fields. Those are implementation/maintainer contracts.
...
The exact matching metadata and transactional protocol are maintainer details; see the tool contract/tests when developing CodeGPT itself.
...
When developing CodeGPT itself, use the maintainer contracts rather than expanding this user guide:
