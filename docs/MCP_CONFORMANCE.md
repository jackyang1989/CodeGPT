...
CodeGPT tracks MCP wire compatibility with the upstream
...
```bash
CODEGPT_MCP_CONFORMANCE_HARNESS_DIR=/path/to/conformance \
...
through `CODEGPT_MCP_CONFORMANCE_HARNESS_DIR` is read-only from this script's
...
supplied report directory must be empty or carry the CodeGPT ownership marker
...
disabled in the isolated test config. The referee therefore reaches real CodeGPT parsing, dispatch and
...
scenarios intentionally expect named reference-fixture tools. When CodeGPT does
...
way to inject CodeGPT authorization headers, so an authentication-blocked
...
|---|---|
| `genuine_protocol_failure` | The intended CodeGPT behavior was reached and a protocol requirement is known to be violated. |
...
- metadata containing the CodeGPT source SHA, referee SHA, profile, upstream
...
- the coverage-aware CodeGPT summary.
