...
这里整理 CodeGPT 部署中常见问题的实用检查。排障时不要粘贴或分享真实 tokens、env files、`Authorization` headers 或完整 `runner.toml` files。

## 运维检查清单

Server：

- `codegpt --version` 能打印版本。
- `codegpt server status --env-file /etc/codegpt/codegpt.env` 报告本地 server reachable。
- 在 server host 上，`curl http://127.0.0.1:8080/openapi.json` 返回 OpenAPI JSON。
- 如果使用 nginx 或其他 reverse proxy，public HTTPS 可访问。

Client：

- `codegpt-runner --version` 能打印版本。
- Hosted quick-start 使用
  `codegpt runner status --profile <connect 输出的 profile>`，应显示
  `runner mode: hosted local process`、`runner active: true` 和
  `client online: yes`。
- `codegpt runner status --profile workstation` 能读取本地 Runner config（`runner.toml`）。
- canonical project 的 `codegpt doctor` 通过；managed deployment 则使用
  `codegpt ops status --strict --server-url https://your-domain.example`。
- `list_runners` / `runtime_status` 显示 Runner online。

## 常见问题

### `codegpt connect` 无法完成

`connect` 最多等待 15 秒来确认完整链路：Server 可访问、同 key 能看到 Runner、
同 key 能看到目标项目。错误信息会给出该 profile 的 Runner 日志路径。先检查：

```bash
codegpt runner status --profile <connect 输出的 profile>
codegpt runner logs --profile <connect 输出的 profile> --lines 100
```
...
Hosted 日志位于
`$XDG_STATE_HOME/codegpt/clients/<profile>/runner.log`（或
...
如果本地 HTTP 正常但 public HTTPS 不通，检查 nginx upstream host/port 和 TLS 配置。CodeGPT CLI 不会自动配置 reverse proxy。
...
### Client 显示 `codegpt: command not found`
...
这在 Runner-only client machines 上可能是正常的。Runner-only client 需要公开 `codegpt` CLI 和 `codegpt-runner`；`codegpt-server` 只在 server host 上需要。
...
刚升级，确认 public HTTPS 已指向新 service，并检查 `journalctl -u codegpt`
...
`--service-file`，后续命令也要传同一 absolute path 与 scope。CodeGPT 不会静默
