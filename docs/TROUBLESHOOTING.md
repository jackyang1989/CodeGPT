...
Practical checks for common CodeGPT deployment issues. Do not paste or share real tokens, env files, `Authorization` headers, or complete `runner.toml` files while debugging.

## Operational checklist

Server:

- `codegpt --version` prints a version.
- `codegpt server status --env-file /etc/codegpt/codegpt.env` reports the local server reachable.
...
- `codegpt-runner --version` prints a version.
- For hosted quick-start, `codegpt runner status --profile <profile-from-connect>`
  reports `runner mode: hosted local process`, `runner active: true`, and
  `client online: yes`.
- `codegpt runner status --profile workstation` can read the local Runner config (`runner.toml`).
- `codegpt doctor` passes for a canonical project, or advanced
  `codegpt ops status --strict --server-url https://your-domain.example`
  passes for a managed deployment.
- `list_runners` / `runtime_status` shows the Runner online.

## Common issues

### `codegpt connect` cannot finish
...
```bash
codegpt runner status --profile <profile-from-connect>
codegpt runner logs --profile <profile-from-connect> --lines 100
```
...
The hosted log is
`$XDG_STATE_HOME/codegpt/clients/<profile>/runner.log` (or the equivalent
...
`codegpt tokens generate` is offline material generation only. It does not
...
Re-run the same `codegpt connect` command. The profile lock prevents duplicate
...
```bash
systemctl status codegpt
journalctl -u codegpt
curl http://127.0.0.1:8080/openapi.json
```

If local HTTP works but public HTTPS does not, check the nginx upstream host/port and TLS configuration. CodeGPT CLI does not automate reverse proxy setup.
...
### Client says `codegpt: command not found`

Install or symlink the CLI onto the client's `PATH`, for example:

```bash
sudo ln -s /opt/codegpt/bin/codegpt /usr/local/bin/codegpt
```
...
That can be acceptable on Runner-only client machines. Runner-only clients need the public `codegpt` CLI and `codegpt-runner`; `codegpt-server` is only required on server hosts.
...
codegpt runner logs --scope user --lines 100

# Administrator-managed system service
sudo codegpt runner status --scope system
sudo codegpt runner logs --scope system --lines 100
```
...
new service and check `journalctl -u codegpt` for startup or auth errors.
...
and scope to later commands. CodeGPT does not silently migrate or overwrite a
