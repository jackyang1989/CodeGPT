...
CodeGPT has several ways to authenticate because Server administration, model/API access, and Runner connectivity are different trust boundaries. Ordinary users do **not** need to learn CodeGPT's internal identifier vocabulary to use it safely.

## The short version

For normal daily use, follow [Full Setup](PERSONAL_SETUP.md): redeem a one-time login code, let `codegpt login` create the local user and Runner credentials, and use the connection values it reports for ChatGPT.

If you are using an existing hosted shared-key Server, use the shared key supplied by its operator with `codegpt connect`. If you are only trying one repository temporarily, use the credential printed by `codegpt share` for that run.
...
CodeGPT also uses non-secret IDs and opaque tool state internally. Users normally do not need to learn their formats.
...
`CODEGPT_TOKEN` is the Server's bootstrap/admin credential. `codegpt server init` stores it in the Server environment. Use it for initial administration, user creation/pairing, and emergency recovery. Do not use it for MCP, GPT Actions, Runner connectivity, or ordinary daily work.
...
A shared key is the simple hosted connection credential used by `codegpt connect`. The same key can authenticate the MCP/runtime client and the matching Runner group. Different shared keys remain isolated from each other.
...
After verification, CodeGPT keeps the non-secret authorization metadata it needs internally. It is not another credential the user needs to copy or manage.

## OAuth2

OAuth lets MCP/GPT clients use the authorization-code flow instead of storing a long-lived PAT in the client. Register the exact callback URL required by the client and follow the connection output from `codegpt share --auth oauth` or `codegpt connect --auth oauth`.
...
- **refresh token** — protocol credential used to refresh access; `offline_access` adds no CodeGPT permission by itself;
- **allowed scopes** — the maximum CodeGPT permissions that client may request.

CodeGPT never silently expands an existing OAuth client's allowed permissions when new scopes are introduced. Changing the allow-list is an explicit administrative action and invalidates old grants so the client must authorize again.

For ordinary hosted shared-key OAuth, `codegpt connect ... --auth oauth` keeps the Runner on its shared key and gives the MCP client a separate OAuth credential. `--oauth-computer-permissions` explicitly enables the additional Computer permissions offered by that flow; `--oauth-local-mcp` explicitly enables access to configured Runner-owned local MCP providers. Managed-user OAuth remains a separate advanced flow (`--auth managed-oauth`).
...
| --- | --- |
| `CODEGPT_TOKEN` | Server env file, commonly `/etc/codegpt/codegpt.env` |
| Managed user PAT | `~/.config/codegpt/<server-slug>/<user>/codegpt-user-token` |
