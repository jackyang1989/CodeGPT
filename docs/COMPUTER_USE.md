...
This page tracks the implementation order for CodeGPT Computer Use. It is a roadmap, not a duplicate semantic or authorization specification. Current authority, scope, retry, and privacy contracts remain defined by [Authentication](AUTH_MODEL.md#computer-observation-and-control-authorization), the model-facing tool definitions, and the Runner implementation.

## Current substrate

Computer Use is semantic-first rather than coordinate-first. The model-facing surface is now deliberately small:

```text
computer_observe(action=targets|windows|displays|applications|...)
  -> computer_observe(action=accessibility_tree|find_elements|element_state|snapshot_window|snapshot_display|read_clipboard)
  -> computer_control(action=launch_application|activate_window|press|focus|scroll_to_element|key|input_text|pointer_move|pointer_click|write_clipboard)

computer_save_snapshot(...)  # separate Computer-read + Project-write boundary
```

`computer_observe` is statically and dynamically read-only. `computer_control` is effectful, but exact scope, permission, Session, and Runner-capability policy is resolved from the closed action before dispatch. `computer_save_snapshot` stays separate because it crosses into Project artifact mutation. The precise Runner operations and capability names (`computer_list_windows`, `computer_snapshot_display`, `computer_pointer_click`, and so on) remain internal protocol/native identities for rolling compatibility and failure attribution; they are no longer separate model-facing tool identities.

The contract keeps read observation separate from effects. Effects name exact opaque Runner/window/element identities, revalidate native state before acting, and never gain AppleScript, shell, paste, coordinate, or synthetic-input fallbacks. A response lost after possible effect dispatch is `outcome_unknown`; callers observe current UI state and reconcile before deciding whether another effect is safe.

### Windows and macOS application discovery and bounded launch

Windows and macOS expose the same two narrow model-facing actions: `computer_observe(action=applications, client_id, limit?)` and `computer_control(action=launch_application, client_id, application_id)`. The precise Runner operations remain `computer_list_applications` and `computer_launch_application`. Discovery is bounded to at most 64 returned entries and reports `applications`, `count`, and `truncated`; each entry contains only a fresh opaque `application_id` plus a bounded display name. Windows uses the AppsFolder Shell namespace. macOS asks Foundation for the OS-defined user/local/system application and administration-application directories, then performs a bounded breadth-first enumeration with depth 2 and the shared 1024-entry native scan ceiling. macOS skips symlinks, never escapes a canonical application root, and treats every `.app` candidate as a leaf without traversing bundle contents. Neither implementation scans `PATH`, invokes a process/shell or AppleScript, or recursively scans arbitrary filesystems. There is no pagination, search DSL, executable-path result, package/AUMID/bundle result, or generic application framework.
...
Launch accepts only `client_id` and the opaque `application_id`. Windows submits the freshly revalidated Shell item by PIDL. macOS submits the freshly revalidated application URL to public `NSWorkspace.openApplication(at:configuration:completionHandler:)` exactly once with activation, prompting/error UI, Recent Items, hide/hide-others, forced new instance, and alternate-running-copy substitution disabled; arguments and environment are explicitly empty, and no document, URL target, or custom Apple event is supplied. It accepts no caller executable/path/bundle ID, argv, cwd, environment, shell command, PowerShell/cmd/script, URL/protocol launcher, `run_process` fallback, focus, activation, or input. The launch request is not a promise to create a new process, window, or instance: either platform may route it to the exact already-running application. Success means only that the native launch request completed successfully; it does not mean a usable window is ready, and CodeGPT does not additionally activate or focus anything.

The canonical workflow is therefore:

```text
computer_observe(action=applications)
  -> computer_control(action=launch_application)
  -> fresh computer_observe(action=windows)
  -> identify the exact new/current surface
  -> computer_control(action=activate_window) only if needed
  -> semantic work
```

Pre-native validation failures, including malformed or `stale_application` IDs and definite non-dispatch, are `not_started` with `state_changed=false`. The asynchronous macOS request has a five-second completion bound: only an unambiguous `(application, no error)` callback is success. Once either native launch may have been dispatched, timeout, callback/response loss, Runner interruption, native failure after submission, ambiguous completion, or inconsistent success metadata is `outcome_unknown`; there is no automatic replay or launch idempotency key, and callers must reconcile with a fresh `computer_observe(action=windows)` before deciding whether another launch is safe. The two additive Runner capabilities, `computer_application_discovery` and `computer_application_launch`, default false when missing, are advertised by native Windows and macOS Runners, and never imply one another or any existing observe/control capability. Other platforms fail closed.

Durable audit keeps discovery result metadata to `count`/`truncated` only; application names and native identities are omitted. Launch audit may retain the opaque `application_id` plus bounded lifecycle/success metadata, never the native PIDL, application URL/path, executable path, bundle/AUMID/package/filesystem identity, or launch parameters.

### Full-display discovery and exact display snapshot

Full-display observation is a wider privacy authority than ordinary window observation. `computer_observe(action=displays, ...)` and `computer_observe(action=snapshot_display, ...)` therefore require both `computer:read` and the explicit `computer:display_read` scope, plus the independent Runner capability `computer_display_observe`. Missing capability fields are false and are never inferred from `computer_observe`, region snapshot support, or platform identity. Exact native implementations are available on Windows and macOS; other unproven backends keep the capability false and fail closed.

Discovery returns at most 16 entries with only `display_id`, display-relative source-pixel `width`/`height`, and `primary`, plus `count`/`truncated`. Every list creates fresh opaque process-local IDs and replaces the prior display registry, so previous IDs become stale and Runner restart invalidates all of them. On macOS this source geometry is the current `CGDisplayMode` backing/source pixel geometry from `pixelWidth`/`pixelHeight`; on Retina/HiDPI configurations it can therefore be larger than Quartz display bounds or UI logical/"looks like" dimensions. Native monitor/interface identity, device paths, logical/global desktop coordinates, scale/DPI mapping, and other topology remain Runner-private.

The underlying `computer_snapshot_display` Runner operation, reached through `computer_observe(action=snapshot_display)`, captures exactly one previously discovered display; there is no virtual-desktop mosaic, caller region, global coordinate, pointer/click input, activation, or fallback. On Windows the Runner re-enumerates native displays and requires an exact private monitor-interface identity plus unchanged display-relative source geometry. On macOS it binds the public ColorSync display UUID and bounded CoreGraphics device identity to the discovery-time native display id and current-mode source pixel geometry, rejects ambiguous identity or a backing-mode geometry change, and uses the no-UI Screen Recording preflight before exact-display capture. The resulting `CGImage` pixel width/height must exactly equal those bound source pixels. Both implementations revalidate identity again after capture so a hotplug/replacement/mode-change race discards the bytes rather than accepting the wrong display. `max_width`/`max_height` reuse the existing bounded JPEG pipeline and only apply aspect-preserving downscale; they never upscale. The output separates `source_width`/`source_height` from encoded `width`/`height`; logical/global display coordinates, scale/DPI, native topology, and any future source-pixel-to-native-pointer mapping remain Runner-private.
...
### macOS and Windows snapshot-fenced coordinate pointer

macOS and Windows expose the same `computer_control(action=pointer_move, ...)` and `computer_control(action=pointer_click, ...)` model surface, routed to the precise `computer_pointer_move` / `computer_pointer_click` Runner operations. The click slice is fixed to one left click; there is no caller button, double/right/middle/X click, drag, wheel, button-down/up primitive, window-relative coordinate API, or global desktop coordinate API. `x`/`y` are integer coordinates in the exact display-local `source_width`/`source_height` space returned by `computer_observe(action=snapshot_display)`, never the downscaled JPEG dimensions.

The `snapshot_generation` is a one-effect freshness fence. Pointer admission requires the latest successful generation for the current process-local `display_id`, private native identity, and source geometry, and the generation must still be unspent. A newer snapshot, fresh display list, Runner restart, display replacement/hotplug, geometry mismatch, or prior pointer use makes it stale. Platform mapping, exact native revalidation, permission and initial shared-input checks, and all fallible native event construction complete before the generation is spent. Crossing the shared effect-admission boundary marks the generation spent before the first native pointer post. A later final pre-post fence can therefore return a spent `not_started` only when it proves that no native pointer event was posted; after any native post, an unprovable result is `outcome_unknown`. Either spent result requires a fresh `computer_observe(action=snapshot_display)` and the old generation is never restored or retried.

The Windows backend re-enumerates and exactly revalidates the private display identity and geometry, then maps display-local source coordinates into Windows virtual-desktop absolute input coordinates. The xcap monitor rectangle union must exactly equal Windows virtual-desktop metrics or the effect fails closed. A move is one absolute virtual-desktop `SendInput`; a click first submits that move, proves the exact cursor with `GetCursorPos`, performs a fresh held-state check, and only then submits one bounded `LEFTDOWN` + `LEFTUP`. Zero move insertion after generation spend is definite `not_started`; partial insertion or any unprovable postcondition is `outcome_unknown`.

The macOS backend supports only exact 0-degree display rotation. It maps source pixels independently per axis as `global_origin + (source_coordinate / source_extent) * global_bounds_extent` using the exact current `CGDisplayBounds`, without hard-coded scale, integer rounding, encoded-image dimensions, or `CGDisplayPixelsWide/High`. It revalidates the same private display identity/source geometry and exact bounds/rotation before preparation and again immediately before the first native post. Both the initial and final held-state guards read `CombinedSessionState`: all 32 Quartz mouse-button slots must be up for move/click, and click additionally requires Shift/Control/Option/Command inactive. Event-post permission uses `CGPreflightPostEventAccess` without requesting permission.

macOS prepares one `CombinedSessionState` `CGEventSource` plus the complete fixed event set before generation spend: exactly one `MouseMoved` for move, or one `MouseMoved` plus one `LeftMouseDown` and one `LeftMouseUp` for click. Dispatch posts only at the HID event tap and has no warp, NSEvent injection, AppleScript, fallback, repair, or retry. After the single `MouseMoved` post, the Runner uses a short monotonic bounded settle window with repeated read-only public Quartz cursor-location observations (`CGEventCreate(NULL)` plus `CGEventGetLocation`); the first observation that exactly equals the prepared fractional global target proves the move, with no epsilon or rounding. Click then performs a second fresh held-state proof before posting down/up. After `LeftMouseUp`, the same bounded exact observation settle must again prove the cursor target before the shared left-button state is required to be up. Quartz posting exposes no insertion-count acknowledgement, so exhaustion without an exact observation, interruption, dirty second click preflight, or any failed final proof after a post is `outcome_unknown`; button events are not attempted when the move proof or second held-state proof fails.

Pointer input uses the shared interactive desktop and does not isolate the agent from simultaneous human input. The Runner never releases or repairs human-held mouse/modifier state. Public results and durable audit contain only the existing opaque display/generation/source-coordinate lifecycle metadata; native display identity, bounds/origin, rotation, global target, scale/mapping, cursor readback coordinates, event source, and raw held-input state remain Runner-private.

Canonical pointer flow:

```text
computer_observe(action=displays)
  -> computer_observe(action=snapshot_display)
  -> model computes source-space x/y
  -> computer_control(action=pointer_move OR pointer_click)
  -> fresh computer_observe(action=snapshot_display) / semantic observation
  -> decide next effect
```

Pointer control requires all four scopes `computer:read`, `computer:display_read`, `computer:control`, and explicit `computer:pointer_control`, plus the independent `computer_pointer_control` Runner capability. The capability defaults false and is advertised by the native macOS and Windows implementations only. The tools perform no implicit snapshot, display/window listing, activation, focus, retry, shell/script/process fallback, clipboard operation, OCR, or browser automation.

### Windows and macOS bounded Unicode-text clipboard

Windows and macOS expose two independent global clipboard actions: `computer_observe(action=read_clipboard, client_id)` and `computer_control(action=write_clipboard, client_id, text)`. The precise Runner operations remain `computer_read_clipboard` and `computer_write_clipboard`. The native plain-text representation is `CF_UNICODETEXT` on Windows and `NSPasteboardTypeString` on the macOS general pasteboard. Neither path exposes binary/image/HTML/RTF/file-drop/file-URL/custom formats, native format IDs, clipboard history, delayed rendering, owner/window handles, change or sequence numbers, or arbitrary format enumeration. There is no clipboard generation/token framework. The independent Runner capabilities `computer_clipboard_read` and `computer_clipboard_write` default false, are not inferred from each other or from ordinary Computer observation/control, and are advertised by native Windows and macOS implementations only.

Clipboard read is a pure observation requiring both `computer:read` and explicit `computer:clipboard_read`. Windows opens the global clipboard only for one bounded read, performs no internal retry, checks `CF_UNICODETEXT`, bounds native storage before locking it, requires a UTF-16 terminating NUL within that storage, and converts to UTF-8. macOS requests only `NSPasteboardTypeString` from `NSPasteboard.generalPasteboard`; it does not enumerate or request other types and checks the native string's UTF-8 byte length before copying it into the result. Both paths reject rather than truncate text beyond 16 KiB. Absence of readable plain Unicode text is `available=false`; an existing empty string is `available=true` with empty `text`. The returned model text is the authorized data plane, but durable audit retains only bounded metadata such as `available` and `text_bytes`: clipboard text, native string/storage bytes, hashes, handles, owners, and change counts never enter durable audit.

Clipboard write is an independent effect requiring both `computer:control` and explicit `computer:clipboard_write`; write authority does not grant read authority. Caller text must be non-empty, NUL-free, and at most 16 KiB of UTF-8. On Windows, UTF-16 conversion, terminating NUL construction, checked allocation sizing, movable global-memory allocation/copy, and a short-lived invisible Runner-owned non-NULL message-only HWND all complete before mutation. After `OpenClipboard(owner_hwnd)` succeeds, the first successful `EmptyClipboard` is the effect boundary: failure before it is definite `not_started`; `SetClipboardData` or close failure after it is `outcome_unknown`. Successful `SetClipboardData(CF_UNICODETEXT, ...)` transfers HGLOBAL ownership to Windows.
...
`computer_control(action=write_clipboard)` is intentionally a **replacement** operation: the native clear removes any previous image, rich-text, HTML/RTF, file-drop/file-URL, or custom formats before installing only the bounded plain Unicode text. It does not merge or preserve rich formats. Neither clipboard action pastes anything, sends a paste shortcut, focuses/activates a target, or becomes a fallback for `computer_control(action=input_text)`, key input, semantic control, or pointer operations. An `outcome_unknown` write is never blindly retried; a caller that separately holds clipboard-read authority may explicitly issue a fresh `computer_observe(action=read_clipboard)`, while a caller without that read scope must retain the unknown outcome rather than bypass authorization. Windows behavior is production-validated; macOS parity has native compile and non-mutating focused test coverage, with live shared-clipboard validation still pending.

## Near-term slices

### CU-4 — semantic element finder

The bounded semantic finder is exposed as `computer_observe(action=find_elements)` over the existing Accessibility observation path; the implementation remains a Control-side adapter rather than a new Runner query protocol.
...
Expose one narrow `computer_control(action=activate_window)` effect that activates/raises an already observed exact `surface_id`, while retaining the precise `computer_activate_window` Runner operation.
...
Expose low-cost read-only state needed to choose the next semantic action without returning sensitive values. Prefer normalized CodeGPT affordances over platform-specific AX vocabulary, for example:
...
The first implementation uses a positive process-local generation per observed surface and does not add a wall-clock TTL or observation timestamp. `computer_observe(action=accessibility_tree)` and the Control-side finder expose the generation attached to their fresh element handles. `computer_observe(action=element_state, surface_id, element_id)` re-resolves an existing handle and reports that same generation; it neither creates a new handle nor advances generation. `value_empty` is actionable only for supported unprotected text input and is `null` for protected/secure content or when unavailable.

Stale recovery should remain re-observation, normally:

```text
stale_element -> computer_observe(action=find_elements, ...) -> new element_id
stale_surface -> computer_observe(action=windows) -> new surface_id
```

The model-facing failure envelope makes the same fact explicit as `recovery_kind=reobserve`. When the failed request proves the complete target, `suggested_call={tool, arguments}` supplies a parser-ready observation using its exact `client_id`: `stale_surface -> computer_observe(action=windows)`, `stale_application -> computer_observe(action=applications)`, and `stale_display -> computer_observe(action=displays)`. A stale or spent snapshot generation can suggest `computer_observe(action=snapshot_display)` with the original `display_id`, without reusing the spent generation. When the original finder filters or complete target are unavailable, only a non-actionable `reconcile_with` family hint is returned, such as the canonical `computer_observe` family for `stale_element`. These two shapes are mutually exclusive. This metadata is advisory only; it does not refresh an opaque identity or grant effect retry authority.
...
The model surface uses `computer_observe(action=snapshot_window)`. Calls with only `client_id` + `surface_id` retain the existing `computer_snapshot` Runner wire and `computer_observe` capability for rolling compatibility. Supplying `region`, `max_width`, or `max_height` switches internally to the additive `computer_snapshot_region` Runner wire and requires both the baseline `computer_observe` capability and the additive `computer_snapshot_region` capability, so an older Runner can still serve whole-window snapshots but fails closed for the new transform. Region coordinates remain in the revalidated surface coordinate space; the Runner maps them into captured pixels, crops inside the bounded image, applies aspect-preserving downscale only when requested, and keeps JPEG encoding/quality system-selected.
...
The uncertain artifact-write response exposes `recovery_kind=reconcile` and `suggested_call={"tool":"read_project_artifact_metadata","arguments":{"project":…, "path":…}}` using the exact target. It retains `execution_state=outcome_unknown` and expected digest/byte-count/MIME evidence. No duplicate recovery-tool or family alias accompanies this complete call.

### CU-9 — bounded scroll

The first scroll slice is exposed as `computer_control(action=scroll_to_element, client_id, surface_id, element_id)`, while retaining its own additive `computer_scroll_to_element` Runner capability and wire kind. An older Runner that advertises only the semantic `computer_control` capability must still fail closed rather than being treated as scroll-capable.
...
Expose key input as `computer_control(action=key, client_id, surface_id, key, modifiers?)` while retaining the additive `computer_key_input` Runner capability and wire kind. An older Runner that only advertises the semantic `computer_control` capability still fails closed. The model-facing vocabulary is deliberately limited to Enter, Escape, Tab, arrows, PageUp/PageDown, Home/End plus the bounded `shift`, `control`, `option`, and `command` modifier set. Ordinary text continues through `computer_control(action=input_text)`; callers cannot supply characters, virtual keycodes, repeat counts, held-key state, or modifier key-down/up sequences.
...
The first Windows parity slice extends the corresponding `computer_observe` actions (`accessibility_status`, `accessibility_tree`, `find_elements`, and `element_state`) to Windows UI Automation while retaining the precise Runner operations. It does not add `windows_*` tools or grant any Windows Computer effect capability.
...
`computer_observe(action=find_elements)` remains a Control-side adapter over the canonical bounded tree, so Windows support adds no finder-specific Runner protocol. `computer_observe(action=element_state)` re-resolves the exact UIA lineage before returning normalized state. W1 deliberately reports all mutation `can_*` affordances false because the corresponding Windows effect capabilities remain independently unavailable until later parity slices.

### Windows UIA parity W2 — exact window activation

Windows uses `computer_control(action=activate_window)` while retaining the precise `computer_activate_window` Runner effect and independent `computer_window_activate` capability. The caller supplies only an exact previously observed `surface_id`; there is no app-name, PID, title, executable-path, command, or fallback target.
...
```text
Windows UIA parity
-> application discovery / bounded launch (implemented on Windows and macOS)
-> full-display discovery and snapshot
-> coordinate pointer
-> clipboard (Windows production accepted; macOS native parity implemented)
```

Windows parity should map UI Automation patterns into the same CodeGPT semantic action/affordance vocabulary instead of exposing a second OS-specific model API.
