...
This checklist is for final release readiness before tagging, publishing artifacts, updating client schemas, or deploying a new CodeGPT server/Runner/runtime build.
...
- MCP and GPT Actions both say they call the same CodeGPT ToolRuntime.
...
```bash
rg "run_codex|Codex delegation|retained runner|future explicit opt-in|CODEGPT_ENABLE_LEGACY_CODEX_RUN|PROJECTS_CONFIG|server_static|/api/codex|api/codex|projects.toml" README.md README.zh-CN.md docs deploy scripts SECURITY.md
```
...
After exact-main CI proof succeeds, the release-readiness workflow builds dedicated debug `codegpt-server` / `codegpt-runner` fixtures once for both supported zero-config transports while the compare eval runs independently. Disposable Server-image validation waits for both release-specific gates. The debug fixtures and images are never uploaded or promoted; `.github/workflows/release-build.yml` remains the only native release-candidate producer after the immutable tag exists.
...
The exact-source release-readiness workflow runs `EVAL_MODE=compare bash scripts/eval_coding_loop.sh`. Run it separately only for focused diagnosis. The eval harness measures scripted CodeGPT tool-call mechanics; it is not a full model-behavior evaluation.
...
- `Cargo.toml`, every local CodeGPT workspace entry in `Cargo.lock`, `npm/codegpt/package.json`, `apps/desktop/package.json`, `apps/desktop/src-tauri/Cargo.toml`, `apps/desktop/src-tauri/tauri.conf.json`, `manifest.example.json`, and the relevant package self-tests must agree on `<VERSION>` before tagging.
- `npm/codegpt/manifest.json` is generated release metadata and is intentionally not tracked. Do not commit real checksums or create a post-tag checksum-only PR.
...
- Pin one `CODEGPT_BUILT_AT` value for the release. Every final `codegpt`, `codegpt-server`, and `codegpt-runner` binary must report `<VERSION>`, the same concrete tag commit, `dirty=false`, and the shared `built_at`.
...
- Server container publication is owned by `.github/workflows/release-image.yml`, separate from the read-only `release-build.yml` candidate workflow. It runs only after a GitHub Release is public (or by guarded manual backfill of that already-public immutable tag), builds native `linux/amd64` and `linux/arm64` images from the exact annotated tag with the release build identity, verifies the non-root runtime, exact Server/CLI identity, health check, and absence of the Runner, then publishes immutable `v<VERSION>` / `<VERSION>` image tags (with SemVer `+` build metadata encoded as `_`, since Docker tags do not allow `+`). From the final immutable multi-arch digest it also generates one self-contained `codegpt-server-bootstrap.sh` from the reviewed publication-workflow source; that script embeds and materializes a Compose definition pinned to the exact image digest, which keeps guarded backfill usable for Releases whose application tags predate the deployment tooling. `codegpt-server-image.json` records the application source SHA, deployment-tooling source SHA, manifest/child digests, and bootstrap SHA-256. Existing Release assets are reconciled byte-for-byte on rerun; only the current latest stable GitHub Release may move the mutable `latest` tag.
- GitHub creates the first GHCR package as private by default. On the first CodeGPT Server image publication only, the repository owner must change `codegpt-server` package visibility to **Public** in GitHub Packages, then rerun the failed anonymous-availability gate. After activation, require anonymous inspection/pull availability for every release; do not distribute registry credentials to end users.
...
8. Require `.github/workflows/release-image.yml` to finish successfully for the same public `v<VERSION>` Release. It must publish/reconcile `ghcr.io/yyjeqhc/codegpt-server:v<VERSION>` for both Linux architectures, attach the digest metadata plus the self-contained clone-free bootstrap, verify its public SHA-256 and embedded pinned-digest contract, and pass its anonymous GHCR availability gate. For the first-ever package only, make the package Public once and rerun that failed gate; do not rebuild or retag the immutable image merely to change visibility.
