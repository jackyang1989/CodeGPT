...
Native Tool Plugins let a CodeGPT Runner expose tools implemented by an
...
Native Plugins are **trusted local executables**. CodeGPT does not sandbox,
...
process PATH. Sensitive CodeGPT process credentials are filtered from that
environment.
...
Server-global CodeGPT tool namespace and are never appended to outer MCP
`tools/list`. A Plugin may define `safe_delete`, `runtime_status`, or any other
valid provider-local name without colliding with CodeGPT built-ins or Plugin
tools on another Runner/provider.

The stable model-facing entry is the first-class CodeGPT tool `plugin_tool`.
Its ToolSpec is static and registered in the same canonical tool metadata path
as other CodeGPT tools; its schema does not depend on Runner availability or
...
and a bounded CodeGPT-generated `detail`. Tool-definition validation failures
also include a small `diagnostic` with a finite CodeGPT-defined code and, when
safe, a validated tool name and finite field label such as `inputSchema`.
Diagnostics come only from CodeGPT protocol parsing/validators; raw serde
...
`codegpt plugin` now has two deliberately separate paths. `plugin init` is a
...
codegpt plugin list
    -> {"action":"list"}
codegpt plugin list --runner special
...
```text
codegpt plugin init ./my-plugin
codegpt plugin init ./MyPlugin --id my-plugin
```
...
`@yyjeqhc/codegpt-plugin-sdk` **exactly** to `0.1.0`; it does not use a `file:`,
...
`codegpt runner status --profile <profile>`; the Server never discovers or returns
...
codegpt plugin init ./my-plugin
    ...
    -> codegpt plugin check --runner special --plugin my-plugin --token-file /path/to/plugin-authoring-pat
    -> codegpt plugin reload --runner special --token-file /path/to/plugin-authoring-pat
    -> codegpt plugin list --runner special --plugin my-plugin --token-file /path/to/plugin-authoring-pat
    -> codegpt plugin describe --runner special --plugin my-plugin --tool echo --token-file /path/to/plugin-authoring-pat
```
...
`--env-file` `CODEGPT_TOKEN`, then `CODEGPT_PAT`; process `CODEGPT_TOKEN`, then
`CODEGPT_PAT`. `CODEGPT_PAT` is only an additive user/API CLI input alias; it does
not redefine the Server bootstrap meaning of `CODEGPT_TOKEN`, and when both names
exist the legacy `CODEGPT_TOKEN` input still wins. If a separate authoring PAT is
needed, use the existing `codegpt tokens create-local` / Server token-management
...
`codegpt plugin init`, ensure `npm run build` has produced `dist/plugin.js` on the
...
produce `plugin_eof`, and CodeGPT does not infer the cause from raw stderr or opaque
argv.
...
There is deliberately no `codegpt plugin call` in this authoring phase. The raw
...
Phase 1 intentionally deferred `codegpt plugin init` until the SDK had a truthful
external dependency contract. That prerequisite is now satisfied:
`@yyjeqhc/codegpt-plugin-sdk@0.1.0` is publicly distributed through npm, and Phase 3
adds the local scaffold using that exact compatibility pin. A generated project
therefore works independently of a CodeGPT source checkout. Repository first-party
...
`@yyjeqhc/codegpt-plugin-sdk` is an optional TypeScript authoring layer for Native
...
## CodeGPT Plugin Protocol v1

The native protocol is newline-delimited JSON-RPC 2.0 framing with the protocol
version `codegpt-plugin-v1`. Each request or response is exactly one JSON line.
It is a CodeGPT Plugin protocol, not MCP.

Initialization:

```json
{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":"codegpt-plugin-v1"}}
```
...
`inputSchema` and `outputSchema` use a deliberately small CodeGPT profile; they
...
system Trash/Recycle Bin without adding permanent deletion to CodeGPT's built-in
...
Before `tools/call` can enter the provider connection, CodeGPT resolves the
...
fails `NotStarted` and must be described again. CodeGPT never re-resolves the
...
`OutcomeUnknown`; CodeGPT does not automatically retry or replay it. Failures
...
- For the shared-key OAuth bridge, opt in explicitly with
  `codegpt connect ... --auth oauth --oauth-local-plugins`; that opt-in grants
...
consequential CodeGPT execution when an explicit `recording_session_id` is
supplied; CodeGPT never infers that Session from MCP transport identity.
