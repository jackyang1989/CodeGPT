...
The `codegpt` command is the unified operator and developer interface. It
...
- `codegpt` — the unified command documented here.
- `codegpt-server` — the Server process (started and supervised with
  `codegpt server ...`).
- `codegpt-runner` — the Runner process that executes project work (started
  and supervised with `codegpt runner ...`).

`codegpt --help` lists the top-level namespaces. The sections below explain what each namespace is for. This is the complete CLI reference; ordinary users do not need to understand every command, credential, or internal configuration field before their first successful setup.

For everyday development, follow the [Full Setup guide](PERSONAL_SETUP.md) and use a regular Server + Runner. `codegpt share` is the explicit **temporary one-project trial/share** entry on Linux, macOS, and Windows; it prepares the temporary project environment, Server, Runner, and optional Tunnel for that foreground run and ends when the process exits. Running bare `codegpt` in an interactive Git checkout remains a Linux/macOS convenience shortcut into that same temporary `share` flow.
...
| --- | --- | --- |
| `codegpt` (no command) | Interactive temporary-share shortcut | Only auto-dispatches to `share` on Linux/macOS when stdin/stdout are terminals and the current directory is inside a Git checkout; otherwise normal help is shown. |
| `codegpt share` | Temporarily share the current project with ChatGPT/MCP | Quick trial/short-lived sharing path on Linux/macOS/Windows; includes temporary setup, local Server + Runner, `cloudflare|openai|none`, and bounded foreground cleanup. Use `PERSONAL_SETUP` for full daily use. |
| `codegpt connect <server>` | Connect the current project to an existing Server | Long-lived path when you already have a Server URL; defaults to hosted shared-key. |
| `codegpt status` | Concise project coding readiness | Short summary; `doctor` is the full diagnostic check. |
| `codegpt doctor` | Read-only readiness checks for the current project | Diagnostics/manual workflow; reports a stable `next action`. |
| `codegpt setup` | Configure the current Git project without starting it | Local-only/manual workflow; creates private state and a Project Credential. |
| `codegpt run` | Start the project-bound loopback Server and local Runner | Local-only/manual workflow; foreground, Ctrl-C stops both. |
| `codegpt disconnect [--project PATH] [--profile NAME]` | Remove one hosted project registration | Exact inverse of `connect` for that repository; never removes the repository or `.git`. |

`codegpt share --auth query-token` is an explicit temporary-share compatibility mode for MCP clients that cannot configure a Bearer header. It accepts the exact share Project Credential only on `/mcp?token=...`, prints a URL-encoded sensitive MCP URL, and tells the client to use No authentication. The mode is disabled for ordinary Server/runtime requests, does not accept PAT/OAuth/shared-key/Runner credentials through the query, and is rejected with `--tunnel openai`. Treat the complete URL as a credential because URL queries may be retained by clients, proxies, clipboards, or access logs. The default remains `--auth bearer`.

`codegpt share --auth oauth --oauth-redirect-uri <exact-callback>` uses OAuth 2.0 Authorization Code with PKCE S256. The OAuth client ID/secret are persisted in protected project state for that project + callback, while the temporary OAuth grants are valid only for the current `share` run. Restarting `share` therefore invalidates old OAuth grants without changing the project. OAuth access tokens are never accepted on Runner transport.

Quick Tunnel origins remain temporary. For an operator-managed stable HTTPS origin, use `--tunnel none --public-url https://share.example` and route that origin to the loopback CodeGPT Server yourself; `--public-url` advertises the external origin/issuer and does not create a proxy or tunnel.

`codegpt share --tunnel openai` is the explicit OpenAI Secure MCP Tunnel provider. It requires `CONTROL_PLANE_TUNNEL_ID` plus a Restricted `CONTROL_PLANE_API_KEY` with Tunnels Read + Use and currently supports only `--auth bearer`. CodeGPT resolves pinned OpenAI `tunnel-client` v0.0.12 from `CODEGPT_TUNNEL_CLIENT_BIN`, `PATH`, or a verified managed download; it runs `doctor` before the daemon and waits for `/readyz`. The temporary CodeGPT Bearer is written only to the private share directory and referenced by `tunnel-client` through a file-backed MCP `Authorization` header. ChatGPT therefore uses Connection: Tunnel + No authentication. `OPENAI_ADMIN_KEY` and `OPENAI_API_KEY` are explicitly removed from the long-lived daemon environment; the Runtime API key remains the control-plane authority.

For public `share`, CodeGPT best-effort copies only the MCP URL to the clipboard and does not copy the temporary credential in the default Bearer/OAuth modes. The explicit `--auth query-token` mode instead copies the sensitive tokenized URL by design and says so in its status output. Interactive Linux/macOS terminals also offer an Enter shortcut to open ChatGPT App settings. Clipboard/browser integration is convenience-only and never gates runtime readiness. Use `--no-copy-url` to suppress clipboard access.

For supervised machine integration, `codegpt share --json --stop-on-stdin-eof` keeps the same foreground lifecycle but also treats the supervising parent's closed stdin as a stop request. This lets Desktop or another structured process owner ask `share` to clean up its own temporary Server, Runner, and Tunnel without shell-command signaling. The flag is rejected outside `--json` mode.

`codegpt connect <server> --auth oauth --oauth-redirect-uri <exact-callback>` is the ordinary hosted OAuth path. The Runner keeps its existing hosted credential while the MCP client uses OAuth. Add `--oauth-computer-permissions`, `--oauth-local-mcp`, or `--oauth-local-ssh` only when those optional capabilities are actually needed; they are explicit permission changes and can require reauthorization. `--oauth-local-ssh` grants the MCP client the optional `ssh:local` authority needed by the model-facing `ssh_resource` onboarding tool; it does not expose SSH credentials or make a registered resource active without the Runner restart reported by that tool. See [MCP](MCP.md#oauth2) for client setup and [Authentication](AUTH_MODEL.md#oauth2) for the security model.

The advanced managed identity flow remains available as `--auth managed-oauth --oauth-redirect-uri <exact-callback>` and requires `codegpt login`; `--user` applies only to that mode.
...
The Runner executable is `codegpt-runner`. Its canonical CLI lifecycle namespace
is `runner`: `codegpt runner ...` manages the `codegpt-runner` process and
service. `codegpt` and `codegpt-runner` remain separate executables.

| Command | Purpose |
| --- | --- |
| `codegpt runner init` | Generate a `runner.toml` config manually |
| `codegpt runner install` | Install, enable, and start the Runner service |
| `codegpt runner run` | Run `codegpt-runner` in the foreground |
| `codegpt runner start` | Start a hosted background Runner or installed profile service |
| `codegpt runner stop` | Stop it |
| `codegpt runner restart` | Restart it |
| `codegpt runner status` | Check Runner lifecycle, config, and connectivity |
| `codegpt runner logs` | Read Runner logs (bounded) |
| `codegpt runner uninstall` | Remove the service unit (requires `--confirm`) |

Service commands accept `--scope user|system`. Non-root users default to user
scope; root defaults to system scope. Profiles created by `codegpt connect`
...
| Command | Purpose |
| --- | --- |
| `codegpt server init` | Initialize/update the Server env file and selected data directory (creates the bootstrap token) |
| `codegpt server install` | Install the Linux systemd `codegpt.socket` + `codegpt.service` pair; default WorkingDirectory follows selected env `CODEGPT_DATA` |
| `codegpt server run [--env-file PATH]` | Run `codegpt-server` in the foreground (direct bind); `--env-file` passes the exact path via `CODEGPT_ENV_FILE` |
| `codegpt server start` / `stop` | Start or stop socket activation and the Server process coherently |
| `codegpt server restart` | Restart only the Server process; keep the managed listener socket active |
| `codegpt server status` | Check authoritative socket/service state, HTTP reachability, and build revisions |
| `codegpt server logs` | Read the Server service journal |
| `codegpt server uninstall` | Stop, disable, and remove the managed socket/service pair |
...
`codegpt server install --service-file /path/name.service` derives the sibling
...
it targets the default `codegpt.service` / `codegpt.socket` pair.
...
| Command | Purpose |
| --- | --- |
| `codegpt ops status` | Summarize runtime, tools, Jobs, Runners, and Projects |
| `codegpt ops runners` | Compact Runner fleet status |
| `codegpt ops runner --client-id <id>` | Exact read-only Runner registration/build status |
| `codegpt ops projects` | Project inventory and smoke suitability |
| `codegpt ops smoke-preflight --project <id>` | Preflight one project for a deploy smoke |
...
The legacy `codegpt task` namespace has been removed with the separate Connector Task/Result/Approval lifecycle. Local `codegpt run` prints the Runtime Console URL (`/runtime`). Runtime review uses the canonical Workflow Session, Job, Git/diff, `show_changes`, and `finish_coding_task` paths rather than a host-side result accept/reject queue.
...
| --- | --- | --- |
| `codegpt pairing create` | Server/admin side: create a short-lived pairing code | Needs server bootstrap/admin auth. |
| `codegpt tokens generate` | Offline token material generation | Registers nothing; pair the output with `tokens register-hash` if the hash must be registered server-side. |
| `codegpt tokens register-hash` | Admin: register an externally computed PAT hash | Uses `--server-url`; for offline-generated material. |
| `codegpt runner-tokens register-hash` | Admin: register an externally computed Runner-token hash | Uses `--server-url`; for offline-generated material. |
...
CodeGPT separates bootstrap administration, account onboarding, runtime API
...
- Normal managed setup: `codegpt login` creates the local user/API and Runner credentials; use the paths and MCP values it reports.
...
codegpt setup
codegpt doctor
codegpt run          # keep this terminal open; output points to /runtime
codegpt status       # in another terminal
```

Existing hosted Server:

```bash
codegpt connect https://your-server.example
codegpt runner status --profile <profile>
codegpt runner logs --profile <profile> --lines 100
```
...
codegpt ops status --server-url https://your-server.example \
...
flags affect only the CLI's own HTTP requests; `codegpt connect` does not
