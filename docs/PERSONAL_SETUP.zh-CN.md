...
这条路径适合 CLI、已有 Server，或需要比 Desktop 更多控制的配置。普通 Windows / macOS 个人安装最推荐先看 [CodeGPT Desktop + 官方 OpenAI Secure Tunnel](desktop-install.zh-CN.md)。无论哪种入口，完整 CodeGPT 的模型都一样：ChatGPT 连接普通 Server，再由长期运行的 Runner 使用你机器上的真实项目、Git、编译器和测试工具。

如果你只想先试几分钟，不想配置长期服务，请直接看[快速试用](QUICK_START.zh-CN.md)并使用 `codegpt share`。`share` 是临时、单项目、Project-scoped 的普通 Adaptive Runtime；关闭命令后连接就会结束。
...
Tunnel 只解决网络可达性。不要为了使用 Tunnel 改成 `codegpt share`；完整使用路径仍然是上一步启动的普通 Server。
...
Runner 运行后，CodeGPT 才真正拥有调用本机文件、Git、编译器和测试工具的执行入口。

## 7. 把 CodeGPT 加到 ChatGPT
...
如果你选择 OpenAI Secure MCP Tunnel，登录时不需要 `--print-mcp-config` 给 ChatGPT 使用；ChatGPT 侧通常选择 Tunnel + No authentication，由本机 tunnel client 注入 CodeGPT 凭据。按对应 Tunnel 指南操作，不要把本地 Bearer 复制给 ChatGPT。
...
| 场景 | 推荐入口 | 特点 |
| --- | --- | --- |
| 我只想几分钟体验一下一个仓库 | `codegpt share` | 一条命令、临时、单项目、普通 Adaptive Runtime、关闭即失效 |
...
更详细的错误处理见[故障排查](TROUBLESHOOTING.zh-CN.md)。命令和配置字段的完整参考见 [CLI](CLI.zh-CN.md)，但第一次成功使用 CodeGPT 不需要先理解那些内部术语。
