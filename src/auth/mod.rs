//! CodeGPT authentication and authorization.
//!
...
pub(crate) use project_credential::{
    ...
    ProjectAuthState, PROJECT_AGENT_TOKEN_FILE_ENV, PROJECT_CREDENTIAL_FILE_ENV,
    PROJECT_GRANT_ID_ENV, PROJECT_RUNNER_CLIENT_ID_ENV,
};
...
impl AuthEnvGuard {
    pub(crate) fn new() -> Self {
        ...
        Self {
            _env_lock: env_lock,
            shared_key_enabled: std::env::var_os("CODEGPT_SHARED_KEY_ENABLED"),
            allow_anonymous: std::env::var_os("CODEGPT_ALLOW_ANONYMOUS"),
            oauth2_shared_key_bridge: std::env::var_os("CODEGPT_OAUTH2_SHARED_KEY_BRIDGE"),
        }
    }
    ...
    pub(crate) fn enable_direct_shared_key(&self) {
        std::env::set_var("CODEGPT_SHARED_KEY_ENABLED", "true");
    }

    pub(crate) fn disable_direct_shared_key(&self) {
        std::env::remove_var("CODEGPT_SHARED_KEY_ENABLED");
    }

    pub(crate) fn enable_open_anonymous(&self) {
        std::env::set_var("CODEGPT_ALLOW_ANONYMOUS", "true");
    }

    pub(crate) fn disable_open_anonymous(&self) {
        std::env::remove_var("CODEGPT_ALLOW_ANONYMOUS");
    }

    pub(crate) fn enable_oauth2_shared_key_bridge(&self) {
        std::env::set_var("CODEGPT_OAUTH2_SHARED_KEY_BRIDGE", "true");
    }

    pub(crate) fn disable_oauth2_shared_key_bridge(&self) {
        std::env::remove_var("CODEGPT_OAUTH2_SHARED_KEY_BRIDGE");
    }
}
...
impl Drop for AuthEnvGuard {
    fn drop(&mut self) {
        restore_test_env("CODEGPT_SHARED_KEY_ENABLED", &self.shared_key_enabled);
        restore_test_env("CODEGPT_ALLOW_ANONYMOUS", &self.allow_anonymous);
        restore_test_env(
...
pub(crate) async fn authenticate_bearer(
    ...
    match authenticate(config, db, token).await {
        ...
        Ok(None) => {
            ...
            // trimming, and it does not look like a CodeGPT managed credential.
...
/// server-wide `CODEGPT_TOKEN` is presented. Kept private to `auth`; the only
