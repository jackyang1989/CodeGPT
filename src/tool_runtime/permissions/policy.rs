...
pub(crate) enum AuthoritySource {
    ...
    Default,
    /// Explicit `CODEGPT_AUTHORITY_MODE`.
    Env,
...
impl AuthoritySource {
    pub(crate) fn as_str(self) -> &'static str {
        match self {
            ...
            Self::Env => concat!("env:", "CODEGPT_AUTHORITY_MODE"),
            Self::LegacyEnv => "migrated_env:CODEGPT_PERMISSION_MODE",
            Self::LegacyEnvRejected => concat!("rejected_legacy_env:", "CODEGPT_PERMISSION_MODE"),
        }
    }
}
...
impl EffectiveAuthorityConfig {
    ...
    ///
    /// Unset or empty `CODEGPT_AUTHORITY_MODE` → [`AuthorityMode::TrustedAgent`]
...
mod tests {
    ...
    fn source_is_reported_for_default_and_env() {
        ...
        assert_eq!(AuthoritySource::Env.as_str(), "env:CODEGPT_AUTHORITY_MODE");
        assert_eq!(
            AuthoritySource::LegacyEnvRejected.as_str(),
            "rejected_legacy_env:CODEGPT_PERMISSION_MODE"
        );
    }
}
