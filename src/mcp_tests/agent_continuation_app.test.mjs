...
const assertAppCallId = call => assert.match(appCallId(call), /^cg_app_call_[0-9a-f]{16}_[1-9][0-9]{0,5}$/);
const wake = {
  wake_id: `cg_wake_RERERERERERERERE`, attempt_id: `cg_wake_attempt_VVVVVVVVVVVVVVVV`,
  state: "claimed", revision: 2, dispatch_observation: null,
  wait_id: null, wait_match_count: null, wait_match_sequence: null,
};
const projection = {
  version: 1, agent_id: `cg_dagent_ERERERERERERERER`, endpoint_id: `cg_endpoint_IiIiIiIiIiIiIiIi`,
...
const waitId = `cg_agent_wait_ZmZmZmZmZmZmZmZm`;
const waitTaskA = `cg_agent_task_d3d3d3d3d3d3d3d3`;
const waitTaskB = `cg_agent_task_iIiIiIiIiIiIiIiI`;
const waitingWait = {
...
test("Agent Wait card tracks waiting -> triggered -> resuming -> resumed without a second dispatcher", async () => {
  ...
  const triggeredWait = {
    ...
    matches: [{
      ...
      task_attempt_id: `cg_agent_task_attempt_mZmZmZmZmZmZmZmZ`,
...
for (const outcome of ["success", "error", "timeout"]) {
  for (const early of [true, false]) {
    test(`continuation input ${early ? "before" : "after"} initialize ${outcome}, without initial result`, async () => {
      ...
      if (outcome === "success") {
        assert.match(bindingId(view), /^cg_host_binding_[A-Za-z0-9_-]{21}[AQgw]$/);
...
const conflicts = {
  agent_id: `cg_dagent_qqqqqqqqqqqqqqqq`,
...
for (const invalid of [
  ...
  { ...input, agent_id: `cg_dagent_${"A".repeat(32)}` },
...
const replacementOutput = (from = projection) => {
  const successor = {
    ...from, endpoint_id: `cg_endpoint_MzMzMzMzMzMzMzMz`,
...
test("input-only dispatch never displays its private binding or consume envelope", async () => {
  ...
  await view.reply(view.calls("agent_continuation_state")[0], toolResult({ agent_continuation: projection }));
  await view.reply(view.calls("agent_continuation_wake_acquire")[0], toolResult({ wake }));
  const consume_token = `cg_wake_consume_zMzMzMzMzMzMzMzMzMzMzA`;
...
test("finish failure keeps the old claim until its ACK is reconciled before acquiring a successor", async () => {
  ...
  await view.fireTimers(3000);
  await view.reply(view.calls("agent_continuation_wake_finish").at(-1), rejected);
  const successor = { ...projection, wake: { ...projection.wake, wake_id: `cg_wake_ZmZmZmZmZmZmZmZm` } };
...
test("all App coordination survives stripped ToolResult metadata through successor Wakes", async () => {
  ...
  for (let round = 0; round < 3; round++) {
    const currentWake = { ...wake, wake_id: `cg_wake_${String(round + 6).repeat(16)}`, attempt_id: `cg_wake_attempt_${String(round + 6).repeat(16)}` };
...
for (const loss of ["timeout", "Host error", "malformed result"]) {
  test(`same View retries bind with the same secure fence after ${loss}`, async () => {
    ...
    assert.match(serverBinding, /^cg_host_binding_[A-Za-z0-9_-]{21}[AQgw]$/);
...
for (const loss of ["timeout", "missing", "wrong type", "blank", "oversized", "wrong Attempt", "business failure"]) {
  test(`prepare ${loss} reconciles delivery_unknown without a second prepare or Host turn`, async () => {
    ...
    if (loss === "timeout") await view.fireTimers(10000);
      ...
      if (loss === "wrong Attempt") output.attempt_id = `cg_wake_attempt_7u7u7u7u7u7u7u7u`;
...
test("Agent Wait sparse reference still requires the exact durable target on App refresh", async () => {
  ...
  await view.reply(view.calls("agent_wait_state").at(-1), toolResult({ agent_wait: {
    ...waitingWait, target_agent_id: `cg_dagent_qqqqqqqqqqqqqqqq`,
  } }));
  assert.equal(view.nodes.binding.textContent, "Unavailable");
