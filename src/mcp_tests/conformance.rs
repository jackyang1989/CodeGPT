...
const CONFORMANCE_URL_FILE_ENV: &str = "CODEGPT_MCP_CONFORMANCE_URL_FILE";
const CONFORMANCE_STOP_FILE_ENV: &str = "CODEGPT_MCP_CONFORMANCE_STOP_FILE";
...
async fn mcp_conformance_fixture_server() {
    let url_file = PathBuf::from(
        std::env::var_os(CONFORMANCE_URL_FILE_ENV)
            .expect("conformance fixture requires CODEGPT_MCP_CONFORMANCE_URL_FILE"),
    );
    let stop_file = PathBuf::from(
        std::env::var_os(CONFORMANCE_STOP_FILE_ENV)
            .expect("conformance fixture requires CODEGPT_MCP_CONFORMANCE_STOP_FILE"),
    );
