...
pub(crate) const CONSOLE_ASSETS_DIR_ENV: &str = "CODEGPT_CONSOLE_ASSETS_DIR";

const ASSET_MODE_HEADER: HeaderName = HeaderName::from_static("x-codegpt-console-assets");
...
fn validate_loopback_addr(bind_addr: &str) -> Result<(), ConsoleAssetConfigError> {
    ...
    if is_loopback {
        ...
        Err(ConsoleAssetConfigError::new(format!(
            "{CONSOLE_ASSETS_DIR_ENV} requires CODEGPT_ADDR to use a loopback address"
        )))
    }
}
...
mod tests {
    ...
    fn embedded_bundle_contains_runtime_and_admin_only() {
        assert!(RUNTIME_HTML.contains("CodeGPT Runtime Console"));
