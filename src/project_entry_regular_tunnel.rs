...
pub(crate) async fn run_regular_server_tunnel(
    ...
    let encoded = serde_json::to_string(&ready).map_err(|_| {
        ProductError::new(
            ...
            "CodeGPT could not encode regular Tunnel readiness",
...
fn validate_local_server_url(value: &str) -> Result<String, ProductError> {
    ...
    let parsed = url::Url::parse(value).map_err(|_| {
        ProductError::new(
            ...
            "Regular OpenAI Tunnel requires a valid local CodeGPT Server URL",
            Some("Start the local CodeGPT runtime before starting the Tunnel."),
        )
    })?;
        ...
        || !matches!(parsed.path(), "" | "/")
    {
        return Err(ProductError::new(
            "unsupported_topology",
            "Regular OpenAI Tunnel only exposes a loopback local CodeGPT Server",
...
async fn wait_for_stdin_eof() {
    ...
    let _ = std::thread::Builder::new()
        .name("codegpt-server-tunnel-stdin".to_string())
        .spawn(move || {
