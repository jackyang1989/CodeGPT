...
pub(crate) fn parse_share_options(args: &[String]) -> Result<ShareCommandOptions, String> {
    ...
    if tunnel == TunnelProvider::OpenAiSecure && auth != ShareAuth::Bearer {
        return Err("--tunnel openai currently requires --auth bearer; CodeGPT keeps that temporary Bearer credential local and tunnel-client injects it into the private MCP hop".to_string());
    }
    match auth {
...
fn share_oauth_state_error(message: impl Into<String>) -> ProductError {
    ProductError::new(
        ...
        message,
        Some("Resolve the protected share OAuth state, then retry codegpt share --auth oauth."),
    )
}
...
fn revoke_previous_share_oauth_grants(
    ...
    db.revoke_oauth_access_tokens_for_client(client_id, now)
        ...
        .map_err(|_| {
            share_oauth_state_error("CodeGPT could not retire previous share OAuth grants")
        })?;
    Ok(())
}
...
fn prepare_share_oauth_client(
    ...
    let db = crate::Database::open(&paths.data.join("codegpt.db"))
        .map_err(|_| share_oauth_state_error("CodeGPT could not open project OAuth state"))?;
    ...
    if present.iter().all(|present| *present) {
        ...
        let existing = db
            .list_oauth_clients()
            .map_err(|_| share_oauth_state_error("CodeGPT could not read project OAuth clients"))?
            .into_iter()
        ...
        if let Some(client) = existing {
            ...
            .map_err(|_| {
                share_oauth_state_error("CodeGPT could not restore the share OAuth client")
            })?;
        }
    ...
    .map_err(|_| share_oauth_state_error("CodeGPT could not create the share OAuth client"))?;
...
impl CloudflareTunnel {
    async fn wait_for_exit(&mut self) -> Result<(), ProductError> {
        ...
        Err(ProductError::new(
            ...
            Some("Check network connectivity and cloudflared, then retry codegpt share."),
        ))
    }
...
fn tunnel_runtime_error() -> ProductError {
    ProductError::new(
        ...
        Some("Check cloudflared installation and retry codegpt share."),
    )
}
...
pub(crate) async fn share(options: &ShareCommandOptions) -> Result<(), ProductError> {
    ...
    ensure_local_runtime_port_available(
        config.port,
        "Stop the conflicting process, then retry codegpt share.",
    )?;
    ...
    let mut runtime = match start_local_runtime(
        ...
        LocalRuntimeOptions {
            ...
            port_conflict_action: "Stop the conflicting process, then retry codegpt share.",
    ...
    let handoff_task = if options.project.json {
        ...
        let encoded = serde_json::to_string(&event).map_err(|_| {
            ProductError::new(
                ...
                "CodeGPT could not encode Quick Share readiness",
...
async fn wait_for_share_stdin_eof() {
    ...
    let _ = std::thread::Builder::new()
        .name("codegpt-share-stdin".to_string())
        .spawn(move || {
...
async fn start_cloudflare_quick_with_binary(
    ...
    let mut child = tunnel_command.spawn().map_err(|_| {
        ProductError::new(
            ...
            Some("Check the cloudflared executable and retry codegpt share."),
        )
    })?;
...
mod tests {
    ...
    fn share_output_contains_only_the_temporary_project_credential() {
        let persistent = "codegpt_persistent-never-print";
        let temporary = "codegpt_temporary-print-once";
    ...
    #[test]
    fn openai_share_output_keeps_codegpt_credential_local() {
