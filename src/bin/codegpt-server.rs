use codegpt::{server_binary_action, ServerBinaryAction};
...
// callers to set RUST_MIN_STACK for `codegpt share`.
#[cfg(target_os = "macos")]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    match server_binary_action(std::env::args().skip(1)) {
        ServerBinaryAction::Run { stop_on_stdin_eof } => {
            codegpt::prepare_server_process_environment().map_err(std::io::Error::other)?;
            build_server_runtime()?
                .block_on(codegpt::run_server_with_parent_liveness(stop_on_stdin_eof))
        }
...
mod tests {
    ...
    fn server_runtime_uses_large_worker_stack() {
        let runtime = build_server_runtime().expect("build CodeGPT Server runtime");
        let stack_size = runtime.block_on(async {
            ...
            .await
            .expect("observe CodeGPT Server worker")
        });
        assert!(
            stack_size >= MACOS_SERVER_RUNTIME_STACK_SIZE,
            "CodeGPT Server worker stack was {stack_size} bytes; expected at least {MACOS_SERVER_RUNTIME_STACK_SIZE}"
        );
    }
}
