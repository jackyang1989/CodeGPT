...
const RUNTIME_BINARIES = Object.freeze(["codegpt", "codegpt-server", "codegpt-runner"]);
...
function validateBinarySet(dir, expectedVersion = PACKAGE_VERSION, platform = process.platform, options = {}) {
  ...
  if (!identities.every((identity) => identity === identities[0])) {
    throw new Error("Installed CodeGPT binaries are not from the same build");
  }
  return identities[0];
}
...
function replaceDirectoryAtomically(stagedDir, finalDir, onWarning = defaultWarning) {
  ...
  if (hadPrevious) {
    try {
      ...
      onWarning("CodeGPT installation succeeded, but the previous binary backup could not be removed.");
    }
  }
}
  ...
  return installBinarySet((stagedDir) => {
    for (const file of runtimeBinaryFiles(platform)) {
      ...
      if (!fs.existsSync(source)) {
        throw new Error(`CODEGPT_BINARY_DIR is missing ${file}`);
      }
...
function validateManifest(manifest) {
  ...
  if (!Array.isArray(manifest.binaries) ||
    ...
    throw new Error("Release manifest must declare the three CodeGPT runtime binaries in canonical order");
  }
...
async function loadManifest(manifestPathOrUrl, options = {}) {
  const input = manifestPathOrUrl || process.env.CODEGPT_MANIFEST || DEFAULT_MANIFEST;
  const limits = resolveLimits(options);
  if (/^https?:/i.test(input)) {
    const tempDir = makeTempDirectory(options.tempDir || os.tmpdir(), "codegpt-manifest-");
...
async function installFromManifest(manifestPathOrUrl, options = {}) {
  ...
  if (!artifact) {
    throw new Error(`No CodeGPT release artifact is available for ${key}`);
  }
  ...
  const tempDir = makeTempDirectory(options.tempDir || os.tmpdir(), "codegpt-artifact-");
...
async function main() {
  if (process.env.CODEGPT_SKIP_DOWNLOAD === "1") {
    console.log("CODEGPT_SKIP_DOWNLOAD=1 set; skipping native binary download.");
    return;
  }
  if (process.env.CODEGPT_BINARY_DIR) {
    copyLocalBinaryDir(process.env.CODEGPT_BINARY_DIR);
    return;
  }
...
if (require.main === module) {
  main().catch((err) => {
    console.error(`CodeGPT install failed: ${err.message}`);
    process.exit(1);
  });
}
