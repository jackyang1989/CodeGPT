...
function nativePath(options = {}) {
  ...
  return pathApi.join(root, "vendor", "bin", exeName("codegpt", platform));
}
...
function bootstrapNative(options = {}) {
  ...
  if (!fs.existsSync(installScript)) {
    console.error("CodeGPT installation is incomplete: install.js is missing. Reinstall the npm package.");
    return false;
  }
  ...
  if (result.error) {
    console.error(`Failed to bootstrap the native CodeGPT binaries: ${result.error.message}`);
    return false;
  }
  if (result.signal) {
    console.error(`CodeGPT native bootstrap was terminated by ${result.signal}.`);
    return false;
  }
...
function runNative(options = {}) {
  ...
  if (needsBootstrap) {
    ...
    if (!fs.existsSync(target)) {
      console.error("CodeGPT installation is incomplete: the native codegpt binary is still missing after bootstrap.");
      process.exitCode = 127;
      return null;
    }
  }
  if (!fs.existsSync(target)) {
    console.error("CodeGPT installation is incomplete: the native codegpt binary is missing. Reinstall the npm package.");
    process.exitCode = 127;
    return null;
  }
  ...
  const child = childProcess.spawn(target, argv, {
    env: { ...nativeEnvironment, CODEGPT_NPM_WRAPPER: "1" },
    stdio: "inherit",
    windowsHide: false,
    shell: false
  });
  ...
  child.once("error", (err) => {
    cleanup();
    console.error(`Failed to execute the native codegpt binary: ${err.message}`);
    process.exitCode = 127;
  });
