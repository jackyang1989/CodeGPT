...
function makeBinary(dir, name, identity = defaultIdentity()) {
  ...
  fs.writeFileSync(
    file,
    `#!/bin/sh\nif [ "\${CODEGPT_REQUIRE_WRAPPER_MARKER-}" = "1" ] && [ "\${CODEGPT_NPM_WRAPPER-}" != "1" ]; then exit 24; fi\nif [ "\${1-}" = "--version" ]; then echo "${name} ${identity}"; exit 0; fi\nprintf '%s\\n' "$@"\nexit "\${CODEGPT_TEST_EXIT:-0}"\n`,
    { mode: 0o755 }
  );
  return file;
}
...
function installedIdentity(destination) {
  const file = path.join(destination, EXE("codegpt"));
  if (PLATFORM === "win32") {
...
function assertNoInstallerTemps(tempRoot) {
  const leftovers = fs.readdirSync(tempRoot).filter((name) =>
    name.startsWith("codegpt-manifest-") || name.startsWith("codegpt-artifact-") || name.startsWith(".bin-staging-")
  );
...
async function main() {
  ...
  assert.deepStrictEqual(packageJson.bin, { codegpt: "bin/codegpt.js" });
  assert.deepStrictEqual(install.RUNTIME_BINARIES, ["codegpt", "codegpt-server", "codegpt-runner"]);
  ...
  assert.strictEqual(
    ...
    path.posix.normalize("/tmp/package/vendor/bin/codegpt")
  );
  assert.strictEqual(
    wrapper.nativePath({ packageRoot: "C:\\package", platform: "win32" }),
    path.win32.normalize("C:\\package\\vendor\\bin\\codegpt.exe")
  );

  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "codegpt-npm-test-"));
  try {
    ...
    assert.ok(!fs.existsSync(path.join(destination, "codegpt-cli")));
    ...
    makeBinary(incomplete, EXE("codegpt"));
    makeBinary(incomplete, EXE("codegpt-server"));
    assert.throws(
      () => install.copyLocalBinaryDir(incomplete, testOptions({ destinationDir: oldDestination })),
      /missing codegpt-runner/
    );
    ...
    makeBinarySet(mixed);
    makeBinary(mixed, EXE("codegpt-runner"), `${packageJson.version} other-revision dirty=false`);
    assert.throws(
    ...
    fs.writeFileSync(path.join(archiveSource, "codegpt-cli"), "legacy");
    ...
    makeDeclaredEntryArchive(oversizedEntry, EXE("codegpt"), 2048);
    ...
    if (PLATFORM === "win32") {
      ...
      wrapperArgs = ["-e", "if (process.env.CODEGPT_NPM_WRAPPER !== '1') process.exit(24); console.log(process.argv[1]); console.log(process.argv[2]); process.exitCode = 23;", "alpha", "two words"];
    } else {
    ...
    const probe = childProcess.spawnSync(
      ...
      { encoding: "utf8", env: { ...process.env, CODEGPT_REQUIRE_WRAPPER_MARKER: "1" } }
    );
