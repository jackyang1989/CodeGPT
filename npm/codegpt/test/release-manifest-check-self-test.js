...
function main() {
  ...
  for (const platform of ["darwin-x64", "darwin-arm64", "win32-x64", "win32-arm64"]) {
    assert.strictEqual(
      ...
      `https://github.com/yyjeqhc/codegpt/releases/download/v${packageJson.version}/codegpt-v${packageJson.version}-${platform}.tar.gz`
    );
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "codegpt-manifest-fixture-"));
  try {
