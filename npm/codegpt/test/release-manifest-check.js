...
const EXPECTED_BINARIES = Object.freeze(["codegpt", "codegpt-server", "codegpt-runner"]);
...
function expectedArtifactUrl(version, platform) {
  return `https://github.com/yyjeqhc/codegpt/releases/download/v${version}/codegpt-v${version}-${platform}.tar.gz`;
}
...
function main() {
  ...
  const requested = args.find((arg) => arg !== "--quiet") || process.env.CODEGPT_RELEASE_MANIFEST;
