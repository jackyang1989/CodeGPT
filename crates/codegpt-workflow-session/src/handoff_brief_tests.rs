...
fn handoff_brief_preserves_instruction_paths_commands_and_code_deterministically() {
    let store = store_with_limit(200);
    let root_instruction = "Continue the handoff task. Inspect /root/git/private-drop/src/tool_runtime/handoff_brief.rs and run `cargo test -p codegpt --lib handoff_brief`.";
