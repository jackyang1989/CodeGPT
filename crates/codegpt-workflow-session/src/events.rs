...
use codegpt_core::workflow_session_contract::is_tool_call_expectation_metadata_field as shared_is_tool_call_expectation_metadata_field;
pub use codegpt_core::workflow_session_contract::{
...
pub fn normalize_observed_project_path(path: &str) -> Option<String> {
    ...
    if starts_with_uri_scheme(path)
        || codegpt_core::validation_bridge::validate_project_relative_path(path).is_err()
    {
        return None;
    }
    let normalized = path
    ...
    if normalized.is_empty()
        ...
        || codegpt_core::validation_bridge::validate_project_relative_path(&normalized).is_err()
    {
        return None;
    }
    Some(normalized)
...
pub fn validation_output_summary_for_tool_result(tool_name: &str, output: &Value) -> Option<Value> {
    ...
    let mut summary = json!({
        ...
        "shell": output
            ...
            .unwrap_or_else(|| match execution_policy.shell {
                codegpt_tool_contracts::ToolAuditExecutionShell::Output => Value::Null,
                codegpt_tool_contracts::ToolAuditExecutionShell::DirectArgv => {
                    Value::String("direct_argv".to_string())
                }
                codegpt_tool_contracts::ToolAuditExecutionShell::ScriptLanguage => {
    ...
    if matches!(
        ...
        codegpt_tool_contracts::ToolAuditExecutionDetail::TestCounts
            | codegpt_tool_contracts::ToolAuditExecutionDetail::TestAssertions
    ) {
...
fn sanitized_test_count_assertion(value: Option<&Value>) -> Option<Value> {
    ...
    if !(1..=codegpt_core::runner_protocol::CARGO_TEST_MIN_TESTS_MAX).contains(&minimum_tests) {
        return None;
    }
