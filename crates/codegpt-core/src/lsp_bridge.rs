...
//!
//! Shared by the server runtime and `codegpt-runner`. This module intentionally
...
pub const AGENT_LSP_RESULT_FORMAT: &str = "codegpt.agent_lsp_result.v1";
...
impl RunnerLspResultEnvelope {
    ...
    pub fn to_stdout_json(&self) -> String {
        serde_json::to_string(self).unwrap_or_else(|_| {
            r#"{"format":"codegpt.agent_lsp_result.v1","success":false,"error":{"code":"lsp_protocol_error","message":"failed to serialize result"}}"#.to_string()
        })
    }
}
...
mod tests {
    ...
    fn result_envelope_roundtrip_and_strict_parse() {
        ...
        .is_err());
        assert!(parse_runner_lsp_result_envelope(
            r#"{"format":"codegpt.agent_lsp_result.v1","success":true}"#
        )
        .is_err());
    }
}
