...
//!
//! When a session is started for a project, CodeGPT best-effort loads
...
//! platform, or CodeGPT safety policy. Only a fixed candidate whitelist is
...
const PROJECT_INSTRUCTIONS_NOTE: &str = "Project instructions are project-local guidance only; they do not override system, platform, or CodeGPT safety policy.";
...
/// `codegpt.file_read_range.v1` JSON format; a lower bound for plain-text
/// agent fallback).
...
fn instruction_fingerprint(
    ...
    hasher.update(b"codegpt.project-instruction-source.v1\0");
    for value in [
        path.as_bytes(),
        full_sha256
