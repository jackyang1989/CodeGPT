//! `codegpt` — standalone management/setup binary for CodeGPT.
//!
...
//!
//! This package implements the single public `codegpt` CLI; background runtime
//! execution remains in the separate `codegpt-server` and `codegpt-runner`
//! binaries.
...
use codegpt_admin as admin_cli;
use codegpt_core::build_info;
use codegpt_runner_config as runner_config;
    ...
    S: AsRef<str>,
{
    ...
    match args[0].as_str() {
        ...
        "--version" | "-V" => CliAction::Exit {
            code: 0,
            stdout: build_info::version_output("codegpt"),
            stderr: String::new(),
        },
        "status" | "doctor" | "run" | "share" => CliAction::Project(args),
        "setup" if args.get(1).map(String::as_str) == Some("single-user") => cli_parse_error(
            "`codegpt setup single-user` was removed; use `codegpt pairing create` followed by `codegpt login`"
                .to_string(),
        ),
        ...
        "client" if args.get(1).map(String::as_str) == Some("enroll") => cli_parse_error(
            "`codegpt client enroll` was removed; use `codegpt login <server-url> --code <code>`"
                .to_string(),
        ),
        ...
        "agent-token" => cli_parse_error(
            "`codegpt agent-token` was removed; use `codegpt runner-tokens ...`".to_string(),
        ),
        "runner-tokens" | "agent-tokens" => parse_runner_token_subcommand(&args[1..]),
        "token" => cli_parse_error(
            "`codegpt token` was removed; use `codegpt tokens ...`".to_string(),
        ),
...
fn parse_connect(args: &[String]) -> CliAction {
    ...
    let Some(server_url) = server_url else {
        return cli_parse_error(
            "connect needs a Server URL, e.g. `codegpt connect https://example.com --project .`"
                .to_string(),
        );
    };
...
fn parse_token_generate(args: &[String]) -> Result<TokenGenerateOptions, String> {
    ...
    while let Some(flag) = p.next() {
        match flag.as_str() {
            ...
            "-h" | "--help" => {
                return Err("Usage: codegpt tokens generate --kind api|runner".to_string())
            }
...
fn parse_runner_token_create_local(
    ...
    while let Some(flag) = p.next() {
        match flag.as_str() {
            ...
            "-h" | "--help" => return Err("Usage: codegpt runner-tokens create-local --server-url URL --username USER --credential CRED --client-id ID [--proxy http://HOST:PORT|--no-system-proxy] [--name NAME] [--scopes S1,S2]".to_string()),
...
fn parse_token_create_local(args: &[String]) -> Result<TokenCreateLocalOptions, String> {
    ...
    while let Some(flag) = p.next() {
        match flag.as_str() {
            ...
            "-h" | "--help" => {
                return Err("Usage: codegpt tokens create-local --server-url URL --username USER --credential CRED [--proxy http://HOST:PORT|--no-system-proxy] [--name NAME] [--scopes S1,S2]".to_string())
            }
...
fn parse_runner_subcommand(args: &[String]) -> CliAction {
    ...
    if command == "run" && args.len() == 2 && matches!(args[1].as_str(), "--version" | "-V") {
        return exit_help(&build_info::version_output("codegpt-runner"));
    }
    if args
        .get(1)
        .is_some_and(|arg| matches!(arg.as_str(), "--help" | "-h"))
    {
        let help = match command {
            ...
            "run" => "Usage: codegpt runner run [--profile NAME|--config PATH]\n\nRun codegpt-runner directly in the foreground.\n",
            "restart" => "Usage: codegpt runner restart [--profile NAME] [--bin PATH] [--scope user|system] [--service-file PATH]\n\nWith a profile created by `codegpt connect`, omitting --scope manages its user-level background Runner; --bin selects an explicit Runner binary for that hosted profile. An explicit scope manages the matching systemd service and does not accept --bin.\n",
            "start" | "stop" => "Usage: codegpt runner <start|stop> [--profile NAME] [--scope user|system] [--service-file PATH]\n\nWith a profile created by `codegpt connect`, omitting --scope manages its user-level background Runner. An explicit scope manages the matching systemd service.\n",
            "status" => runner_status_usage(),
            "logs" => "Usage: codegpt runner logs [--profile NAME] [--scope user|system] [--service-file PATH] [--lines N] [--since VALUE] [--follow]\n",
            "uninstall" => "Usage: codegpt runner uninstall [--profile NAME] [--scope user|system] [--service-file PATH] --confirm\n",
            "install-service" => "`codegpt runner install-service` was removed; use `codegpt runner install`.\n",
            _ => runner_usage(),
        };
    ...
    match command {
        ...
        "install-service" => exit_error(
            "`codegpt runner install-service` was removed; use `codegpt runner install`.\n",
        ),
...
fn parse_ops_runner(args: &[String]) -> Result<OpsRunnerOptions, String> {
    ...
    let mut request_timeout_ms = codegpt_cli::ops::DEFAULT_RUNNER_REQUEST_TIMEOUT_MS;
    let mut iter = args.iter();
    while let Some(arg) = iter.next() {
        match arg.as_str() {
...
fn parse_server_subcommand(args: &[String]) -> CliAction {
    ...
    if command == "run" && args.len() == 2 && matches!(args[1].as_str(), "--version" | "-V") {
        return exit_help(&build_info::version_output("codegpt-server"));
    }
    if args
        .get(1)
        .is_some_and(|arg| matches!(arg.as_str(), "--help" | "-h"))
    {
        let help = match command {
            ...
            "run" => "Usage: codegpt server run [--env-file PATH] [--help|--version]\n\nRun codegpt-server directly in the foreground. --env-file passes the exact path through CODEGPT_ENV_FILE; the Server remains the authoritative env-file parser.\n",
            "tunnel" => server_tunnel_usage(),
            "start" | "stop" | "restart" => "Usage: codegpt server <start|stop|restart>\n",
            "status" => server_status_usage(),
            "logs" => "Usage: codegpt server logs [--lines N] [--since VALUE] [--follow]\n",
            "uninstall" => "Usage: codegpt server uninstall --confirm\n",
            "up" => "`codegpt server up` was removed; use `codegpt server init`.\n",
            "install-service" => "`codegpt server install-service` was removed; use `codegpt server install`.\n",
            _ => server_usage(),
        };
        return exit_help(help);
    }
    match command {
        ...
        "up" => exit_error("`codegpt server up` was removed; use `codegpt server init`.\n"),
        "install-service" => exit_error(
            "`codegpt server install-service` was removed; use `codegpt server install`.\n",
        ),
        other => exit_error(&format!(
...
fn parse_server_run(args: &[String]) -> Result<InternalRunOptions, String> {
    ...
    let bin = discover_internal_binary("codegpt-server").ok_or_else(|| {
        "codegpt-server was not found beside codegpt or in an absolute PATH entry".to_string()
    })?;
    Ok(InternalRunOptions {
        bin,
        args: Vec::new(),
        env: env_file
            .map(|path| vec![(OsString::from("CODEGPT_ENV_FILE"), path.into_os_string())])
            .unwrap_or_default(),
    })
}
...
fn parse_runner_run(args: &[String]) -> Result<InternalRunOptions, String> {
    ...
    let bin = discover_internal_binary("codegpt-runner").ok_or_else(|| {
        "codegpt-runner was not found beside codegpt or in an absolute PATH entry".to_string()
    })?;
    Ok(InternalRunOptions {
        bin,
    ...
    if matches!(command, "start" | "stop" | "restart") {
        if let Some(flag) = args.first() {
            if flag == "--root" || flag == "--state-dir" || flag == "--console-assets-dir" {
                return Err(format!(
                    "`codegpt runner {command}` manages the installed service; use `codegpt run` for project runtime options"
                ));
            }
...
fn parse_runner_service_action(
    ...
    if let Some(bin) = runner_bin.as_ref() {
        ...
        if command != "restart" {
            return Err(
                "--bin is valid only with `codegpt runner restart --profile <name>`".to_string(),
            );
        }
    }
...
fn parse_runner_install_service_with_identity(
    ...
    let service_file = service_file
        ...
        .unwrap_or_else(|| runner_service_file_for_scope(scope, profile.as_deref()))?;
    validate_service_file_scope(scope, &service_file)?;
    let bin = match bin.or_else(|| discover_internal_binary("codegpt-runner")) {
        Some(path) => path,
        None => {
            return Err(
                "--bin is required because codegpt-runner was not found in PATH".to_string(),
            )
        }
    };
...
fn parse_server_install_service(args: &[String]) -> Result<ServerInstallServiceOptions, String> {
    ...
    let mut bin: Option<PathBuf> = None;
    let mut service_file = PathBuf::from("/etc/systemd/system/codegpt.service");
    let mut user = None;
    let mut group = None;
    let mut working_directory: Option<PathBuf> = None;
    let mut overwrite = false;
    ...
    let bin = match bin.or_else(|| discover_internal_binary("codegpt-server")) {
        Some(path) => path,
        None => return Err("--bin is required because codegpt-server was not found beside codegpt or in an absolute PATH entry".to_string()),
    };
    let working_directory = match working_directory {
        Some(path) => path,
        None if env_file.exists() => match read_env_file_value(&env_file, "CODEGPT_DATA")? {
            Some(value) if !value.trim().is_empty() => PathBuf::from(value.trim()),
            Some(_) => {
                return Err(format!(
                    "{} defines an empty CODEGPT_DATA; pass --working-directory explicitly",
                    env_file.display()
                ))
            }
...
/// Small flag parser for `codegpt runner init`. Produces an
...
pub async fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match cli_action(args) {
        CliAction::Project(args) => {
            let output = codegpt::run_project_command(args).await;
