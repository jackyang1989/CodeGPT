...
pub(crate) fn resolve_account_credential(
    ...
    let value = std::env::var("CODEGPT_ACCOUNT_CREDENTIAL")
        .map_err(|_| {
            "--credential, --credential-env, or CODEGPT_ACCOUNT_CREDENTIAL is required".to_string()
        })?
        .trim()
        .to_string();
    if value.is_empty() {
        return Err("CODEGPT_ACCOUNT_CREDENTIAL cannot be empty".to_string());
    }
    Ok(value)
}
