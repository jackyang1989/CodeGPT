...
pub(crate) fn run_plugin_init(opts: PluginInitOptions) -> Result<String, String> {
    ...
    Ok(format!(
        "Created Native Tool Plugin project at {}\nProvider id: {}\nSDK: @yyjeqhc/codegpt-plugin-sdk@{}\n\n{}\nNext steps:\n1. In the generated project, run: npm install\n2. Run: npm run build\n3. Copy the provider block above into the target Runner's startup-bound runner.toml. This config path is Runner-local; if you are unsure which profile/service config is active, inspect it on that Runner host with: codegpt runner status --profile <profile>\n4. Authenticate Plugin CLI requests. Prefer: --token-file /path/to/plugin-authoring-pat. CODEGPT_PAT is a fallback alias for user/API CLI input; CODEGPT_TOKEN remains preferred when both are set.\n5. codegpt plugin check --runner <runner> --plugin {}\n6. codegpt plugin reload --runner <runner>\n7. codegpt plugin list --runner <runner> --plugin {}\n8. codegpt plugin describe --runner <runner> --plugin {} --tool echo\n",
...
fn scaffold_files(provider_id: &str) -> Vec<ScaffoldFile> {
    vec![
        ...
        ScaffoldFile {
            path: "README.md",
            content: format!(
                "# {provider_id}\n\nThis project is a TypeScript Native Tool Plugin scaffold generated by `codegpt plugin init`. It uses the published `@yyjeqhc/codegpt-plugin-sdk@{PLUGIN_INIT_SDK_VERSION}` package and does not depend on a CodeGPT source checkout. Node.js 18 or newer is required to run the compiled Plugin.\n\n## Install and build\n\n```bash\nnpm install\nnpm run build\n```\n\n`npm run typecheck` is also available for a no-output type check. `plugin init` itself does not install dependencies or execute generated code.\n\n## Configure one Runner manually\n\nCopy and edit this provider entry in the target Runner's startup-bound `runner.toml`. Replace the portable absolute-path placeholder with the real Runner-local path to this project's compiled `dist/plugin.js`.\n\n```toml\n[[plugins.providers]]\nid = \"{provider_id}\"\nname = \"{provider_id}\"\ncommand = \"node\"\nargs = [\"/absolute/path/to/PLUGIN_DIRECTORY/dist/plugin.js\"]\ntimeout_secs = 30\n```\n\nCodeGPT does not edit Runner configuration as part of `plugin init`. The config path and executable path stay local to the Runner host. If you are unsure which profile/service config is active, inspect it on that host with `codegpt runner status --profile <profile>` or the equivalent service invocation.\n\n## Credential and author loop\n\nFor repeat authoring, prefer an explicit PAT file, for example `--token-file /path/to/plugin-authoring-pat`. User/API CLI commands also accept `CODEGPT_PAT` as a fallback environment alias. Existing `CODEGPT_TOKEN` input remains preferred when both environment names are present. `list` and `describe` require `plugin:inspect`; canonical `plugin_tool` invocation requires `plugin:invoke`; `check` and `reload` require `plugin:manage`. `--oauth-local-plugins` grants only `plugin:inspect + plugin:invoke`, never `plugin:manage`. CodeGPT does not mint or widen a credential as part of Plugin authoring.\n\n```text\ncodegpt plugin check --runner <runner> --plugin {provider_id} --token-file /path/to/plugin-authoring-pat\ncodegpt plugin reload --runner <runner> --token-file /path/to/plugin-authoring-pat\ncodegpt plugin list --runner <runner> --plugin {provider_id} --token-file /path/to/plugin-authoring-pat\ncodegpt plugin describe --runner <runner> --plugin {provider_id} --tool echo --token-file /path/to/plugin-authoring-pat\n```\n\nInvocation remains on the canonical existing `plugin_tool describe -> call` path; there is intentionally no `codegpt plugin call` command.\n"
            ),
        },
    ]
}
