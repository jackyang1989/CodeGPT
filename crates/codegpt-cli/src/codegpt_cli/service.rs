...
pub(crate) const SERVER_SERVICE_FILE: &str = "/etc/systemd/system/codegpt.service";
pub(crate) const SERVER_SERVICE_UNIT: &str = "codegpt.service";
pub(crate) const SERVER_SOCKET_UNIT: &str = "codegpt.socket";
pub(crate) const RUNNER_SERVICE_UNIT: &str = "codegpt-runner.service";
...
fn uninstall_error_with_rollback(error: String, rollback_errors: Vec<String>) -> String {
    let mut message = format!("uninstallation failed for codegpt Server unit pair: {error}");
    if !rollback_errors.is_empty() {
        let mut summary = rollback_errors.join("; ");
        if summary.len() > 600 {
            summary.truncate(600);
...
pub(crate) fn install_server_unit_pair_with_executor<E: ProcessExecutor>(
    ...
    {
        return Err(format!(
            "cannot migrate an active legacy {service_unit} directly to socket activation: stop the legacy Server first, then rerun `codegpt server install --overwrite`; this one-time migration boundary is intentionally fail-closed"
        ));
    }
...
mod tests {
    ...
    fn server_pair_control_keeps_restart_socket_out_of_target_and_stop_prevents_activation() {
        ...
        let mut legacy_stop =
            FakeExecutor::with_outputs(vec![failed("Unit codegpt.socket not found."), ok()]);
        control_server_unit_pair_with_executor(
            &mut legacy_stop,
            systemctl,
            SERVER_SERVICE_UNIT,
    ...
    fn server_pair_second_file_write_failure_restores_first_file() {
        ...
        let socket_file = tmp.path().join("codegpt.socket");
    ...
    fn user_scope_covers_install_status_control_logs_and_uninstall_manager_args() {
        ...
        let unit = "codegpt-runner-work.service";
    ...
    fn system_scope_never_adds_user_manager_flags() {
        let systemctl = Path::new("/usr/bin/systemctl");
        let unit = "codegpt-runner.service";
    ...
    fn new_install_daemon_reload_failure_removes_unit_and_reloads_again() {
        ...
        .unwrap_err();
        assert!(error.contains("installation failed for codegpt.service"));
        assert!(!service_file.exists());
        assert_eq!(
            executor.calls[0],
            [
    ...
    fn systemd_encoders_keep_distinct_program_argument_and_path_rules() {
        ...
        for value in ["codegpt", "code_gpt-1.service", "group.name"] {
    ...
    fn user_unit_parent_is_created_without_touching_systemd() {
        let tmp = tempfile::tempdir().unwrap();
        let service_file = tmp.path().join("xdg/systemd/user/codegpt-runner.service");
    ...
    fn external_vendor_runtime_and_generated_units_are_rejected_before_side_effects() {
        for (load_state, fragment) in [
            ("loaded", "/usr/lib/systemd/system/codegpt.service"),
            ("loaded", "/run/systemd/system/codegpt.service"),
            ("loaded", "/run/systemd/generator/codegpt.service"),
            ("generated", "/run/systemd/generator.late/codegpt.service"),
        ] {
            let tmp = tempfile::tempdir().unwrap();
            let service_file = tmp.path().join("codegpt.service");
    ...
    fn unit_name_comes_from_selected_service_file() {
        assert_eq!(
            service_unit_name(
                Path::new("/etc/systemd/system/codegpt-runner-special.service"),
                RUNNER_SERVICE_UNIT
            ),
            "codegpt-runner-special.service"
        );
    }
}
