...
pub(crate) fn render_login_result(
    ...
    if json {
        ...
        let summary = serde_json::json!({
            ...
            "credential_usage": {
                "codegpt-user-token": "GPT Actions, MCP, and REST/project APIs",
...
pub(crate) fn render_status(connections: &[Connection], json: bool) -> Result<String, String> {
    ...
    if connections.is_empty() {
        return Ok(
            "Not logged in to any server.\n\nRun: codegpt login <server-url> --code <pairing-code>\n"
                .to_string(),
        );
    }
...
pub(crate) async fn run_login(opts: LoginOptions) -> Result<String, String> {
    ...
    let mut output_allowed_roots = codegpt_runner_config::effective_allowed_roots(
        &opts.allowed_roots,
...
mod tests {
    ...
    fn default_device_name_uses_the_platform_hostname_source() {
        ...
        #[cfg(windows)]
        let env = crate::codegpt_cli::test_support::EnvGuard::new()
            .remove("COMPUTERNAME")
            .remove("HOSTNAME");
        #[cfg(not(windows))]
        let env = crate::codegpt_cli::test_support::EnvGuard::new().remove("HOSTNAME");
        let _env = env;

        #[cfg(windows)]
        {
            // COMPUTERNAME is the OS-owned source and wins over HOSTNAME.
            let _c = crate::codegpt_cli::test_support::EnvGuard::new()
            ...
            let _c2 = crate::codegpt_cli::test_support::EnvGuard::new()
                .remove("COMPUTERNAME")
                .set("HOSTNAME", "Msys-Host");
            assert_eq!(default_device_name(), "msys-host");
            // Neither: the stable fallback.
            let _c3 = crate::codegpt_cli::test_support::EnvGuard::new()
                .remove("COMPUTERNAME")
                .remove("HOSTNAME");
            assert_eq!(default_device_name(), "device");
            // An empty COMPUTERNAME is treated as missing.
            let _c4 = crate::codegpt_cli::test_support::EnvGuard::new()
                .set("COMPUTERNAME", "  ")
                .remove("HOSTNAME");
            assert_eq!(default_device_name(), "device");
        }
        #[cfg(not(windows))]
        {
            ...
            let _h = crate::codegpt_cli::test_support::EnvGuard::new().set("HOSTNAME", "web-1");
            assert_eq!(default_device_name(), "web-1");
            // COMPUTERNAME is a foreign variable on Unix and must not win.
            let _h2 = crate::codegpt_cli::test_support::EnvGuard::new()
                .set("HOSTNAME", "web-2")
    ...
    fn a_fresh_login_publishes_through_staging_and_leaves_nothing_behind() {
        ...
        assert!(!paths.dir.join("codegpt-runner-token").exists());
    ...
    fn staged_login_persists_exact_network_project_authority_override() {
        ...
        assert!(codegpt_runner_config::paths::paths_equal(
    ...
    fn assert_no_credentials_in(dir: &Path) {
        ...
        for entry in std::fs::read_dir(dir).unwrap() {
            ...
            if matches!(
                name.as_str(),
                "server.toml" | "runner.toml" | "codegpt-user-token"
    ...
    async fn login_project_registers_existing_workspace_into_published_registry() {
        ...
        let records =
            crate::codegpt_cli::connect::profile::read_project_files(&paths.project_registry_dir)
                .unwrap();
    ...
    async fn login_project_allows_raw_unc_to_reach_canonicalization_before_redemption() {
        ...
        let address = listener.local_addr().unwrap();
        drop(listener);
        let unc = PathBuf::from(r"\\server\share\codegpt-unreachable-repo");
        let opts = LoginOptions {
            server_url: format!("http://{address}"),
            server_http: ServerHttpOptions {
                proxy: None,
    ...
    fn render_login_result_includes_safe_metadata_and_no_tokens_by_default() {
        ...
        assert!(!text.contains("codegpt runner install"), "{text}");
    ...
    fn render_login_result_with_project_gives_runner_next_action() {
        ...
        assert!(text.contains("codegpt runner run --config"), "{text}");
        assert!(text.contains("Keep this terminal open."), "{text}");
        assert!(text.contains("Ctrl-C stops this Runner."), "{text}");
        assert_eq!(
            text.contains("codegpt runner install --scope user --config"),
            cfg!(target_os = "linux"),
            "{text}"
        );
    ...
    fn render_login_result_quotes_config_paths_and_exposes_argv() {
        ...
        assert!(value["next_steps"][0]
            .as_str()
            .unwrap()
            .contains("codegpt project register"));
        assert_eq!(
        ...
        std::fs::write(parser_env.path().join("codegpt-runner"), "").unwrap();
        #[cfg(windows)]
        std::fs::write(parser_env.path().join("codegpt-runner.exe"), "").unwrap();
        let _guard = crate::codegpt_cli::test_support::env_test_guard();
    ...
    fn render_login_result_root_has_usable_commands_and_warning() {
        ...
        assert!(text.contains("codegpt runner run"));
        ...
        if cfg!(target_os = "linux") {
            ...
            // package, so no sibling `codegpt-runner` binary is guaranteed to
