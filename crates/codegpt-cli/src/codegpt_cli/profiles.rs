...
pub(crate) fn client_base_dir_for_scope(scope: ServiceScope) -> Result<PathBuf, String> {
    match scope {
        ServiceScope::User => Ok(user_config_home()?.join("codegpt")),
        ServiceScope::System => Ok(PathBuf::from("/etc/codegpt")),
    }
}
...
pub(crate) fn runner_service_file_for_scope(
    ...
    let name = match profile {
        Some(profile) => format!("codegpt-runner-{profile}.service"),
        None => "codegpt-runner.service".to_string(),
    };
...
pub(crate) fn client_profile_user_token_file(profile: &str) -> Result<PathBuf, String> {
    Ok(client_profile_dir(profile)?.join("codegpt-user-token"))
}

pub(crate) fn client_profile_runner_token_file(profile: &str) -> Result<PathBuf, String> {
    Ok(client_profile_dir(profile)?.join("codegpt-runner-token"))
}
