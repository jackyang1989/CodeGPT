...
fn ops_http_failure_action(kind: OpsHttpFailureKind) -> &'static str {
    match kind {
        OpsHttpFailureKind::Unauthorized => {
            "provide a user token/PAT or bearer token accepted by the CodeGPT server"
        }
        OpsHttpFailureKind::Forbidden => {
        ...
        OpsHttpFailureKind::ServerError => {
            "inspect CodeGPT server logs for the failing ops request"
        }
        OpsHttpFailureKind::NonJson => {
...
pub(crate) fn ops_status_report(server_url: &str, runtime: &Option<Value>) -> OpsReport {
    ...
    if online == 0 {
        verdict.fail_reason(
            "no_online_agents",
            "start a codegpt-runner and rerun ops status",
        );
    } else {
        if stale > 0 {
            verdict.warn_reason(
...
pub(crate) fn ops_runners_report(server_url: &str, runtime: &Option<Value>) -> OpsReport {
    ...
    if online == 0 {
        verdict.fail_reason(
            "no_online_agents",
            "start a codegpt-runner and rerun ops runners",
        );
    }
    if stale > 0 {
        verdict.warn_reason(
