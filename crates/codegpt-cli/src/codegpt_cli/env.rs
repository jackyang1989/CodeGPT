...
use codegpt_runner_config::paths;
...
pub(crate) fn default_server_paths() -> Result<ServerPathDefaults, String> {
    if paths::is_effective_root() {
        return Ok(ServerPathDefaults {
            data_dir: PathBuf::from("/var/lib/codegpt"),
            env_file: PathBuf::from("/etc/codegpt/codegpt.env"),
        });
    }
    ...
    Ok(ServerPathDefaults {
        data_dir: home.join(".local/share/codegpt"),
        env_file: home.join(".config/codegpt/codegpt.env"),
    })
}
...
pub(crate) fn render_server_env(opts: &ServerInitOptions, token: &str) -> String {
    ...
    content.push_str(&format!("CODEGPT_ADDR={}\n", opts.listen.trim()));
    content.push_str(&format!("CODEGPT_DATA={}\n", opts.data_dir.display()));
    content.push_str(&format!("CODEGPT_TOKEN={}\n", token));
    if let Some(public_url) = &opts.public_url {
        let public_url = public_url.trim().trim_end_matches('/');
        content.push_str(&format!("CODEGPT_PUBLIC_URL={public_url}\n"));
        content.push_str("CODEGPT_OAUTH2_ENABLED=true\n");
        content.push_str(&format!("CODEGPT_OAUTH2_ISSUER={public_url}\n"));
        content.push_str("CODEGPT_OAUTH2_SHARED_KEY_BRIDGE=true\n");
    }
    content.push_str("CODEGPT_SHARED_KEY_ENABLED=true\n");
    if opts.open {
        content.push_str("CODEGPT_ALLOW_ANONYMOUS=true\n");
    }
    content
}
