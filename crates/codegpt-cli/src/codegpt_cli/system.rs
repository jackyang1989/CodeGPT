...
pub(crate) fn resolve_user_api_token(
    ...
    if let Some(path) = env_file {
        for key in ["CODEGPT_TOKEN", "CODEGPT_PAT"] {
...
mod tests {
    ...
    fn user_api_token_resolution_preserves_explicit_and_file_precedence() {
        ...
        let _env = EnvGuard::new()
            .set("CODEGPT_TOKEN", "process-token")
            .set("CODEGPT_PAT", "process-pat");
    ...
    fn user_api_token_resolution_uses_pat_only_as_env_fallback() {
        ...
        std::fs::write(&env_file, "CODEGPT_PAT=env-pat\n").unwrap();
        ...
        std::env::remove_var("CODEGPT_TOKEN");
        let selected = resolve_user_api_token(&None, &None, &None).unwrap();
