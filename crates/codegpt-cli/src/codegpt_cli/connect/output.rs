...
pub(super) fn render_connect_output(
    ...
    output.push_str("CodeGPT connected\n\nWhat to do next\n");
...
mod tests {
    ...
    fn generated_key_output_prioritizes_chatgpt_setup_and_discloses_once() {
        ...
        assert!(output.starts_with("CodeGPT connected\n\nWhat to do next"));
        assert_eq!(output.matches("https://codegpt.example/mcp").count(), 1);
