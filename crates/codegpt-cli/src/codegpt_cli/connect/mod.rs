...
pub(crate) fn write_connect_result(
    ...
    for marker in result.disclosure_markers {
        if let Err(error) = atomic_write(&marker, b"disclosed = true\n", false) {
            let _ = writeln!(
                stderr,
                "Warning: the connection is healthy, but CodeGPT could not record that a generated credential was displayed ({error}). It may be displayed again on the next connect."
            );
            let _ = stderr.flush();
        }
    }
