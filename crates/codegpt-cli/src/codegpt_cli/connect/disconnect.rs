...
pub(crate) async fn run_disconnect(opts: DisconnectOptions) -> Result<DisconnectResult, String> {
    ...
    if !config_base.is_dir() || !state_base.is_dir() {
        return Err(format!(
            "no hosted `codegpt connect` profile registers project {}",
            canonical_project.display()
        ));
    }
    ...
    let project_registry_dir =
        codegpt_runner_config::paths::select_project_registry_dir(&candidate.profile_dir)?;
    ...
    let config_path =
        codegpt_runner_config::paths::resolve_runner_config_path(&candidate.profile_dir)?;
    ...
    if !runner.managed {
        return Err(format!(
            "profile {} is not a hosted `codegpt connect` profile",
            candidate.profile
        ));
    }
    ...
    for profile in names {
        ...
        if !marker.exists() {
            if explicit_profile.is_some() {
                return Err(format!(
                    "profile {profile} is not a hosted `codegpt connect` profile"
                ));
            }
            continue;
        }
...
mod tests {
    ...
    async fn successful_live_unregister_accepts_runner_removed_registration_and_stops_last_runner()
    {
        ...
        let runner = tmp.path().join("codegpt-runner");
        std::fs::write(
            &runner,
