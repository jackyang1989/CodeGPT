...
fn spawn_log_writer(state_dir: &Path) -> Result<SpawnedLogWriter, String> {
    ...
    let executable = std::env::current_exe()
        ...
        .map_err(|error| format!("failed to resolve the codegpt executable: {error}"))?;
...
fn start_runner(
    ...
    let mut child = match command.spawn() {
        ...
        Err(error) => {
            ...
            return Err(format!(
                "failed to start codegpt-runner {}: {error}",
                executable.display()
            ));
        }
    };
    ...
    if let Some(status) = immediate_status {
        drop(command);
        finish_owned_log_writer(log_writer.owned);
        return Err(format!("codegpt-runner exited immediately with {status}"));
    }
    ...
    match action {
        ...
        LocalRunnerServiceAction::Start | LocalRunnerServiceAction::Restart => {
            ...
            let runner = runner_bin
                ...
                .ok_or_else(|| {
                    "codegpt-runner was not found beside codegpt or in an absolute PATH entry"
                        .to_string()
                })?;
...
mod tests {
    ...
    fn local_runner_reuses_process_recovers_stale_pid_and_stops_windows() {
        ...
        let runner = tmp.path().join("codegpt-runner.cmd");
        std::fs::write(
            &runner,
