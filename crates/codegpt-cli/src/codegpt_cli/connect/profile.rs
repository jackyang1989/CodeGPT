...
pub(crate) struct ConnectOptions {
    ...
    pub(crate) oauth_local_ssh: bool,
...
impl ProfileLock {
    pub(super) fn acquire(state_dir: &Path) -> Result<Self, String> {
        ...
        file.try_lock_exclusive().map_err(|_| {
            format!(
                "another CodeGPT command is updating this profile; retry after it finishes ({})",
                path.display()
            )
        })?;
        Ok(Self { file })
...
pub(super) fn stored_project_matches(project: &ProjectFile, canonical_project: &Path) -> bool {
    Path::new(&project.path).canonicalize().is_ok_and(|path| {
        ...
        codegpt_runner_config::paths::paths_equal(&path, canonical_project)
    })
}
