...
fn read_managed_identity(
    ...
    let connection = match connections.len() {
        ...
        0 if requested_user.is_some() => {
            return Err(format!(
                "no logged-in CodeGPT user '{}' exists for this Server; run `codegpt login` first",
                requested_user.unwrap_or_default()
            ))
        }
        0 => {
            return Err(
                "OAuth connect requires a managed login for this Server; run `codegpt login <server> --code ...` first"
                    .to_string(),
            )
        }
        _ => {
...
async fn fetch_oauth_metadata(
    ...
    if response.status().as_u16() == 404 {
        return Err("the remote CodeGPT Server does not have OAuth enabled".to_string());
    }
    ...
    if scopes_supported.is_empty() {
        return Err(
            "Server OAuth metadata exposes no delegable CodeGPT permission scopes".to_string(),
        );
    }
...
async fn create_runner_token(
    ...
    let value = post_json_authed(ApiCall {
        ...
        body: json!({
            "username": username,
            "client_id": client_id,
            "name": format!("codegpt connect oauth {client_id}"),
        }),
    })
    .await?;
...
async fn ensure_oauth_client(
    ...
    let (client_id, client_secret) = create_oauth_client(
        ...
        opts,
        user_token,
        &format!("CodeGPT connect {profile_name}"),
...
pub(super) async fn run_oauth_connect(opts: ConnectOptions) -> Result<ConnectResult, String> {
    ...
    let config_path = codegpt_runner_config::paths::resolve_runner_config_path(&profile_dir)?;
    let oauth_path = profile_dir.join(OAUTH_PROFILE_FILE);
    let project_registry_dir =
        codegpt_runner_config::paths::select_project_registry_dir(&profile_dir)?;
    ...
    let runner_bin = opts
        .runner_bin
        .clone()
        .or_else(|| discover_internal_binary("codegpt-runner"))
        .ok_or_else(|| {
    ...
    ) = existing_oauth
    {
        ...
        let (oauth_client_id, oauth_client_secret) = match create_oauth_client(
            ...
            &opts,
            &identity.user_token,
            &format!("CodeGPT connect {profile}"),
            &redirect_uri,
            &scopes,
        )
        .await
...
pub(super) fn observer_token_for_disconnect(
    ...
    if connections.len() != 1 {
        return Err(format!(
            "OAuth hosted profile requires the managed login for user {}; run `codegpt login` again before disconnecting the live project",
            profile.username
        ));
    }
...
mod tests {
    ...
    async fn oauth_discovery_uses_closed_hosted_connect_scope_set() {
        let (server, handle) = one_json_response(json!({
            "issuer": "https://codegpt.example",
            "authorization_endpoint": "https://codegpt.example/oauth/authorize",
            "token_endpoint": "https://codegpt.example/oauth/token",
            "scopes_supported": [
                "runtime:read",
                "job:detach",
                "computer:launch",
