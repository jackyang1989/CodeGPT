...
fn bridge_scope_output(profile: &SharedKeyOAuthProfile) -> String {
    if profile.computer_permissions_enabled {
        format!(
            "Client may request: {}\nProtocol scope: offline_access\nBrowser consent: Additional Computer permissions are granted only when selected on the CodeGPT authorization page.\n",
...
fn bridge_browser_authorization_key_line(resolved_key: &ResolvedKey, config_path: &Path) -> String {
    if resolved_key.generated {
        format!(
            "Browser authorization key: {}\nEnter this key only on the CodeGPT authorize page; do not put it in ChatGPT.\n",
            resolved_key.value
        )
    } else if resolved_key.recovered_profile.is_some() {
        format!(
            "Browser authorization key source: {} (top-level token; not reprinted).\nEnter that key only on the CodeGPT authorize page; do not put it in ChatGPT.\n",
            config_path.display()
        )
    } else {
        "Browser authorization key: use the shared key supplied to this command on the CodeGPT authorize page; do not put it in ChatGPT.\n".to_string()
    }
}
