...
pub(crate) fn resolve_pairing_create_token(opts: &PairingCreateOptions) -> Result<String, String> {
    ...
    if let Some(path) = &opts.env_file {
        let token = read_pairing_server_env_file_value(path, "CODEGPT_TOKEN")?
            .unwrap_or_default()
            .trim()
            .to_string();
        if token.is_empty() {
            return Err(format!(
                "env file {} does not contain CODEGPT_TOKEN",
                path.display()
            ));
        }
        return Ok(token);
    ...
    let token = std::env::var("CODEGPT_TOKEN")
        ...
        .to_string();
    if token.is_empty() {
        return Err("CODEGPT_TOKEN cannot be empty".to_string());
    }
    Ok(token)
}
