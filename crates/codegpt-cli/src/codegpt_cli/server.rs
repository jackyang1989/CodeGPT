...
pub(crate) async fn run_server_tunnel(opts: ServerTunnelOptions) -> Result<(), String> {
    ...
    let runtime_parent = opts
        ...
        .to_path_buf();
    codegpt::run_regular_server_tunnel(codegpt::RegularServerTunnelOptions {
        local_server_url,
        bootstrap_token,
        runtime_parent,
    })
...
pub(crate) fn derive_regular_tunnel_bootstrap_token(env_file: &Path) -> Result<String, String> {
    let value = match std::env::var("CODEGPT_TOKEN") {
        Ok(value) => Some(value),
        Err(std::env::VarError::NotPresent) => read_env_file_value(env_file, "CODEGPT_TOKEN")?,
        Err(std::env::VarError::NotUnicode(_)) => {
            return Err("CODEGPT_TOKEN is not valid UTF-8".to_string())
        }
    };
    value
        ...
        .ok_or_else(|| {
            "regular Server Tunnel requires the effective local Server CODEGPT_TOKEN".to_string()
        })
}
...
pub(crate) fn derive_regular_tunnel_server_url(env_file: &Path) -> Result<String, String> {
    ...
    let value = read_env_file_value(env_file, "CODEGPT_ADDR")?
        .ok_or_else(|| format!("{} does not define CODEGPT_ADDR", env_file.display()))?;
    let mut addr = value.trim().parse::<SocketAddr>().map_err(|error| {
        format!(
            "CODEGPT_ADDR {:?} from {} is not a fixed IP socket address: {error}",
            value.trim(),
            env_file.display()
        )
    })?;
    ...
    if !addr.ip().is_loopback() {
        return Err("server tunnel requires a loopback CODEGPT_ADDR".to_string());
    }
    Ok(format!("http://{addr}"))
}
...
pub(crate) fn run_server_init(opts: ServerInitOptions) -> Result<String, String> {
    ...
    let existing_token = if opts.env_file.exists() {
        read_env_file_value(&opts.env_file, "CODEGPT_TOKEN")?
    ...
    out.push_str("CodeGPT Server configured.\n\n");
    out.push_str("Data:\n");
    out.push_str(&format!("  {}\n", opts.data_dir.display()));
    out.push_str("\nNext:\n");
    if let Some(command) = &install_command {
...
fn configured_socket_addr(env_file: &Path) -> Result<String, String> {
    ...
    if value.is_empty() {
        return Err(format!(
            "{} defines an empty CODEGPT_ADDR",
            env_file.display()
        ));
    }
    let addr = value.parse::<SocketAddr>().map_err(|error| {
        format!(
            "CODEGPT_ADDR {value:?} from {} is not a fixed IP socket address valid for systemd ListenStream: {error}",
            env_file.display()
        )
    })?;
    Ok(addr.to_string())
...
fn preflight_server_install(opts: &ServerInstallServiceOptions) -> Result<(), String> {
    let binary = std::fs::metadata(&opts.bin).map_err(|_| {
        format!(
            "cannot install CodeGPT Server: binary {} does not exist",
            opts.bin.display()
        )
    })?;
    if !binary.is_file() {
        return Err(format!(
            "cannot install CodeGPT Server: binary {} is not a regular file",
            opts.bin.display()
        ));
    }
    #[cfg(unix)]
    {
        ...
        if binary.permissions().mode() & 0o111 == 0 {
            return Err(format!(
                "cannot install CodeGPT Server: binary {} is not executable",
                opts.bin.display()
            ));
        }
    }
    let working = std::fs::metadata(&opts.working_directory).map_err(|_| {
        format!(
            "cannot install CodeGPT Server: WorkingDirectory {} does not exist\n\nCreate it or pass --working-directory <existing-directory>.",
            opts.working_directory.display()
        )
    })?;
    if !working.is_dir() {
        return Err(format!(
            "cannot install CodeGPT Server: WorkingDirectory {} is not a directory",
            opts.working_directory.display()
        ));
    }
    let env = std::fs::metadata(&opts.env_file).map_err(|_| {
        format!(
            "cannot install CodeGPT Server: env file {} does not exist",
            opts.env_file.display()
        )
    })?;
    if !env.is_file() {
        return Err(format!(
            "cannot install CodeGPT Server: env file {} is not a regular file",
            opts.env_file.display()
        ));
    }
    std::fs::File::open(&opts.env_file).map_err(|error| {
        format!(
            "cannot install CodeGPT Server: env file {} is not readable: {error}",
            opts.env_file.display()
        )
    })?;
    if let Some(user) = &opts.user {
        if !system_user_exists(user) {
            return Err(format!(
                "cannot install CodeGPT Server: User={user} does not name a local account"
            ));
        }
    }
    if let Some(group) = &opts.group {
        if !system_group_exists(group) {
            return Err(format!(
                "cannot install CodeGPT Server: Group={group} does not name a local group"
            ));
        }
    }
    Ok(())
...
fn render_systemd_unit(
    ...
    unit.push_str("[Unit]\n");
    unit.push_str("Description=CodeGPT Runtime\n");
...
fn resolve_status_token(opts: &ServerStatusOptions) -> Result<Option<String>, String> {
    ...
    if let Some(path) = &opts.env_file {
        ...
        } else if let Some(token) = read_env_file_value(path, "CODEGPT_TOKEN")? {
    ...
    if let Ok(token) = std::env::var("CODEGPT_TOKEN") {
        let token = token.trim().to_string();
        if !token.is_empty() {
            return Ok(Some(token));
        }
...
pub(crate) fn derive_server_status_url(opts: &ServerStatusOptions) -> Result<String, String> {
    ...
    let Some(value) = read_env_file_value(env_file, "CODEGPT_ADDR")? else {
        return Ok(opts.url.clone());
    };
    let value = value.trim();
    if value.is_empty() {
    ...
    let mut addr = value.parse::<SocketAddr>().map_err(|error| {
        format!(
            "CODEGPT_ADDR {value:?} from {} is not a fixed IP socket address: {error}",
            env_file.display()
        )
    })?;
    if addr.ip().is_unspecified() {
...
pub(crate) async fn run_server_status(opts: ServerStatusOptions) -> Result<String, String> {
    ...
    if !http.reachable {
        if let Some(env_file) = opts.env_file.as_ref() {
            ...
            out.push_str("  Start the CodeGPT Server, then run this status command again.\n");
        }
    } else if agents_online_count == Some(0) {
        out.push_str(
            "  Create a one-time login code in another terminal with `codegpt pairing create`.\n",
        );
    } else if agents_online_count.is_some_and(|count| count > 0) {
        out.push_str(
            "  Check project readiness on the project machine with `codegpt runner status`.\n",
        );
    } else {
        out.push_str("  Continue first-run setup with `codegpt pairing create`, or check an existing Runner.\n");
    }
    out.push_str("\nDetails:\n");
