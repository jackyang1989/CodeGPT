...
use crate::codegpt_cli::run_ops_command;
...
fn ops_help_entrypoints_print_usage() {
    let cases: &[(&[&str], &[&str])] = &[
        ...
        (
            ...
            &[
                "Usage: codegpt ops smoke-preflight",
                "--project PROJECT_ID",
                "--server-url URL",
                "--env-file PATH",
                "--token-file PATH",
...
// `CODEGPT_TOKEN` from the process env is the tested product behavior: it
    ...
    let projects = json!({
        ...
        "projects": [
            ...
            },
            {
                ...
                "connected": false,
                "allow_patch": true,
                "path": "/srv/codegpt-offline-smoke",
                "capabilities": {
                    "git_available": true,
                    "safe_smoke_project": true,
                    "recommended_for_smoke": true
...
fn ops_json_and_human_outputs_do_not_contain_secret_values() {
    ...
    assert!(!json_output.contains("CODEGPT_TOKEN="));
    assert!(!human_output.contains(secret));
    assert!(!human_output.contains("CODEGPT_TOKEN="));
