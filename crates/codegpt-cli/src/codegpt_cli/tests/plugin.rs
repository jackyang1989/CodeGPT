use super::support::*;
use crate::codegpt_cli::plugin::{parse_plugin_command, run_plugin_command, PluginCommand};
use crate::codegpt_cli::plugin_init::{
...
async fn run_once(
    ...
    Result<crate::codegpt_cli::plugin::PluginCommandOutput, String>,
    Vec<String>,
) {
...
fn plugin_init_creates_exact_public_sdk_scaffold_without_executing_dependencies() {
    ...
    assert!(output.contains("@yyjeqhc/codegpt-plugin-sdk@0.1.0"));
    ...
    assert!(
        output.contains("codegpt runner status --profile <profile>"),
        "{output}"
    );
    assert!(
    ...
    assert!(output.contains("CODEGPT_PAT"), "{output}");
    let snippet = output
        .split_once(
    ...
    assert!(readme.contains("codegpt plugin check --runner <runner> --plugin echo-plugin"));
    assert!(readme.contains("--tool echo"));
    assert!(readme.contains("--token-file /path/to/plugin-authoring-pat"));
    assert!(readme.contains("CODEGPT_PAT"));
...
fn plugin_init_provider_snippet_uses_toml_escaping_for_cross_platform_paths() {
    for entrypoint in [
        ...
        r#"C:\Program Files\CodeGPT\plugin "quoted"\dist\plugin.js"#,
    ] {
