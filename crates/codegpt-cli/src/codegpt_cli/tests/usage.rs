...
fn cli_help_and_version_exit_before_dispatch() {
    match cli_action(["--help"]) {
        CliAction::Exit { code, stdout, .. } => {
            assert_eq!(code, 0);
            assert!(stdout.contains("Usage: codegpt"));
        }
    ...
    match cli_action(["--version"]) {
        CliAction::Exit { code, stdout, .. } => {
            assert_eq!(code, 0);
            assert!(stdout.starts_with(&format!("codegpt {} (commit ", env!("CARGO_PKG_VERSION"))));
            assert!(stdout.trim_end().ends_with(')'));
            assert_ne!(stdout, format!("codegpt {}\n", env!("CARGO_PKG_VERSION")));
        }
...
fn cli_version_output_includes_build_metadata() {
    match cli_action(["-V"]) {
        CliAction::Exit {
            ...
            assert!(stdout.starts_with("codegpt "));
...
#[test]
fn codegpt_cli_help_presents_primary_mental_model() {
    ...
    assert!(stdout.contains("codegpt server --help"));
    assert!(stdout.contains("codegpt runner --help"));
...
fn foreground_run_banner_describes_lifetime_without_false_readiness() {
    ...
    assert!(server.contains("Starting CodeGPT Server in the foreground."));
    ...
    assert!(runner.contains("Starting CodeGPT Runner in the foreground."));
...
fn unified_project_and_auth_commands_dispatch() {
    ...
    match cli_action(["auth", "status", "--help"]) {
        CliAction::Exit { code, stdout, .. } => {
            assert_eq!(code, 0);
            assert!(stdout.contains("Usage: codegpt auth status"));
        }
...
#[test]
fn codegpt_cli_runner_help_mentions_lifecycle_subcommands() {
    match cli_action(["runner", "--help"]) {
        CliAction::Exit { code, stdout, .. } => {
            ...
            assert!(stdout.contains("codegpt run"));
        }
        other => panic!("expected help exit, got {other:?}"),
    }
    match cli_action(["runner", "init", "--help"]) {
        CliAction::Exit { code, stdout, .. } => {
            assert_eq!(code, 0);
            assert!(stdout.contains("Usage: codegpt runner init"));
    ...
    for command in [
        ...
        match cli_action(["runner", command, "--help"]) {
            CliAction::Exit { code, stdout, .. } => {
                ...
                assert!(
                    stdout.contains("codegpt runner"),
...
fn existing_removed_lifecycle_stubs_stay_fail_closed() {
    for (args, replacement) in [
        (vec!["server", "up"], "codegpt server init"),
        (vec!["server", "install-service"], "codegpt server install"),
        (vec!["runner", "install-service"], "codegpt runner install"),
    ] {
...
fn removed_legacy_onboarding_paths_fail_closed_with_migration_guidance() {
    for (args, replacement) in [
        ...
        (vec!["client", "enroll", "--help"], "codegpt login"),
    ] {
        match cli_action(args) {
            CliAction::Exit {
                code,
...
fn canonical_plural_admin_actions_dispatch_and_singular_groups_fail_closed() {
    ...
    for (group, replacement) in [
        ("token", "codegpt tokens"),
        ("agent-token", "codegpt runner-tokens"),
    ] {
...
fn login_help_describes_root_and_non_root_default_directories() {
    match cli_action(["login", "--help"]) {
        CliAction::Exit { code, stdout, .. } => {
            assert_eq!(code, 0);
            assert!(stdout.contains("root /etc/codegpt"), "{stdout}");
            assert!(stdout.contains("non-root ~/.config/codegpt"), "{stdout}");
        }
        other => panic!("expected help exit, got {other:?}"),
    }
}
