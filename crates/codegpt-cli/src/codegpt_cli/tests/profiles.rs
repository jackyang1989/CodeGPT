...
/// shared `codegpt_runner_config::paths` policy, so setting them pins the
...
fn user_scope_runner_config_keeps_legacy_only_and_rejects_dual_names() {
    ...
    let config_dir = tmp.path().join("codegpt");
...
fn runner_status_explicit_paths_win_and_no_profile_uses_canonical_default() {
    ...
    assert_eq!(default.config, PathBuf::from("/etc/codegpt/runner.toml"));
    assert_eq!(
        default.service_file,
        PathBuf::from("/etc/systemd/system/codegpt-runner.service")
    );
...
fn runner_install_service_profile_derives_config_and_service_file() {
    ...
    assert!(unit.contains(
        "ExecStart=\"/opt/codegpt/bin/codegpt-runner\" \"--config\" \"/etc/codegpt/clients/special/runner.toml\""
    ));
}
...
fn runner_install_service_explicit_paths_win_and_rejects_unsafe_profile() {
    ...
    assert_eq!(
        opts.service_file,
        PathBuf::from("/tmp/codegpt-runner.service")
    );
...
/// `codegpt_runner_config::paths` tests.
#[cfg(unix)]
#[test]
fn runner_service_scope_parsing_defaults_and_paths_are_deterministic() {
    ...
    let user = parse_runner_install_service_with_identity(
        &args(&["--bin", "/opt/codegpt/bin/codegpt-runner", "--dry-run"]),
        false,
    )
    .unwrap();
    ...
    assert_eq!(
        user.config,
        PathBuf::from("/tmp/alice-config/codegpt/runner.toml")
    );
    assert_eq!(
        user.service_file,
        PathBuf::from("/tmp/alice-config/systemd/user/codegpt-runner.service")
    );
    ...
    assert_eq!(root.config, PathBuf::from("/etc/codegpt/runner.toml"));
    assert_eq!(
        root.service_file,
        PathBuf::from("/etc/systemd/system/codegpt-runner.service")
    );
...
fn user_scope_falls_back_to_home_and_profile_paths() {
    ...
    assert_eq!(
        opts.config,
        PathBuf::from("/home/bob/.config/codegpt/clients/work/runner.toml")
    );
    assert_eq!(
        opts.service_file,
        PathBuf::from("/home/bob/.config/systemd/user/codegpt-runner-work.service")
    );
}
...
fn runner_service_scope_rejects_invalid_and_conflicting_flags() {
    let bin = "/opt/codegpt/bin/codegpt-runner";
...
fn explicit_service_paths_override_defaults_but_wrong_scope_paths_are_rejected() {
    let user = parse_runner_install_service_with_identity(
        &args(&[
            ...
            "--service-file",
            "/tmp/codegpt-runner-custom.service",
            "--working-directory",
            "/tmp",
            "--bin",
            "/opt/codegpt/bin/codegpt-runner",
    ...
    assert_eq!(
        user.service_file,
        PathBuf::from("/tmp/codegpt-runner-custom.service")
    );
    ...
    let error = parse_runner_install_service_with_identity(
        &args(&[
            ...
            "user",
            "--service-file",
            "/etc/systemd/system/codegpt-runner.service",
            "--bin",
            "/opt/codegpt/bin/codegpt-runner",
        ]),
        false,
    ...
    let error = parse_runner_service_action(
        ...
        &args(&[
            ...
            "user",
            "--service-file",
            "/home/alice/.config/systemd/user/../../../../etc/systemd/system/codegpt-runner.service",
        ]),
    )
    .unwrap_err();
...
fn hosted_profile_runner_bin_override_is_narrow_and_explicit() {
    let _guard = env_test_guard();
    let _env = deterministic_user_env();
    let bin = "/tmp/codegpt-dev-runner";
    ...
    for (command, values, expected) in [
        ...
        (
            ...
            "valid only with `codegpt runner restart",
        ),
    ] {
...
fn omitted_scope_hosted_status_keeps_xdg_profile_paths_for_root() {
    ...
    assert_eq!(
        opts.config,
        PathBuf::from("/tmp/test-xdg/codegpt/clients/hosted/runner.toml")
    );
    assert_eq!(
        opts.user_token_file,
        Some(PathBuf::from(
            "/tmp/test-xdg/codegpt/clients/hosted/codegpt-user-token"
        ))
    );
    assert_eq!(
        opts.runner_token_file,
        Some(PathBuf::from(
            "/tmp/test-xdg/codegpt/clients/hosted/codegpt-runner-token"
        ))
    );
...
fn every_runner_service_action_accepts_scope_and_service_file() {
    let service_file = "/home/alice/.config/systemd/user/codegpt-runner-work.service";
    for command in ["start", "stop", "restart"] {
        ...
        assert_eq!(parsed.unit, "codegpt-runner-work.service");
    }
