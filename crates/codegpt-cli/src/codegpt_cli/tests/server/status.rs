...
fn server_status_accepts_custom_service_file() {
    let opts = parse_server_status(&args(&[
        "--service-file",
        "/etc/systemd/system/codegpt-canary.service",
    ]))
    .unwrap();
    assert_eq!(
        opts.service_file,
        std::path::PathBuf::from("/etc/systemd/system/codegpt-canary.service")
    );
}
...
async fn server_status_parses_env_token_posts_and_does_not_print_token() {
    ...
    let handle = thread::spawn(move || {
        ...
        let body = r#"{"success":true,"output":{"service":"codegpt","auth_enabled":true,"configured_public_url":"https://example.test","tools":{"count":12},"agents":{"online_count":2}}}"#;
        write!(
            stream,
    ...
    std::fs::write(&env_file, format!("CODEGPT_TOKEN={}\n", token)).unwrap();
    let opts = parse_server_status(&args(&[
        "--url",
        &format!("http://{}", addr),
        "--no-system-proxy",
