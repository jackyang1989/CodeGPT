...
fn runner_init_token_file_and_env_fallback() {
    ...
    let _env = EnvGuard::new().set("CODEGPT_AGENT_TOKEN", "agent_fake_env_token");
    let opts = parse_cli_runner_init(&args(&[
        "--server-url",
        "https://v4.example.test",
        "--client-id",
...
/// Windows uses `USERPROFILE` instead (see `codegpt_runner_config::paths`).
#[cfg(unix)]
#[test]
...
fn server_init_parse_defaults() {
    ...
    if is_effective_root() {
        assert_eq!(opts.data_dir, PathBuf::from("/var/lib/codegpt"));
        assert_eq!(opts.env_file, PathBuf::from("/etc/codegpt/codegpt.env"));
    } else {
        assert!(opts.data_dir.ends_with(".local/share/codegpt"));
        assert!(opts.env_file.ends_with(".config/codegpt/codegpt.env"));
    }
...
fn server_init_writes_env_file_and_0600_permissions() {
    let tmp = tempfile::tempdir().unwrap();
    let env_file = tmp.path().join("etc/codegpt.env");
    ...
    assert!(data_dir.is_dir(), "server init must create CODEGPT_DATA");
    assert!(output.contains("CodeGPT Server configured."), "{output}");
    assert!(output.contains("Data:"), "{output}");
    assert!(output.contains("Next:"), "{output}");
    let foreground = crate::codegpt_cli::shell_command(&[
        "codegpt".to_string(),
        "server".to_string(),
        "run".to_string(),
        "--env-file".to_string(),
    ...
    if cfg!(target_os = "linux") && is_effective_root() {
        let install = crate::codegpt_cli::shell_command(&[
            "codegpt".to_string(),
            "server".to_string(),
            "install".to_string(),
            "--env-file".to_string(),
        ...
        assert!(!output.contains("codegpt server install"), "{output}");
    }
    ...
    assert!(content.contains("CODEGPT_OAUTH2_SHARED_KEY_BRIDGE=true\n"));
    assert!(content.contains("CODEGPT_SHARED_KEY_ENABLED=true\n"));
    let token = parse_env_content_value(&content, "CODEGPT_TOKEN").unwrap();
...
fn server_init_refuses_overwrite_unless_requested() {
    ...
    std::fs::write(&env_file, "CODEGPT_TOKEN=old\n").unwrap();
    ...
    let content = std::fs::read_to_string(&env_file).unwrap();
    assert!(content.contains("CODEGPT_ADDR="));
    assert!(content.contains("CODEGPT_TOKEN=old"));
    assert!(content.contains("CODEGPT_SHARED_KEY_ENABLED=true"));
    assert!(!content.contains("CODEGPT_OAUTH2_ENABLED=true"));
    assert!(!content.contains("CODEGPT_OAUTH2_SHARED_KEY_BRIDGE=true"));
}
...
fn server_init_secret_env_file_has_protected_windows_dacl() {
    let tmp = tempfile::tempdir().unwrap();
    let env_file = tmp.path().join("config/codegpt.env");
    let data_dir = tmp.path().join("data");
    let opts = parse_server_init(&args(&[
        "--listen",
        "127.0.0.1:0",
    ...
    let sddl = crate::codegpt_cli::system::windows_dacl_sddl(&env_file).unwrap();
...
fn server_init_rejects_legacy_full_token_stdout_mode() {
    ...
    assert!(!server_init_usage().contains("full CODEGPT_TOKEN"));
}
