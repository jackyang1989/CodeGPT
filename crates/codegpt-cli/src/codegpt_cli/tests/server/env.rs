...
fn users_create_builds_admin_request_via_admin_cli() {
    // codegpt users create ... reuses admin_cli parsing.
...
// `CODEGPT_ACCOUNT_CREDENTIAL` is the tested product behavior: it must stay
...
async fn default_server_http_uses_ambient_http_proxy() {
    ...
    let value = crate::codegpt_cli::post_json_unauthed(
...
async fn authenticated_explicit_proxy_request_redacts_bearer_from_error() {
    ...
    let error = crate::codegpt_cli::post_json_authed(crate::codegpt_cli::ApiCall {
