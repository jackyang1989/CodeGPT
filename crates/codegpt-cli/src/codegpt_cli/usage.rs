pub(crate) fn usage() -> &'static str {
    "Usage: codegpt [COMMAND]\n\n\
Unified command-line interface for CodeGPT.\n\n\
Quick trial:\n\
...
Daily self-hosted setup:\n\
  ...
  See `codegpt server --help` and `codegpt runner --help` for full lifecycle commands.\n\n\
Existing Server:\n\
...
pub(crate) fn connect_usage() -> &'static str {
    "Usage: codegpt connect <SERVER_URL> [OPTIONS]\n\n\
Connect a local project to a hosted CodeGPT Server. Shared-key auth is the default;\n\
...
after the matching Runner group is connected. Enter the shared key only on CodeGPT's\n\
...
pub(crate) fn disconnect_usage() -> &'static str {
    "Usage: codegpt disconnect [OPTIONS]\n\n\
Disconnect exactly one local repository registered by hosted `codegpt connect`.\n\
...
pub(crate) fn project_register_usage() -> &'static str {
    "Usage: codegpt project register --config PATH <PROJECT> [OPTIONS]\n\n\
...
pub(crate) fn project_activate_usage() -> &'static str {
    "Usage: codegpt project activate --config PATH --user-token-file PATH <PROJECT> [OPTIONS]\n\n\
...
pub(crate) fn ops_usage() -> &'static str {
    "Usage: codegpt ops <COMMAND>\n\n\
     Read-only operator workflow checks for CodeGPT.\n\n\
     Commands:\n\
     ...
     Common flags:\n\
       --server-url URL        CodeGPT server URL [default: http://127.0.0.1:8080]\n\
       --proxy http://HOST:PORT Explicit proxy override for Server requests\n\
       --no-system-proxy       Ignore proxy environment and connect directly\n\
       --env-file PATH         Read user/API credential from CODEGPT_TOKEN, then CODEGPT_PAT\n\
     ...
     CODEGPT_TOKEN then CODEGPT_PAT, process CODEGPT_TOKEN then CODEGPT_PAT.\n\n\
...
pub(crate) fn ops_status_usage() -> &'static str {
    "Usage: codegpt ops status [OPTIONS]\n\n\
...
pub(crate) fn ops_runners_usage() -> &'static str {
    "Usage: codegpt ops runners [OPTIONS]\n\n\
...
pub(crate) fn ops_runner_usage() -> &'static str {
    "Usage: codegpt ops runner --client-id CLIENT_ID [OPTIONS]\n\n\
...
pub(crate) fn ops_projects_usage() -> &'static str {
    "Usage: codegpt ops projects [OPTIONS]\n\n\
...
pub(crate) fn ops_windows_usage() -> &'static str {
    "Usage: codegpt ops windows --project PROJECT_ID [OPTIONS]\n\n\
...
pub(crate) fn ops_smoke_preflight_usage() -> &'static str {
    "Usage: codegpt ops smoke-preflight --project PROJECT_ID [OPTIONS]\n\n\
...
Use `codegpt plugin <COMMAND> --help` for command-specific options.\n\
...
pub(crate) fn plugin_init_usage() -> &'static str {
    "Usage: codegpt plugin init <DIRECTORY> [--id PROVIDER_ID]\n\n\
Create a deterministic local TypeScript/ESM Native Tool Plugin project. The generated project\n\
pins @yyjeqhc/codegpt-plugin-sdk exactly to 0.1.0 and builds independently of a CodeGPT\n\
...
fn plugin_common_usage() -> &'static str {
  ...
  -h, --help                Print help and exit\n\n\
Credential precedence: --token, --token-file, selected --env-file CODEGPT_TOKEN then\n\
CODEGPT_PAT, process CODEGPT_TOKEN then CODEGPT_PAT. CODEGPT_PAT is only an additive\n\
user/API CLI alias; existing CODEGPT_TOKEN behavior remains preferred. For repeat Plugin\n\
authoring prefer --token-file /path/to/plugin-authoring-pat.\n"
}

pub(crate) fn plugin_list_usage() -> String {
    format!(
        "Usage: codegpt plugin list [--runner RUNNER [--plugin PLUGIN]] [OPTIONS]\n\n\
...
pub(crate) fn plugin_describe_usage() -> String {
    format!(
        "Usage: codegpt plugin describe --runner RUNNER --plugin PLUGIN --tool TOOL [OPTIONS]\n\n\
...
pub(crate) fn plugin_check_usage() -> String {
    format!(
        "Usage: codegpt plugin check --runner RUNNER --plugin PLUGIN [OPTIONS]\n\n\
...
pub(crate) fn plugin_reload_usage() -> String {
    format!(
        "Usage: codegpt plugin reload --runner RUNNER [OPTIONS]\n\n\
...
pub(crate) fn server_usage() -> &'static str {
    "Usage: codegpt server <COMMAND>\n\n\
The Server is the first part of the full daily CodeGPT setup.\n\n\
Commands:\n\
  init        Initialize or update Server configuration\n\
  install     Install, enable, and start the Linux systemd socket/service pair\n\
  run         Run codegpt-server directly in the foreground\n\
...
Windows supports `server init`, foreground `server run`, and `server tunnel`; CodeGPT-managed Windows Server services are not supported yet.\n\
...
pub(crate) fn server_tunnel_usage() -> &'static str {
    "Usage: codegpt server tunnel --provider openai --env-file PATH --json --stop-on-stdin-eof\n\n\
Run the canonical OpenAI Secure Tunnel for an already-running local CodeGPT Server.\n\n\
Options:\n\
...
pub(crate) fn server_init_usage() -> &'static str {
    "Usage: codegpt server init [OPTIONS]\n\n\
Options:\n\
...
pub(crate) fn server_install_service_usage() -> &'static str {
    "Usage: codegpt server install [OPTIONS]\n\n\
Options:\n\
  --env-file PATH             EnvironmentFile= path\n\
  --bin PATH                  codegpt-server path; sibling then absolute PATH by default\n\
  --service-file PATH         Service unit path; sibling .socket is derived [default: /etc/systemd/system/codegpt.service]\n\
...
pub(crate) fn server_status_usage() -> &'static str {
    "Usage: codegpt server status [OPTIONS]\n\n\
     Options:\n\
       --url URL              Runtime URL [default: http://127.0.0.1:8080]\n\
       --proxy http://HOST:PORT Explicit proxy override for Server requests\n\
       --no-system-proxy      Ignore proxy environment and connect directly\n\
       --env-file PATH        Read CODEGPT_TOKEN from env file [default: root /etc/codegpt/codegpt.env; user ~/.config/codegpt/codegpt.env]\n\
       --token-file PATH      Read bearer token from file\n\
       --service-file PATH    Managed service unit path; sibling .socket is derived [default: /etc/systemd/system/codegpt.service]\n\
       --json                 Print a machine-readable summary\n\
       -h, --help             Print help and exit\n\n\
     Token priority: --token-file, CODEGPT_TOKEN from --env-file, process\n\
     CODEGPT_TOKEN, then no token for auth-disabled servers.\n"
}

pub(crate) fn runner_usage() -> &'static str {
    "Usage: codegpt runner <COMMAND>\n\n\
...
Commands:\n\
  ...
  run         Run codegpt-runner directly in the foreground (all supported platforms)\n\
...
`codegpt run` is the current-project runtime coordinator. `codegpt runner run` directly executes the standalone Runner.\n"
}
pub(crate) fn runner_init_usage() -> &'static str {
    "Usage: codegpt runner init --server-url URL [--token TOKEN|--token-file PATH] --client-id ID --owner USER [OPTIONS]\n\n\
     Options:\n\
       --server-url URL           CodeGPT server URL\n\
     ...
     /etc/codegpt/clients/<profile> for root or\n\
     ~/.config/codegpt/clients/<profile> for non-root users. Explicit path\n\
     flags override profile-derived defaults.\n"
}

pub(crate) fn runner_install_service_usage() -> &'static str {
    "Usage: codegpt runner install [--profile NAME] [--config PATH] [OPTIONS]\n\n\
Options:\n\
  ...
  --bin PATH                 codegpt-runner path; sibling then absolute PATH by default\n\
  --service-file PATH        Unit path [default: codegpt-runner[-<profile>].service]\n\
...
The unit runs codegpt-runner --config <config>. Tokens are never inlined.\n"
}
pub(crate) fn runner_status_usage() -> &'static str {
    "Usage: codegpt runner status [OPTIONS]\n\n\
     Options:\n\
       ...
       --json                     Print a machine-readable summary\n\
       -h, --help                 Print help and exit\n\n\
     User scope derives config under $XDG_CONFIG_HOME/codegpt (or\n\
     $HOME/.config/codegpt) and units under $XDG_CONFIG_HOME/systemd/user\n\
     (or $HOME/.config/systemd/user). System scope uses /etc/codegpt and\n\
...
pub(crate) fn login_usage() -> &'static str {
    "Usage: codegpt login <SERVER-URL> (--code <PAIRING-CODE>|--code-stdin) [OPTIONS]\n\n\
     Use a one-time login code (`codegpt pairing create`) to connect this project machine.\n\
     ...
     \x20\x20--dir PATH           Where connections are stored [default: root /etc/codegpt;\n\
     \x20\x20                       non-root ~/.config/codegpt]\n\
...
pub(crate) fn logout_usage() -> &'static str {
    "Usage: codegpt logout <SERVER-URL> [OPTIONS]\n\n\
     ...
     \x20\x20--dir PATH     Where connections are stored [default: ~/.config/codegpt]\n\
...
pub(crate) fn status_usage() -> &'static str {
    "Usage: codegpt auth status [OPTIONS]\n\n\
