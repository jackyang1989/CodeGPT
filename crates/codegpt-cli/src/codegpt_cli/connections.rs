...
//! also drop a `codegpt-runner-token` file, which left two copies that could
...
/// A server URL reduced to the exact identity CodeGPT uses for it.
...
///
/// Anything the CodeGPT server URL has no meaning for is rejected rather than
...
pub(crate) fn descriptor_toml(
    ...
    format!(
        "# Written by `codegpt login`. The directory name is only an index;\n\
...
fn read_connection(dir: PathBuf) -> Option<Connection> {
    ...
    paths.project_registry_dir =
        match codegpt_runner_config::paths::select_project_registry_dir(&paths.dir) {
            Ok(path) => path,
            Err(error) => {
                eprintln!(
...
mod tests {
    ...
    fn real_windows_directory_tree_accepts_dos_short_name_alias() {
        ...
        let long = temp
            .path()
            .join("codegpt-long-directory-name-for-short-alias");
