...
fn authorize_canonical_project(
    ...
    let effective_roots = codegpt_runner_config::effective_allowed_roots(configured_roots, true)?;
    let canonical_roots =
        codegpt_runner_config::paths::canonicalize_usable_allowed_roots(&effective_roots);

    match codegpt_runner_config::paths::validate_project_path_policy(
        canonical_project,
        &canonical_roots,
        allow_cwd_anywhere,
    ) {
        ...
        Err(_) => {
            ...
            codegpt_runner_config::paths::validate_project_path_policy(
                canonical_project,
                &canonical_roots,
                allow_cwd_anywhere,
            )?;
...
fn prepare_project_authority(
    ...
    codegpt_runner_config::paths::validate_project_path_ingress(project)?;
    if project
        .components()
...
fn prepare_exact_project_authority(
    ...
    let canonical_project = canonical_existing_directory(project, "project path")?;
    let canonical_roots =
        codegpt_runner_config::paths::canonicalize_usable_allowed_roots(configured_roots);
    let exact_root_present = canonical_roots
        .iter()
        .any(|root| codegpt_runner_config::paths::paths_equal(root, &canonical_project));
    let mut allowed_roots = configured_roots.to_vec();
    if !exact_root_present {
        allowed_roots.push(canonical_project.clone());
    }
    let canonical_after =
        codegpt_runner_config::paths::canonicalize_usable_allowed_roots(&allowed_roots);
    codegpt_runner_config::paths::validate_project_path_policy(
        &canonical_project,
        &canonical_after,
        allow_cwd_anywhere,
...
fn registration_project_registry_dir(config: &RegistrationRunnerConfig) -> Result<PathBuf, String> {
    match (
        ...
        (None, None) => {
            let base = codegpt_runner_config::paths::default_client_config_base_dir()?;
            codegpt_runner_config::paths::select_project_registry_dir(&base)
        }
    }
}
...
async fn reconcile_project_from_inventory(
    ...
    for project in projects {
        ...
        if codegpt_runner_config::paths::paths_equal(candidate, canonical_project)
            || candidate
                .canonicalize()
                .ok()
                .as_ref()
                .is_some_and(|resolved| {
                    codegpt_runner_config::paths::paths_equal(resolved, canonical_project)
                })
        {
...
pub(crate) fn run_project_register(opts: ProjectRegisterOptions) -> Result<String, String> {
    ...
    let restart_guidance = if cfg!(target_os = "linux") {
        format!(
            "Next:\n  If the Runner is in the foreground, stop it with Ctrl-C, then run:\n    {runner_command}\n  If it is installed as a service, use the matching `codegpt runner restart` command instead.\n"
        )
    } else {
        format!(
...
mod tests {
    use super::*;
    use crate::codegpt_cli::test_support::canonical_test_tempdir;
    ...
    fn human_registration_output_prioritizes_project_and_reload_action() {
        ...
        assert!(first.contains("codegpt runner run --config"), "{first}");
        assert_eq!(
    ...
    fn omitted_project_registry_dir_uses_the_same_default_as_runner_config_loading() {
        let _guard = crate::codegpt_cli::test_support::env_test_guard();
        let config = RegistrationRunnerConfig {
            project_registry_dir: None,
            legacy_projects_dir: None,
            policy: RegistrationPolicy::default(),
        };
        let base = codegpt_runner_config::paths::default_client_config_base_dir().unwrap();
        let expected = codegpt_runner_config::paths::select_project_registry_dir(&base).unwrap();
        assert_eq!(
    ...
    fn explicit_selection_supplies_authority_without_home() {
        let tmp = canonical_test_tempdir();
        let _lock = crate::codegpt_cli::test_support::env_test_guard();
        let _env = crate::codegpt_cli::test_support::EnvGuard::new()
            .remove("HOME")
            .remove("USERPROFILE")
            .remove("HOMEDRIVE")
            .remove("HOMEPATH");
    ...
    fn raw_unc_and_verbatim_unc_proceed_to_project_canonicalization() {
        ...
        let registry = tmp.path().join("registry");
        for network in [
            PathBuf::from(r"\\server\share\codegpt-unreachable-repo"),
            PathBuf::from(r"\\?\UNC\server\share\codegpt-unreachable-repo"),
        ] {
            let error =
    ...
    fn user_selected_network_project_authorizes_only_the_exact_canonical_root() {
        ...
        assert!(roots
            .iter()
            .any(|root| codegpt_runner_config::paths::paths_equal(root, &network)));
        assert!(!roots.iter().any(|root| {
            codegpt_runner_config::paths::paths_equal(root, Path::new(r"\\nas\work"))
                || codegpt_runner_config::paths::paths_equal(root, Path::new(r"\\nas\share"))
        }));
    }
    ...
    fn persisted_network_authority_keeps_the_exact_root_in_runner_config() {
        ...
        assert!(roots.iter().any(|value| {
            value.as_str().is_some_and(|root| {
                codegpt_runner_config::paths::paths_equal(Path::new(root), &network)
            })
        }));
    }
    ...
    fn raw_local_disk_path_proceeds_to_project_canonicalization() {
        let tmp = canonical_test_tempdir();
        let registry = tmp.path().join("registry");
        let missing = PathBuf::from(r"C:\codegpt-definitely-missing-p2-regression\repo");
