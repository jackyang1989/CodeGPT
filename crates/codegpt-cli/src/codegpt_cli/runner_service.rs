...
pub(crate) fn render_runner_systemd_unit(
    ...
    unit.push_str("[Unit]\n");
    unit.push_str("Description=CodeGPT Runner\n");
        ...
        if local_runner_profile_marker(&local.state_dir).is_file() {
            return match &opts.kind {
                ...
                ServiceActionKind::Uninstall { .. } => Err(
                    "hosted connect profiles are not system services; use `codegpt runner stop --profile <name>` and remove local profile files explicitly if desired"
                        .to_string(),
                ),
            };
        }
    ...
    if client_online == Some(true) {
        match (loaded_project_count, configured_project_count) {
            (Some(loaded), Some(configured)) if loaded == configured && loaded > 0 => {
                out.push_str("\nCodeGPT is ready to use registered projects.\n");
            }
            (Some(0), Some(0)) => {
            ...
            (Some(_), Some(_)) => {
                ...
                out.push_str("\nNext:\n  Restart the existing Runner, then check `codegpt runner status` again.\n");
            }
            (_, Some(0)) => {
...
mod tests {
    ...
    fn human_readiness_requires_registered_project_and_observed_connection() {
        let config = Path::new("/tmp/codegpt/runner.toml");
        ...
        assert!(
            ready.contains("CodeGPT is ready to use registered projects."),
            "{ready}"
        );
        ...
        assert!(
            zero.contains(
                "codegpt project register --config /tmp/codegpt/runner.toml /path/to/project"
            ),
            "{zero}"
        );
    ...
    fn service_unit_name_tracks_the_selected_profile_unit() {
        assert_eq!(
            service_unit_name(
                Path::new("/etc/systemd/system/codegpt-runner-workstation.service"),
                RUNNER_SERVICE_UNIT,
            ),
            "codegpt-runner-workstation.service"
        );
        assert_eq!(
            service_unit_name(
                Path::new("/etc/systemd/system/codegpt-runner.service"),
                RUNNER_SERVICE_UNIT,
            ),
            RUNNER_SERVICE_UNIT
        );
