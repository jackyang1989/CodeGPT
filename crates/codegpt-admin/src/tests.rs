...
fn admin_usage_keeps_rest_registration_commands_but_not_create_local() {
    ...
    assert!(stdout.contains("codegpt tokens create"));
    assert!(stdout.contains("codegpt tokens register-hash"));
    assert!(stdout.contains("codegpt runner-tokens create"));
    assert!(stdout.contains("codegpt runner-tokens register-hash"));
    assert!(!stdout.contains("codegpt agent-tokens create"));
    assert!(!stdout.contains("codegpt agent-tokens register-hash"));
    assert!(!stdout.contains("codegpt token register-hash"));
    assert!(!stdout.contains("codegpt agent-token register-hash"));
}
...
fn env_token_fallback_is_used() {
    let mut env = EnvGuard::new();
    env.set("CODEGPT_TOKEN", "fake-env-token");
    let cmd = parse_admin_cli(&args(&[
        "users",
        "list",
        "--server-url",
...
fn explicit_token_wins_over_default_account_credential_env() {
    let mut env = EnvGuard::new();
    env.set("CODEGPT_ACCOUNT_CREDENTIAL", "fake-account-credential");
    let cmd = parse_admin_cli(&args(&[
        "tokens",
        "register-hash",
        "--server-url",
