...
pub fn usage() -> &'static str {
    "Admin commands:\n\
      codegpt users create --server-url URL [--token TOKEN|--token-file PATH] --username USER [--display-name NAME] [--role ROLE] [--issue-credential]\n\
      codegpt users list --server-url URL [--token TOKEN|--token-file PATH]\n\
      codegpt tokens create --server-url URL [--token TOKEN|--token-file PATH] --username USER [--name NAME] [--scope SCOPE...]\n\
      codegpt tokens register-hash --server-url URL --username USER --hash HASH --prefix PREFIX [--credential CRED] [--name NAME] [--scope SCOPE...]\n\
      codegpt tokens list --server-url URL [--token TOKEN|--token-file PATH] --username USER\n\
      codegpt tokens revoke --server-url URL [--token TOKEN|--token-file PATH] --username USER --token-id ID\n\
      codegpt runner-tokens create --server-url URL [--token TOKEN|--token-file PATH] --username USER --client-id ID [--name NAME] [--scope SCOPE...]\n\
      codegpt runner-tokens register-hash --server-url URL --username USER --client-id ID --hash HASH --prefix PREFIX [--credential CRED] [--name NAME] [--scope SCOPE...]\n\
      codegpt runner-tokens list --server-url URL [--token TOKEN|--token-file PATH] --username USER\n\
      codegpt runner-tokens revoke --server-url URL [--token TOKEN|--token-file PATH] --username USER --token-id ID\n\n\
    Token fallback: CODEGPT_TOKEN\n\
...
fn resolve_bearer_token(opts: &AdminOptions, prefer_credential: bool) -> Result<String, String> {
    if opts.token.is_some() || opts.token_file.is_some() || opts.token_env.is_some() {
        return resolve_token(opts, "CODEGPT_TOKEN");
    }
    ...
    resolve_token(opts, "CODEGPT_TOKEN")
}
...
fn resolve_credential_token(opts: &AdminOptions) -> Result<Option<String>, String> {
    ...
    match std::env::var("CODEGPT_ACCOUNT_CREDENTIAL") {
