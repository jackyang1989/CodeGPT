...
pub(super) fn validate_id(
    ...
    if codegpt_core::compact::decode::<12>(suffix).is_none() {
        return Err(CommunicationStoreError::new(
            code,
...
pub(super) fn allocate_identity(
    ...
    allocate_identity_with(conn, exists_query, || {
        format!("{prefix}{}", codegpt_core::compact::random_suffix::<12>())
    })
}
...
pub(super) fn new_proof(prefix: &str) -> String {
    format!("{prefix}{}", codegpt_core::compact::random_suffix::<16>())
}
...
fn communication_request_hash(value: &Value) -> String {
    digest_json("codegpt.communication.request.v1", value)
...
mod lifecycle_contract_tests {
    ...
    fn streaming_json_digest_matches_buffered_text_digest() {
        let domain = "codegpt.compatibility.test.v1";
        let value = json!({
            "escaped": "line\n\"quoted\"\\slash",
            "unicode": "你好 🦀 日本語",
