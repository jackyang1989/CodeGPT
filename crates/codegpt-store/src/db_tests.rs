...
fn api_key_records_round_trip_and_revoked_keys_are_ignored() {
    let tmp = tempfile::tempdir().unwrap();
    let db = Database::open(&tmp.path().join("codegpt.db")).unwrap();
    let user = UserRecord {
        id: "user-1".to_string(),
        username: "alice".to_string(),
        created_at: 10,
...
fn oauth_fixture_hash(value: &str) -> String {
    let mut hasher = Sha256::new();
...
fn oauth_seed_client(db: &Database, user: &UserRecord, name: &str) -> (OAuthClientRecord, String) {
    ...
    let record = OAuthClientRecord {
        ...
        client_id,
        client_secret_hash: oauth_fixture_hash(&plaintext_secret),
        name: name.to_string(),
...
fn can_insert_and_get_authorization_code_by_hash() {
    ...
    let plaintext_code = oauth_fixture_plaintext("authorization_code");
...
fn can_insert_and_get_access_token_by_hash() {
    ...
    let plaintext_token = oauth_fixture_plaintext("access_token");
...
fn can_insert_and_get_refresh_token_by_hash() {
    ...
    let plaintext_token = oauth_fixture_plaintext("refresh_token");
    let token_hash = oauth_fixture_hash(&plaintext_token);
    let now = chrono::Utc::now().timestamp();
...
fn oauth_plaintext_tokens_are_never_stored() {
    ...
    assert_eq!(stored_secret_hash, oauth_fixture_hash(&plaintext_secret));
    drop(conn);
