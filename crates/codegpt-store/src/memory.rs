...
pub use codegpt_core::memory_contract::{
...
fn validate_memory_id(value: &str) -> Result<(), MemoryStoreError> {
    if value
        ...
        .and_then(codegpt_core::compact::decode::<12>)
        .is_some()
    {
        Ok(())
    } else {
...
fn memory_definition_digest(
    ...
    hasher.update(b"codegpt-project-memory-definition-v1\0");
...
pub fn memory_state_revision(
    ...
    let mut hasher = Sha256::new();
    hasher.update(b"codegpt-project-memory-state-v2\0");
...
pub fn memory_catalog_revision(records: &[ProjectMemoryRecord]) -> String {
    ...
    pairs.sort_unstable();
    let mut hasher = Sha256::new();
    hasher.update(b"codegpt-project-memory-catalog-v1\0");
