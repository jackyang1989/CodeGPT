...
use codegpt_core::runner_operation::{RunnerFileOperation, RunnerJobOperation, RunnerOperation};
...
impl RunnerRegistry {
    ...
    async fn complete_checked(
        ...
        let (stdout, server_stdout_truncated) = if pending.operation.is_large_native_image_request()
        {
            retain_result_stream_to_with_evidence(
                raw_stdout.clone(),
                codegpt_core::artifact_policy::MAX_MCP_IMAGE_RESPONSE_BYTES,
            )
        } else {
...
fn normalize_persistent_shell_result(
    result: &mut codegpt_core::runner_protocol::PersistentShellResult,
) -> Result<(), String> {
