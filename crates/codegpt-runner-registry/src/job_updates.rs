...
pub struct ShellJobLogObservation {
    pub wait: JobLogWait,
    pub log_delta_status: codegpt_core::job_observation::JobLogDeltaStatus,
...
impl Default for ShellJobLogObservation {
    fn default() -> Self {
        Self {
            ...
            log_delta_status: codegpt_core::job_observation::JobLogDeltaStatus::Baseline,
...
fn frozen_shell_job_log_projection(
    job: &ShellJobRecord,
    after: Option<&codegpt_core::job_observation::JobObservationToken>,
    ...
    if explicit_paging {
        ...
        view.observation_token = codegpt_core::job_observation::JobObservationToken::new_baseline(
            job.job_id.clone(),
    ...
    let base_mode = match after {
        None => codegpt_core::job_observation::JobLogSelectionMode::Baseline,
        Some(token) if token.requires_baseline() || !epoch_matches => {
            codegpt_core::job_observation::JobLogSelectionMode::Reset
        }
        Some(token) => codegpt_core::job_observation::JobLogSelectionMode::Delta {
            cursor: token
                .stdout_cursor
    ...
    let stdout = codegpt_core::job_observation::project_log_stream(
        &job.stdout.tail,
    ...
    let stderr = codegpt_core::job_observation::project_log_stream(
        &job.stderr.tail,
    ...
    let mut view = job_view(job);
    view.observation_token = codegpt_core::job_observation::JobObservationToken::new(
        job.job_id.clone(),
        job.observation.epoch.to_string(),
        job.observation.revision.load(Ordering::Relaxed),
    ...
    let observation = ShellJobLogObservation {
        wait,
        log_delta_status: codegpt_core::job_observation::combined_delta_status(
...
struct JobPublicMutationSignature {
    ...
    validation_progress: Option<codegpt_core::runner_protocol::ShellJobValidationProgress>,
    test_count_evidence: Option<codegpt_core::runner_protocol::ShellJobTestCountEvidence>,
...
fn detached_job_id_for_key(
    ...
    hasher.update(b"codegpt-detached-initiation-v1\0");
...
impl RunnerRegistry {
    ...
    pub async fn start_job_with_metadata_for_access(
        ...
        if validation_steps
            .iter()
            .any(codegpt_core::runner_protocol::ShellJobValidationStep::is_structured_go_test_json)
            && !runner
                .runner_features
    ...
    > {
        ...
        let after = after_observation_token
            .map(|value| {
                codegpt_core::job_observation::JobObservationToken::parse(value)
