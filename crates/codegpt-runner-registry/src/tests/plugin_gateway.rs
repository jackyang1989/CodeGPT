...
async fn typed_operation_decode_does_not_grant_runner_capability() {
    ...
    let wire = codegpt_core::runner_protocol::RunnerRequest::from_operation(
        codegpt_core::runner_operation::RunnerInvocationMetadata {
            request_id: "typed-operation-capability-proof".to_string(),
            client_id: "no-plugin-runner".to_string(),
            requested_by: "test".to_string(),
            created_at: 0,
        },
        codegpt_core::runner_operation::RunnerOperation::PluginGateway(operation.clone()),
    )
    .unwrap();
    assert!(matches!(
        wire.decode_operation().unwrap(),
        codegpt_core::runner_operation::RunnerOperation::PluginGateway(_)
    ));
