...
use codegpt_core::lsp_bridge::{RunnerLspPayload, RunnerLspRequest};
use codegpt_core::mcp_gateway::{
...
impl RunnerRegistry {
    ...
    pub async fn enqueue_skill_resource_execution(
        ...
        if !(codegpt_core::runner_protocol::STRUCTURED_EXECUTION_TIMEOUT_MIN_SECS
            ..=codegpt_core::runner_protocol::STRUCTURED_EXECUTION_TIMEOUT_MAX_SECS)
        ...
        if normalized_cwd.as_deref().is_some_and(|cwd| {
            cwd.len() > codegpt_core::runner_protocol::PROCESS_CWD_MAX_BYTES || cwd.contains('\0')
        }) {
            ...
            .supports(RunnerFeature::SkillResourceExecution)
        {
            return Err(format!(
                ...
                codegpt_core::runner_protocol::RUNNER_CAPABILITY_SKILL_RESOURCE_EXECUTION
            ));
        }
        enqueue_pending_request_locked(
            self.telemetry.as_ref(),
    ...
    pub async fn enqueue_script(
                ...
                .supports(RunnerFeature::StructuredScriptTypescript)
        {
            return Err(format!(
                "capability_unavailable: runner {client_id} does not support {}",
                codegpt_core::runner_protocol::RUNNER_CAPABILITY_STRUCTURED_SCRIPT_TYPESCRIPT
            ));
        }
        enqueue_pending_request_locked(
            self.telemetry.as_ref(),
    ...
    /// Enqueue one CodeGPT-generated POSIX program through the dedicated
    ...
    pub async fn enqueue_internal_posix_script(
        ...
        codegpt_core::runner_protocol::validate_raw_shell_wire_command(&script)?;
        let script = ShellScriptPayload {
            language: codegpt_core::runner_protocol::ShellScriptLanguage::Sh,
            script,
            args: Vec::new(),
        };
        validate_script_enqueue_request(
    ...
    /// process lease. The caller supplies only CodeGPT typed Run semantics; raw
