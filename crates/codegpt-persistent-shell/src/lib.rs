...
Microsoft.PowerShell.Utility\Write-Information -Tags 'CodeGPTPersistentShellCommandStatus' -MessageData ([pscustomobject]@{ Ok = $true; Native = 0 }) -InformationAction SilentlyContinue
return"#);
        assert_eq!(
...
Microsoft.PowerShell.Utility\Write-Information -Tags 'CodeGPTPersistentShellCommandStatus' -MessageData ([pscustomobject]@{ Ok = $false; Native = 123 }) -InformationAction SilentlyContinue"#);
        assert_eq!(
    ...
    fn configured_pwsh_user_command_status_is_host_authoritative() {
        let Ok(program) = std::env::var("CODEGPT_TEST_PWSH") else {
            return;
        };
        assert!(
...
$visible += @([CodeGPTPersistentShell.Controller].GetFields([Reflection.BindingFlags]'Public,NonPublic,Static') | ForEach-Object { 'CONTROLLER_FIELD|' + $_.Name + '|' + [string]$_.GetValue($null) })
