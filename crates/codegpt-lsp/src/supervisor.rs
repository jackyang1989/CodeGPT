...
impl LspCommand {
    ...
    fn spawn(
        ...
        project_root: &Path,
        kind: LspServerKind,
    ) -> Result<codegpt_process::ManagedChild, LspError> {
        ...
        command
            ...
            .stdout(Stdio::piped())
            .stderr(Stdio::piped());
        codegpt_process::ManagedChild::spawn(&mut command)
...
impl LspSupervisor {
    ...
    fn ensure_reaper_started(&self) {
        ...
        let spawned = thread::Builder::new()
            .name("codegpt-lsp-reaper".to_string())
            .spawn(move || loop {
...
fn rustup_home_dir() -> Option<PathBuf> {
    ...
    codegpt_runner_config::paths::home_dir().map(|home| home.join(".rustup"))
}
...
struct ServerInstance {
    key: ProcessKey,
    child: Mutex<codegpt_process::ManagedChild>,
...
impl ServerInstance {
    fn start(
        ...
        let reader_thread = match thread::Builder::new()
            .name("codegpt-lsp-reader".to_string())
            .spawn(move || {
                reader_loop(
                    stdout,
                    reader_writer,
        ...
        let stderr_thread = match thread::Builder::new()
            .name("codegpt-lsp-stderr".to_string())
            .spawn(move || {
    ...
    fn initialize(&self, timeout: Duration) -> Result<(), LspError> {
        ...
        // CodeGPT LSP tools are read-only semantic navigation. Starting the
...
fn cleanup_failed_lsp_child(managed: &mut codegpt_process::ManagedChild, deadline: Instant) {
