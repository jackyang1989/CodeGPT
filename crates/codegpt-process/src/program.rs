//! Cross-platform executable resolution shared by CodeGPT process consumers.
...
/// True when `name` ends in one of the extensions CodeGPT can execute
...
/// declared order, filtered to the extensions CodeGPT can actually execute:
...
mod tests {
    ...
    fn executable_lookup_matches_windows_exe_suffix_resolution() {
        let (temp, executable) = temp_dir_with("codegpt-path-probe.exe", pe_bytes());
        let path = std::env::join_paths([temp.path()]).unwrap();
        assert_eq!(
            find_executable_in_path("codegpt-path-probe", &path),
            Some(executable)
        );
    }
