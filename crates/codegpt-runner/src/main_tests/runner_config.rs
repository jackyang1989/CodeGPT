use super::*;
use crate::codegpt_runner::config::restart_required_fields;
...
fn runner_config_skill_roots_validation_fails_closed() {
    ...
    let too_long = std::env::current_dir()
        .unwrap()
        .join("x".repeat(crate::codegpt_runner::config::MAX_CONFIGURED_SKILL_ROOT_PATH_BYTES + 1));
    std::fs::write(
        &config_path,
...
fn runner_config_bounds_polling_idle_floor_but_not_unused_websocket_value() {
    ...
    for transport in [TRANSPORT_POLLING, TRANSPORT_AUTO] {
        std::fs::write(
            ...
            format!(
                "server_url = \"http://127.0.0.1:8000\"\ntoken = \"t\"\nclient_id = \"oe\"\ntransport = \"{transport}\"\npoll_interval_ms = {}\nproject_registry_dir = \"project-registry\"\n[policy]\nallow_cwd_anywhere = true\n",
                codegpt_runner_config::MAX_POLL_INTERVAL_MS + 1
            ),
        )
        .unwrap();
        let error = load_config(&path).unwrap_err();
        ...
        std::fs::write(
            ...
            format!(
                "server_url = \"http://127.0.0.1:8000\"\ntoken = \"t\"\nclient_id = \"oe\"\ntransport = \"{transport}\"\npoll_interval_ms = {}\nproject_registry_dir = \"project-registry\"\n[policy]\nallow_cwd_anywhere = true\n",
                codegpt_runner_config::MAX_POLL_INTERVAL_MS
            ),
        )
        .unwrap();
...
"#;
    ...
    // Defaults applied.
    assert_eq!(quic.alpn, "codegpt-runner/1");
...
fn runner_cli_help_and_version_exit_before_runtime() {
    ...
    match parse_runner_args(["--help"]).unwrap() {
        RunnerCliAction::Exit {
            ...
            assert_eq!(code, 0);
            assert!(stdout.contains("Usage: codegpt-runner"));
            assert!(!stdout.contains("codegpt-runner init"));
    ...
    match parse_runner_args(["--version"]).unwrap() {
        RunnerCliAction::Exit {
            ...
            assert!(stdout.starts_with(&format!(
                "codegpt-runner {} (commit ",
                env!("CARGO_PKG_VERSION")
            )));
            assert!(stdout.trim_end().ends_with(')'));
            assert_ne!(
                stdout,
                format!("codegpt-runner {}\n", env!("CARGO_PKG_VERSION"))
            );
            assert!(stderr.is_empty());
        }
...
fn runner_version_output_includes_build_metadata() {
    ...
    match parse_runner_args(["-V"]).unwrap() {
        RunnerCliAction::Exit {
            ...
            assert!(stdout.starts_with("codegpt-runner "));
            assert!(stderr.is_empty());
        }
        other => panic!("expected version exit, got {other:?}"),
    }
...
fn runner_cli_config_env_prefers_runner_name_and_keeps_legacy_alias_fail_closed() {
    ...
    let _env = EnvGuard::new()
        .set("CODEGPT_RUNNER_CONFIG", "/tmp/runner.toml")
        .remove("CODEGPT_AGENT_CONFIG");
    assert_eq!(
    ...
    let _legacy = EnvGuard::new()
        .remove("CODEGPT_RUNNER_CONFIG")
        .set("CODEGPT_AGENT_CONFIG", "/tmp/agent.toml");
    assert_eq!(
        parse_runner_args(std::iter::empty::<&str>()).unwrap(),
        RunnerCliAction::Run {
            config_path: PathBuf::from("/tmp/agent.toml"),
...
fn runner_profile_config_resolution_accepts_legacy_only_and_rejects_dual_files() {
    ...
    let _env = EnvGuard::new()
        ...
        .remove("CODEGPT_RUNNER_CONFIG")
        .remove("CODEGPT_AGENT_CONFIG");
    let profile_dir = tmp.path().join("codegpt/clients/special");
...
"#,
    ...
    assert_eq!(tmp.default_cwd.as_deref(), Some("/opt/codegpt-edge"));
    assert_eq!(
        cfg.ssh
            .resources
            .get("no_default")
...
"#,
    ...
    assert_eq!(summary.id, "codegpt");
    assert_eq!(summary.name.as_deref(), Some("codegpt"));
    assert_eq!(summary.path, "/root/git/codegpt");
