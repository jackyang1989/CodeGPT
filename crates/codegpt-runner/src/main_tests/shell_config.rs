use super::*;
#[cfg(windows)]
use crate::codegpt_runner::config::default_windows_shell_program_for_path;
...
fn shell_config_path_prepend_discovers_fake_executable() {
    ...
    let exe = bin_dir.join("codegpt-fake-tool");
...
fn shell_config_path_prepend_discovers_fake_executable_and_keeps_windows_path() {
    ...
    let exe = bin_dir.join("codegpt-fake-tool.cmd");
    std::fs::write(
        &exe,
    ...
    let prepend_index = entries
        .iter()
        .position(|entry| codegpt_runner_config::paths::paths_equal(entry, &bin_dir))
...
fn shell_config_dialect_field_parses_and_validates() {
    use crate::codegpt_runner::config::ShellDialect;
...
fn shell_config_default_environment_is_inherited() {
    ...
    let cfg = test_config(tmp.path().join("config/project-registry"));
    let cwd = tmp.path().to_string_lossy().to_string();
    let _env = EnvGuard::new().set("CODEGPT_INHERITED_TEST", "inherited-ok");
    let result = run_shell(
        &cfg.policy,
        &ShellConfig::default(),
        Some(&cwd),
        &shell_env_var("CODEGPT_INHERITED_TEST"),
        None,
        10,
        None,
    );
...
fn shell_config_env_values_are_available() {
    ...
    let cfg = test_config(tmp.path().join("config/project-registry"));
    let shell = ShellConfig {
        env: HashMap::from([("CODEGPT_TEST_VALUE".to_string(), "env-ok".to_string())]),
        ..ShellConfig::default()
    };
    let cwd = tmp.path().to_string_lossy().to_string();
    let result = run_shell(
        ...
        &shell,
        Some(&cwd),
        &shell_env_var("CODEGPT_TEST_VALUE"),
        None,
        10,
        None,
    );
...
fn shell_config_init_script_is_sourced() {
    let tmp = tempfile::tempdir().unwrap();
    let cfg = test_config(tmp.path().join("config/project-registry"));
    let init = write_init_script(tmp.path(), "CODEGPT_INIT_TEST", "init-ok");
    let shell = ShellConfig {
    ...
    let result = run_shell(
        ...
        &shell,
        Some(&cwd),
        &shell_env_var("CODEGPT_INIT_TEST"),
        None,
        10,
        None,
    );
...
fn shell_config_init_script_awkward_path_is_sourced() {
    ...
    let init = write_init_script(&init_dir, "CODEGPT_INIT_TEST", "init-ok");
    let shell = ShellConfig {
        init_script: Some(init),
        ..ShellConfig::default()
    };
