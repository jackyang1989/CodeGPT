...
fn reload_field_classification_is_exhaustive_and_allowlisted() {
    ...
    hot_only.tool_providers.strategy =
        codegpt_runner::config::ToolProviderStrategy::ClaudeCodeThenNative;
    assert!(codegpt_runner::config::restart_required_fields(&startup, &hot_only).is_empty());
    ...
    assert_eq!(
            codegpt_runner::config::restart_required_fields(&startup, &changed).join(" "),
...
fn mcp_provider_config_is_hot_reloadable_with_exact_identity_replacement() {
    ...
    let stale = runtime.mcp_gateway().handle(
        codegpt_core::mcp_gateway::McpGatewayRequest::ProviderStatus {
    ...
    assert_eq!(
        ...
        codegpt_core::mcp_gateway::McpGatewayDispatchState::NotStarted
    );
...
fn unix_sighup_listener_still_triggers_authoritative_reload() {
    ...
    let listener = codegpt_runner::install_reload_listener(Arc::clone(&runtime)).unwrap();
    std::fs::write(
        &path,
        reload_toml(
            "oe",
