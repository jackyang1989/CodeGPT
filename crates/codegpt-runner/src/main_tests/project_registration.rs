...
fn resolve_or_register_project_authorized_network_paths_reach_filesystem_resolution() {
    ...
    for network_path in [
        r"\\server\share\codegpt-unreachable-repo",
        r"\\?\UNC\server\share\codegpt-unreachable-repo",
    ] {
        let error = project_error_value(handle_resolve_or_register_project(
            &policy,
            &project_registry_dir,
...
fn model_facing_network_ingress_requires_authority_before_filesystem_resolution() {
    ...
    let raw = Path::new(r"\\untrusted-host\share\codegpt-unreachable-repo");
    let error = project_error_value(handle_resolve_or_register_project(
        &denied,
        &project_registry_dir,
        &project_request(
...
fn register_project_network_path_requires_authority_before_filesystem_resolution() {
    let tmp = tempfile::tempdir().unwrap();
    let project_registry_dir = tmp.path().join("project-registry");
    let path = r"\\server\share\codegpt-unreachable-repo";
...
fn auto_project_id_collision_extends_hash_without_overwriting() {
    ...
    #[cfg(windows)]
    let identity = codegpt_runner_config::paths::normalize_path_identity(&canonical);
    #[cfg(not(windows))]
...
fn path_registration_publish_failure_leaves_no_config_or_temp_file() {
    ...
    let project_registry_dir = tmp.path().join("project-registry");
    std::fs::create_dir(&project_dir).unwrap();
    codegpt_runner::projects::fail_next_project_publish_before_rename();
