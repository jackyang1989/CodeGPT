...
fn prepared_profile_env_is_available_to_run_shell() {
    ...
    let shell = shell_with_profiles(
        ...
        vec![(
            "test",
            ShellProfileConfig {
                env: profile_env(&[("CODEGPT_TEST_PROFILE", "from_env")]),
    ...
    let result = run_profile_shell(
        ...
        tmp.path(),
        &shell_env_var("CODEGPT_TEST_PROFILE"),
    );
...
fn prepared_profile_init_script_is_project_relative() {
    ...
    std::fs::write(
        ...
        format!(
            "{}\n",
            profile_init_export("CODEGPT_PROJECT_VENV", "project_local")
        ),
    )
    .unwrap();
    ...
    let result = run_profile_shell(
        ...
        &PreparedShellProfileCache::default(),
        &project_dir,
        &shell_env_var("CODEGPT_PROJECT_VENV"),
    );
    assert_eq!(result.exit_code, Some(0), "{result:?}");
    ...
    let shell = shell_with_profiles(
        ...
        vec![
            (
                "default",
                ShellProfileConfig {
                    env: profile_env(&[("CODEGPT_TEST_PROFILE", "default")]),
                    ..ShellProfileConfig::default()
                },
            ),
            (
                "project",
                ShellProfileConfig {
                    env: profile_env(&[("CODEGPT_TEST_PROFILE", "project")]),
                    ..ShellProfileConfig::default()
                },
            ),
        ],
...
fn prepared_profile_run_shell_and_run_job_see_same_env() {
    ...
    let shell = shell_with_profiles(
        ...
        vec![(
            "test",
            ShellProfileConfig {
                env: profile_env(&[("CODEGPT_TEST_PROFILE", "same")]),
                ..ShellProfileConfig::default()
            },
        )],
    );
    ...
    let lsp = codegpt_runner::LspSupervisor::default();
    let mut cfg = test_config(project_registry_dir.clone());
    cfg.shell = shell.clone();
    let hot = runtime_config(&cfg);
    let persistent_shells = codegpt_runner::PersistentShellManager::new(
        &cfg.shell,
        codegpt_runner::SshConnectionPool::default(),
    );
    dispatch_request(
        &sink,
        &hot.snapshot(),
        ...
        &project_registry_dir,
        &lsp,
        shell_job_request(&project_dir, &shell_env_var("CODEGPT_TEST_PROFILE")),
    )
    .unwrap();
...
fn prepared_profile_init_script_runs_once_per_project_profile_generation() {
    ...
    let init_script = format!(
        ...
        shell_tree_quote(&counter.to_string_lossy()),
        profile_init_export("CODEGPT_TEST_PROFILE", "counted"),
    );
    #[cfg(not(windows))]
    let init_script = format!(
...
fn prepared_profile_init_script_stdout_noise_does_not_break_env_capture() {
    ...
    #[cfg(windows)]
    let init_script =
        "Write-Output 'noise before env'\n$env:CODEGPT_TEST_PROFILE = 'ok'".to_string();
    #[cfg(not(windows))]
    let init_script = "echo noise before env\nexport CODEGPT_TEST_PROFILE=ok".to_string();
    let shell = shell_with_profiles(
        Some("test"),
        vec![(
            "test",
...
fn prepared_profile_prepare_reaps_background_pipe_holder() {
    ...
    let init_script = format!(
            "sleep 60 & background_pid=$!; printf '%s' \"$background_pid\" > {}; export CODEGPT_TEST_PROFILE=ready",
...
#[test]
fn prepared_profile_filters_codegpt_token_env() {
    ...
    #[cfg(windows)]
    let spellings = ["CODEGPT_TOKEN", "CodeGPT_Token", "authorization"];
    #[cfg(not(windows))]
    let spellings = ["CODEGPT_TOKEN"];
    ...
    let shell = shell_with_profiles(
        ...
        vec![(
            ...
            ShellProfileConfig {
                ...
                init_script: Some("export CODEGPT_TEST_PROFILE=ok".to_string()),
                ..ShellProfileConfig::default()
            },
        )],
    );
...
fn prepared_profile_program_spawn_failure_mentions_profile() {
    ...
    let shell = shell_with_profiles(
        ...
        vec![(
            "test",
            ShellProfileConfig {
                program: Some("/definitely/missing/codegpt-shell".to_string()),
                args: Some(vec!["-c".to_string()]),
                ..ShellProfileConfig::default()
            },
        )],
