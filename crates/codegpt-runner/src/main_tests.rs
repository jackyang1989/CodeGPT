use super::*;
use crate::codegpt_runner::config::validate_shell_config;
use crate::codegpt_runner::projects::{project_root_fingerprint, RunnerProjectFile};
use crate::codegpt_runner::run_shell_with_profiles;
use crate::codegpt_runner::{
...
fn test_config(project_registry_dir: PathBuf) -> RunnerConfig {
    RunnerConfig {
        ...
        skills: crate::codegpt_runner::config::SkillsConfig::default(),
    ...
    SHELL_TREE_HELPER
        .get_or_init(|| {
            let source = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
                .join("src/codegpt_runner/validation/validation_tree_helper.rs");
            let temp = tempfile::tempdir().unwrap();
            let output = temp
                .path()
            ...
            let result = std::process::Command::new(rustc)
                .arg("--edition=2021")
                .arg("--crate-name=codegpt_shell_tree_helper")
                .arg(&source)
                .arg("-o")
                .arg(&output)
                .output()
...
fn shell_job_unicode_stdout_stderr_env_and_cwd() {
    ...
    let _env = EnvGuard::new().set("CODEGPT_UNICODE_ENV", "值 测试");
    let result = run_shell(
        &cfg.policy,
        &ShellConfig::default(),
        Some(&cwd),
        &shell_env_var("CODEGPT_UNICODE_ENV"),
        None,
        10,
        None,
    );
...
fn prepared_profile_unicode_env_round_trip_and_unicode_init_path() {
    ...
    content.push_str("$env:CODEGPT_TEST_PROFILE = 'café 值'\n");
...
fn managed_worktree_network_source_requires_runner_authority_before_resolution() {
    ...
    let source = Path::new(r"\\untrusted-host\share\codegpt-unreachable-repo");
