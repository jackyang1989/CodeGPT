...
//! the Runner host's OpenSSH client. CodeGPT only keeps named resource
...
#[cfg(any(test, windows))]
const WINDOWS_DIRECT_BOOTSTRAP_PREFIX: &str = ": CODEGPT_SSH_PROGRAM_BYTES=";
...
fn apply_transport_failure_policy(
    ...
    if let Some(stderr) = result.result.stderr.as_mut() {
        append_bounded_line(
            stderr,
            "codegpt: SSH transport ended after dispatch; the command may have started and was not retried",
            max_output_bytes,
        );
    }
...
fn remote_script(cwd: Option<&str>, command: &str) -> String {
    match cwd {
        Some(cwd) => format!(
            "if ! cd {}; then printf >&2 '%s\\n' 'codegpt: remote cwd is unavailable'; exit 125; fi\n{}",
            shell_quote(cwd),
            command
        ),
...
fn windows_direct_program_bootstrap(frame_len: usize) -> String {
    format!(
        r#"{WINDOWS_DIRECT_BOOTSTRAP_PREFIX}{frame_len}; eval "$(set +e; CODEGPT_SSH_N={frame_len}; [ "$CODEGPT_SSH_N" -gt 0 ] 2>/dev/null && [ "$CODEGPT_SSH_N" -le {WINDOWS_DIRECT_FRAME_MAX_BYTES} ] 2>/dev/null || {{ printf >&2 '%s\n' 'codegpt: SSH program length out of range'; printf '%s' 'exit 126'; exit 0; }}; CODEGPT_SSH_FRAME=$(dd bs=1 count="$CODEGPT_SSH_N" 2>/dev/null); CODEGPT_SSH_DD_STATUS=$?; [ "$CODEGPT_SSH_DD_STATUS" -eq 0 ] || {{ printf >&2 '%s\n' 'codegpt: SSH program upload failed'; printf '%s' 'exit 126'; exit 0; }}; CODEGPT_SSH_ACTUAL=$(printf '%s' "$CODEGPT_SSH_FRAME" | wc -c); [ "$CODEGPT_SSH_ACTUAL" -eq "$CODEGPT_SSH_N" ] 2>/dev/null || {{ printf >&2 '%s\n' 'codegpt: incomplete SSH program upload'; printf '%s' 'exit 126'; exit 0; }}; printf '%s' "$CODEGPT_SSH_FRAME")""#
    )
}
...
fn run_piped_ssh_command(
    ...
    if stopped {
        append_bounded_line(
            &mut stderr,
            "codegpt: local SSH client was stopped after dispatch; remote command outcome is unknown; do not blindly retry",
            max_output_bytes,
        );
        return ShellCommandResult::outcome_unknown(CommandResult {
            exit_code: None,
...
mod tests {
    ...
    use crate::codegpt_runner::config::{RunnerPolicy, SshConfig, SshResourceConfig};
    ...
    impl TestSshServer {
        ...
        fn start() -> Option<Self> {
            ...
            let alias = "codegpt-test-ssh".to_string();
    ...
    fn shell_parser_error_without_transport_failure(stderr: &[u8]) -> bool {
        ...
        !stderr.is_empty() && !text.contains("codegpt:")
    }
    ...
    fn windows_direct_bootstrap_matches_direct_shell_at_exact_eof() {
        ...
        let cases = vec![
            ...
            "printf '%s' '#codegpt-ssh-frame'".to_string(),
            "printf x;".to_string(),
            "printf x   \t".to_string(),
        ...
        assert!(
            !bootstrap.contains("#codegpt-ssh-frame"),
    ...
    fn windows_direct_bootstrap_preserves_umask_and_loader_state() {
            ...
            [("022", 0o644, 0o755), ("027", 0o640, 0o750)]
        {
            ...
            let setup = format!(
                "umask {mask}; set -- before 'two words'; export CODEGPT_SSH_PROGRAM_BYTES=original CODEGPT_SSH_N=OLD_N CODEGPT_SSH_FRAME=OLD_TRANSPORT_FRAME CODEGPT_SSH_FRAMED=OLD_FRAME CODEGPT_SSH_DD_STATUS=OLD_STATUS CODEGPT_SSH_PROGRAM=OLD_PROGRAM CODEGPT_SSH_ACTUAL=OLD_ACTUAL n=N i=I d=D f=F actual=A; cleanup() {{ printf cleanup-original; }}"
            );
            let program = format!(
                "printf 'mask=%s|meta=%s|private=%s,%s,%s,%s,%s,%s|generic=%s,%s,%s,%s,%s|args=%s,%s,%s|' \"$(umask)\" \"$CODEGPT_SSH_PROGRAM_BYTES\" \"$CODEGPT_SSH_N\" \"$CODEGPT_SSH_FRAME\" \"$CODEGPT_SSH_FRAMED\" \"$CODEGPT_SSH_DD_STATUS\" \"$CODEGPT_SSH_PROGRAM\" \"$CODEGPT_SSH_ACTUAL\" \"$n\" \"$i\" \"$d\" \"$f\" \"$actual\" \"$#\" \"$1\" \"$2\"; cleanup; touch {}; mkdir {}",
    ...
    fn reuses_session_transport_but_not_remote_shell_state_and_reconnects() {
        ...
        assert!(
            unavailable_cwd
                .stderr
                .as_deref()
                .is_some_and(|stderr| stderr.contains("codegpt: remote cwd is unavailable")),
            "{unavailable_cwd:?}"
        );
    ...
    fn remote_async_jobs_stream_output_and_stop_through_the_existing_lifecycle() {
        ...
        let sink = crate::codegpt_runner::RunnerSink::WebSocket {
            tx,
        ...
        manager.enqueue(
            ...
            crate::PendingJobStart::from_wire(
                11,
                RunnerPolicy::default(),
                crate::codegpt_runner::ShellConfig::default(),
                config.clone(),
                PathBuf::new(),
    ...
    fn remote_persistent_shell_preserves_state_across_execs() {
        ...
        let config = ssh_config(&server);
        let pool = SshConnectionPool::with_test_config(server.client_config.clone());
        let manager = crate::codegpt_runner::PersistentShellManager::new(
            &crate::codegpt_runner::ShellConfig::default(),
            pool,
        );
...
const PROGRAM_PREFIX: &str = ": CODEGPT_SSH_PROGRAM_BYTES=";
...
"#,
                ...
                let result = Command::new(rustc)
                    .arg("--edition=2021")
                    .arg("--crate-name=codegpt_fake_ssh")
                    .arg(&source)
                    .arg("-o")
                    .arg(&output)
                    .output()
    ...
    fn ssh_job_operation(job_id: &str) -> codegpt_core::runner_operation::RunnerJobOperation {
        let request = ssh_job_request(job_id, "printf ignored", 30);
        let codegpt_core::runner_operation::RunnerOperation::Job(operation) = request
            .decode_operation()
    ...
    fn enqueue_job(
        ...
        sink: crate::codegpt_runner::RunnerSink,
        config: SshConfig,
        policy: RunnerPolicy,
        job_id: &str,
        command: &str,
    ...
    fn windows_one_shot_and_job_keep_remote_program_out_of_createprocess_argv() {
        ...
        assert!(
            !job_args.iter().any(|arg| arg.contains("/srv/codegpt")),
            "{job_args:?}"
        );
        assert_eq!(
            stdin_frame(&job.program_delivery),
            windows_direct_program_frame("if ! cd '/srv/codegpt'; then printf >&2 '%s\\n' 'codegpt: remote cwd is unavailable'; exit 125; fi\nprintf job")
        );
        assert_eq!(pool.connection_count(), 0);
    }
    ...
    fn windows_persistent_shell_prepares_direct_literal_ssh_exe_argv() {
        ...
        assert_eq!(prepared.default_cwd.as_deref(), Some("/srv/codegpt"));
    }

    #[test]
    fn windows_real_host_one_shot_and_background_are_opt_in() {
        let Ok(host) = std::env::var("CODEGPT_TEST_WINDOWS_SSH_HOST") else {
            eprintln!("skipping Windows real-host SSH integration: CODEGPT_TEST_WINDOWS_SSH_HOST is unset");
            return;
        };
        let host = host.trim();
        if host.is_empty() {
            eprintln!("skipping Windows real-host SSH integration: CODEGPT_TEST_WINDOWS_SSH_HOST is empty");
            return;
        }
        ...
        let missing_cwd = format!("/tmp/codegpt-missing-{}", uuid::Uuid::new_v4().simple());
