...
pub(super) fn permanent_result_rejection_log_line(
    ...
    format!(
        "codegpt-runner result permanently rejected request_id={} error={}; dropping this result and continuing to poll",
...
pub(super) fn dropped_result_log_line(
    ...
    format!(
        "codegpt-runner result submission retries exhausted request_id={} attempts={} error={}; dropping this result and continuing to poll",
        concise_log_error(request_id, token),
        attempts,
        concise_log_error(error, token)
    )
...
impl RunnerSink {
    ...
    /// remains Runner-local; only the bounded typed CodeGPT response crosses
    /// the Server transport.
    pub(crate) fn submit_plugin_gateway_result(
        &self,
        request_id: String,
        response: codegpt_core::plugin::PluginGatewayResponse,
    ) -> Result<ResultSubmission, SubmitResultError> {
        self.submit_result_payload(RunnerResultPayload {
            result: RunnerResultRequest {
                client_id: self.client_id().to_string(),
    ...
    pub(crate) fn submit_coding_agent_result(
        &self,
        request_id: String,
        response: codegpt_core::coding_agent::CodingAgentResponse,
    ) -> Result<ResultSubmission, SubmitResultError> {
        self.submit_result_payload(RunnerResultPayload {
            result: RunnerResultRequest {
                client_id: self.client_id().to_string(),
