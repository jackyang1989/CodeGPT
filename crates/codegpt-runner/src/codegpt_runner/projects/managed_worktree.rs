...
fn choose_managed_destination(
    ...
    hash.update(b"codegpt/managed-worktree-routing/v1\0");
...
fn choose_managed_worktree_root(
    ...
    for root in roots {
        if !codegpt_runner_config::paths::path_is_within(source, &root) {
            continue;
        }
        let candidate = root.join(".codegpt-managed-worktrees");
        if codegpt_runner_config::paths::path_is_within(&candidate, source) {
            continue;
        }
        return Ok(candidate);
    }
    if policy.allow_cwd_anywhere {
        if let Some(parent) = source.parent() {
            let candidate = parent.join(".codegpt-managed-worktrees");
            if !codegpt_runner_config::paths::path_is_within(&candidate, source) {
                return Ok(candidate);
            }
        }
