...
pub(crate) fn find_project_shell_context(
    ...
    load_runner_project_shell_contexts_from_dir(project_registry_dir)
        ...
        .filter_map(|project| {
            ...
            if codegpt_runner_config::paths::path_is_within(&cwd, &project_path) {
...
pub(crate) fn project_root_fingerprint(canonical_root: &Path) -> String {
    let identity = codegpt_runner_config::paths::normalize_path_identity(canonical_root);
    ...
    for (hook, commands) in &project.hooks {
        for (idx, command) in commands.iter().enumerate() {
            if command.trim().is_empty() {
                eprintln!(
                    "codegpt-runner project warning: {} hook {} command {} is empty",
                    source.display(),
                    hook,
                    idx
                );
...
fn load_runner_project_summaries_from_dir_with_shutdown(
    ...
    let entries = match std::fs::read_dir(dir) {
        ...
        Err(e) => {
            eprintln!(
                "codegpt-runner project warning: failed to read {}: {}",
                dir.display(),
                e
            );
            return Vec::new();
    ...
    for file in files {
        ...
        let project = match parse_runner_project_toml(&content) {
            ...
            Err(e) => {
                eprintln!(
                    "codegpt-runner project warning: skipping {}: {}",
                    file.display(),
                    e
                );
                continue;
        ...
        if !seen.insert(project.id.clone()) {
            eprintln!(
                "codegpt-runner project warning: duplicate project id {} in {}; skipping",
                project.id,
                file.display()
            );
            continue;
...
mod git_lifecycle_tests {
        ...
        GIT_TREE_HELPER
            .get_or_init(|| {
                ...
                let result = Command::new(rustc)
                    .arg("--edition=2021")
                    .arg("--crate-name=codegpt_git_tree_helper")
                    .arg(&source)
                    .arg("-o")
                    .arg(&output)
                    .output()
    ...
    fn spawn_failure_reports_spawn_error() {
        ...
        let error = match run_git_bounded_with_program(
            "codegpt-git-command-that-does-not-exist-xyz",
            cwd.path(),
            &["--version"],
