...
///
/// The shared `codegpt_runner_config::paths::validate_project_path_ingress`
...
pub(super) fn validate_windows_project_root(path: &Path) -> Result<(), &'static str> {
    if codegpt_runner_config::paths::project_path_has_parent_traversal(path) {
        return Err("path_outside_allowed_roots");
    }
    codegpt_runner_config::paths::validate_project_path_ingress(path)
...
pub(super) fn validate_model_network_project_ingress_authority(
    ...
    #[cfg(windows)]
    {
        if !codegpt_runner_config::paths::is_windows_network_share_path(path) {
            return Ok(());
        }
        ...
        if policy.allowed_roots.iter().any(|root| {
            root.is_absolute() && codegpt_runner_config::paths::path_is_within(path, root)
        }) {
            return Ok(());
        }
        ...
        let canonical_roots =
            codegpt_runner_config::paths::canonicalize_usable_allowed_roots(&policy.allowed_roots);
        if canonical_roots
            .iter()
            .any(|root| codegpt_runner_config::paths::path_is_within(path, root))
        {
            return Ok(());
        }
        return Err("path_outside_allowed_roots");
...
fn canonical_project_path_hash(canonical_path: &Path) -> String {
    ...
    format!(
        "{:x}",
        Sha256::digest(
            codegpt_runner_config::paths::normalize_path_identity(canonical_path).as_bytes()
        )
    )
}
