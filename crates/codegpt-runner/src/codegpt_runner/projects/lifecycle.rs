...
use codegpt_core::runner_operation::{RunnerProjectOperation, RunnerProjectOperationKind};
...
pub(crate) fn handle_project_operation(
    ...
    #[cfg(windows)]
    if create && codegpt_runner_config::paths::is_windows_network_share_path(Path::new(&path)) {
    ...
    #[cfg(windows)]
    if codegpt_runner_config::paths::is_windows_network_share_path(&canonical_for_policy) {
