...
use codegpt_browser::BrowserSupervisor;
...
fn install_shutdown_listener(
    ...
    std::thread::Builder::new()
        .name("codegpt-runner-shutdown".to_string())
        .spawn(move || {
...
fn install_parent_liveness_listener(runtime: RunnerRuntimeState) -> Result<(), String> {
    ...
    let listener = std::thread::Builder::new()
        .name("codegpt-runner-parent-lease".to_string())
        .spawn(move || {
...
fn send_polling_offline_best_effort(client: &Client, cfg: &RunnerConfig, runner_instance_id: &str) {
    ...
    match request.json(&body).send() {
        ...
        Ok(response) => tracing::debug!(
            ...
            "codegpt-runner polling offline notice was not accepted"
        ),
        Err(error) => tracing::debug!(
            error = %concise_log_error(&error.to_string(), &cfg.token),
            "codegpt-runner polling offline notice failed"
        ),
    }
}
...
fn registered_log_line(
    ...
    format!(
        "codegpt-runner registered client_id={} server={} preferred_transport={} actual_transport={} projects={}",
        cfg.client_id,
...
fn auto_quic_not_configured_log_line() -> &'static str {
    "codegpt-runner transport auto: quic not configured; skipping"
}

fn auto_trying_log_line(transport: &str) -> String {
    format!("codegpt-runner transport auto: {} trying", transport)
}
...
pub(crate) fn install_reload_listener(
    ...
    std::thread::Builder::new()
        .name("codegpt-runner-reload".to_string())
        .spawn(move || {
...
fn schedule_reconnect(transport: &str, backoff: &mut RetryBackoff) -> Duration {
    ...
    eprintln!(
        "codegpt-runner reconnect attempt scheduled transport={} delay={}",
        transport,
        format_delay(delay)
    );
    tracing::debug!(
        ...
        "codegpt-runner reconnect attempt scheduled"
    );
    delay
}
            ...
            match decide_stream_session(mode, transport, once, result) {
                ...
                StreamSessionDecision::Reconnect(error) => {
                    if let Some(error) = error {
                        eprintln!(
                            "codegpt-runner {} error: {}; reconnecting",
                            transport.name(),
                            error
                        );
                        tracing::debug!(
                            ...
                            error = %error,
                            "codegpt-runner stream transport transient error"
                        );
                    } else {
                        reset_backoff_after_stable_session(&mut backoff, session_started);
                        eprintln!(
                            "codegpt-runner {} connection closed; reconnecting",
                            transport.name()
                        );
                    }
                ...
                StreamSessionDecision::TryNext(error) => {
                    ...
                    match transport {
                        StreamTransport::Quic => eprintln!(
                            "codegpt-runner transport auto: quic unavailable: {}; trying websocket",
                            log_error
                        ),
                        StreamTransport::WebSocket => eprintln!(
                            "codegpt-runner transport auto: websocket failed: {}; falling back to polling",
                            log_error
                        ),
                    }
                    tracing::debug!(
                        transport = transport.name(),
                        error = %log_error,
                        "codegpt-runner auto transport attempt failed"
                    );
                }
    ...
    match error.recovery_action() {
        ...
        PollingRecoveryAction::RetryPoll => {
            *recovering = true;
            ...
            eprintln!(
                "codegpt-runner transient poll failure; retrying delay={} error={}",
                format_delay(delay),
        ...
        PollingRecoveryAction::ReRegister => {
            ...
            *session_refreshed_during_recovery = false;
            let delay = recovery_backoff.next_delay();
            eprintln!(
                "codegpt-runner polling session lost; re-registering delay={} error={}",
                format_delay(delay),
                concise_log_error(&error.to_string(), &cfg.token)
            );
            if sleep_or_shutdown(delay, shutdown) {
        ...
        if !registered {
            match register(
                ...
                Ok((projects_count, registered_jobs, registered_projects, _inventory_status)) => {
                    ...
                    if recovering {
                        eprintln!(
                            "codegpt-runner polling session refreshed during recovery client_id={}",
                ...
                Err(error) => match error.recovery_action() {
                    ...
                    RegisterRecoveryAction::Retry => {
                        ...
                        let delay = recovery_backoff.next_delay();
                        eprintln!(
                            "codegpt-runner transient register failure; retrying delay={} error={}",
                            format_delay(delay),
                            concise_log_error(&error.to_string(), &cfg.token)
                        );
                    ...
                    RegisterRecoveryAction::WaitForLease => {
                        ...
                        eprintln!(
                            "codegpt-runner active-instance lease conflict; waiting client_id={} delay={}",
                            concise_log_error(&cfg.client_id, &cfg.token),
                            format_delay(delay)
                        );
                        if sleep_or_shutdown(delay, shutdown.as_ref()) {
        ...
        match poll_result {
            Ok((ran_request, inventory_status)) => {
                ...
                if recovering {
                    idle_backoff.reset();
                    eprintln!(
                        "codegpt-runner polling recovery succeeded phase=poll client_id={}",
                        concise_log_error(&cfg.client_id, &cfg.token)
                    );
                    recovering = false;
                    session_refreshed_during_recovery = false;
        ...
        match self {
            Self::WebSocket { reader, .. } => loop {
                let message = match reader.next().await {
                    ...
                    Some(Err(error)) => {
                        tracing::debug!(
                            transport = "websocket",
                            error = ?error,
                            "codegpt-runner websocket read error"
                        );
                    ...
                    None => {
                        tracing::debug!(
                            transport = "websocket",
                            "codegpt-runner websocket stream ended"
                        );
                        return Ok(StreamRead::Closed);
                    }
                };
                if let tokio_tungstenite::tungstenite::Message::Close(frame) = message {
                    if let Some(frame) = frame {
                        tracing::debug!(
                            ...
                            "codegpt-runner websocket close frame received"
                        );
                    } else {
                        tracing::debug!(
                            transport = "websocket",
                ...
                match RunnerEnvelope::from_slice(text.as_bytes()) {
                    ...
                    Err(error) => {
                        eprintln!("codegpt-runner websocket malformed envelope: {}", error);
                    }
                }
            },
            Self::Quic { reader, .. } => match read_quic_frame(reader).await {
                ...
                Err(QuicFrameError::EmptyStream) => {
                    tracing::debug!(
                        transport = "quic",
                        "codegpt-runner quic stream closed by peer"
                    );
...
fn handle_stream_envelope(
    ...
    match envelope {
        ...
        other => {
            eprintln!(
                "codegpt-runner {} ignoring unexpected envelope: {}",
                transport.name(),
                other.kind()
            );
            None
    ...
    F: std::future::Future<Output = ()>,
{
        ...
        tokio::select! {
            ...
            writer = &mut writer_task => {
                ...
                tracing::debug!(
                    transport = transport.name(),
                    reason_code,
                    "codegpt-runner stream writer ended; terminating session"
                );
                break;
            }
            read = stream.receive() => {
            ...
            _ = ping_interval.tick() => {
                tracing::debug!(
                    transport = transport.name(),
                    "codegpt-runner stream keepalive ping"
                );
    ...
    if shutdown_requested {
        ...
        tracing::warn!(
            transport = transport.name(),
            "codegpt-runner stream disconnected with active jobs; reconnecting without waiting"
        );
    }
...
fn classify_quic_runner_connect_error(error: &str) -> &'static str {
        ...
        || lower.contains("unknownissuer")
    {
        ...
        "handshake failed; check CODEGPT_QUIC_ENABLED, listener bind, and ALPN"
    } else if lower.contains("applicationclosed")
        || lower.contains("connectionclosed")
        || lower.contains("closed")
    {
        "handshake failed; check CODEGPT_QUIC_ENABLED, listener bind, and server availability"
    } else {
        "handshake failed"
    }
}
