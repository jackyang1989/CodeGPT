...
fn test_runner_config(server_url: String) -> RunnerConfig {
    RunnerConfig {
        ...
        websocket_connect_timeout_secs:
            crate::codegpt_runner::default_websocket_connect_timeout_secs(),
        quic: None,
...
fn runtime_shutdown_is_fast_ordered_and_runs_once_without_resources() {
    ...
    assert!(lines[0].starts_with("codegpt-runner shutdown complete "));
}
...
fn runtime_completion_log_follows_bounded_background_cleanup() {
    ...
    assert!(
        lines
            .last()
            .unwrap()
            .starts_with("codegpt-runner shutdown complete "),
...
fn polling_persistent_shell_request(
    ...
    request.persistent_shell = Some(crate::runner_protocol::PersistentShellRequest {
        ...
        workflow_session_id: "cg_sess_lkaw44QDG0J6FfyY".to_string(),
...
fn runner_real_process_polling_persistent_shell_exec_remains_responsive_to_close() {
    ...
    let shell_id = "cg_shell_polling_e1";
...
fn auto_log_lines_are_concise_and_redacted() {
    ...
    assert_eq!(
        ...
        "codegpt-runner transport auto: websocket trying"
    );
    assert_eq!(
        auto_trying_log_line(TRANSPORT_POLLING),
        "codegpt-runner transport auto: polling trying"
    );
...
fn registered_log_includes_actual_transport_without_url_query_or_token() {
    ...
    let mut cfg = test_runner_config(format!(
        "https://codegpt.example.test/agent/path?token={}",
        token
    ));
    ...
    assert!(
        line.contains("server=https://codegpt.example.test"),
        "{line}"
    );
...
fn quic_transport_config_applies_keepalive_without_changing_application_ping() {
    let quic = QuicClientConfig {
        ...
        alpn: crate::codegpt_runner::default_quic_alpn(),
