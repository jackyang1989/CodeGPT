...
fn fake_binary() -> Arc<FakeBinary> {
    ...
    let output = temp.path().join(format!(
        "codegpt-mcp-bridge-fake{}",
        env::consts::EXE_SUFFIX
    ));
    let source =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("src/codegpt_runner/fake_mcp_gateway.rs");
    let rustc = env::var_os("RUSTC").unwrap_or_else(|| "rustc".into());
    let result = Command::new(rustc)
        .arg("--edition=2021")
        .arg("--crate-name=codegpt_mcp_gateway_fake")
        .arg(source)
        .arg("-o")
        .arg(&output)
        .output()
...
fn provider_execution_context_is_explicit_cleared_and_private() {
    ...
    let _env = crate::tests::EnvGuard::new()
        ...
        .set("CODEGPT_MCP_MAPPED_SOURCE", "mapped-provider-secret-value")
        .set("CODEGPT_MCP_UNLISTED", "must-not-reach-provider");
    ...
    let fixture = Fixture::with_execution_context(
        ...
        BTreeMap::from([
            ...
            (
                "CODEGPT_MCP_MAPPED_CHILD".to_string(),
                "CODEGPT_MCP_MAPPED_SOURCE".to_string(),
            ),
        ]),
    );
    let provider = fixture.provider();
...
fn missing_mapped_source_fails_before_provider_spawn() {
    let _guard = crate::tests::test_env_lock();
    let _env = crate::tests::EnvGuard::new().remove("CODEGPT_MCP_TEST_MISSING_SOURCE");
    let fixture = Fixture::with_execution_context(
        "normal",
        2,
        None,
        ...
        BTreeMap::from([(
            ...
            "CODEGPT_MCP_TEST_MISSING_SOURCE".to_string(),
        )]),
    );
