// Standalone newline-delimited CodeGPT Tool Plugin fixture. Runner tests
...
fn main() -> io::Result<()> {
    ...
    if scenario == "execution_context" {
        append(
            marker,
            if env::var("CODEGPT_PLUGIN_TEST_ENV").as_deref() == Ok("profile-ready") {
                "profile-env-ok\n"
            } else {
                "profile-env-bad\n"
            },
        ...
        append(
            marker,
            if env::var_os("CODEGPT_AGENT_TOKEN").is_none()
                && env::var_os("CODEGPT_PAT").is_none()
            {
