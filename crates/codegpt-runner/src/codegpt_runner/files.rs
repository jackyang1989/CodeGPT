        ...
        match std::fs::symlink_metadata(&target) {
            Ok(metadata) => {
                ...
                if !containment.as_ref().is_some_and(|candidate| {
                    codegpt_runner_config::paths::path_is_within(candidate, &canonical_root)
                }) {
                    return delete_project_files_error(
                        start,
            ...
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => {
                ...
                if !codegpt_runner_config::paths::path_is_within(&ancestor, &canonical_root) {
                    return delete_project_files_error(
                        start,
                        "delete_project_files target is outside the project",
                    );
...
fn ensure_file_read_target_in_project(
    ...
    if codegpt_core::sensitive_paths::is_secret_path(&request.path)
        || codegpt_core::sensitive_paths::is_secret_path(&relative.to_string_lossy())
    {
...
fn handle_file_read_range_request(
    ...
    let range =
        codegpt_workspace::file_read_range::EffectiveRange::new(Some(start_line), Some(limit));
...
fn handle_skill_read_file_request(
    ...
    if codegpt_core::sensitive_paths::is_secret_path(requested_project_relative)
        || codegpt_core::sensitive_paths::is_secret_path(canonical_project_relative.as_ref())
    {
