...
pub(super) fn runner_tool_trace_enabled() -> bool {
    std::env::var("CODEGPT_TOOL_REQUEST_TRACE")
        .ok()
    ...
    match request.kind.as_str() {
        ...
        "coding_agent" => sink
            .submit_coding_agent_result(
                request_id,
                codegpt_core::coding_agent::CodingAgentResponse::error(
                    codegpt_core::coding_agent::CodingAgentDispatchState::NotStarted,
        ...
        "plugin_gateway" => sink
            .submit_plugin_gateway_result(
                request_id,
                codegpt_core::plugin::PluginGatewayResponse::error(
                    codegpt_core::plugin::PluginDispatchState::NotStarted,
    ...
    let dispatch_result = match invocation.operation {
        RunnerOperation::CodingAgent(operation) => {
            let response = match runtime.coding_agents() {
                ...
                None => codegpt_core::coding_agent::CodingAgentResponse::error(
                    codegpt_core::coding_agent::CodingAgentDispatchState::NotStarted,
        ...
        RunnerOperation::PluginGateway(operation) => {
            let response = match operation {
                codegpt_core::plugin::PluginGatewayRequest::ProjectCatalog { project_id } => {
                    runtime
                        .plugins()
        ...
        RunnerOperation::Job(operation) => {
            if let RunnerJobOperation::Stop { job_id } = &operation {
                ...
                if let Err(error) = jobs.stop(job_id) {
                    eprintln!("codegpt-runner stop_job error: {error}");
                }
                Ok(true)
            } else {
                jobs.enqueue(
...
pub(crate) fn dispatch_request(
    ...
    dispatch_request_with_outcome(
        ...
        project_registry_dir,
        lsp,
        &codegpt_browser::BrowserSupervisor::new(),
        request,
    )
