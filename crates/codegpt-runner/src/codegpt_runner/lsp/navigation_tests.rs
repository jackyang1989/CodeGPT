...
fn status_does_not_start_server_and_unavailable_succeeds() {
    ...
    let supervisor = LspSupervisor::new(LspSupervisorConfig {
        commands: HashMap::from([(
            LspServerKind::RustAnalyzer,
            LspCommand::new("/nonexistent/rust-analyzer-codegpt-test"),
        )]),
        ..LspSupervisorConfig::default()
    });
    let policy = RunnerPolicy {
...
///
/// Run with:
/// `cargo test -p codegpt-runner --bin codegpt-runner real_pyright -- --ignored --nocapture`
#[test]
#[ignore = "requires a real pyright-langserver (npm i -g pyright)"]
fn real_pyright_document_symbols_end_to_end() {
    let Some(pyright) = real_language_server("CODEGPT_PYRIGHT", "pyright-langserver") else {
        panic!("pyright-langserver not found; set CODEGPT_PYRIGHT or install pyright");
    };
...
/// Node). Run with:
/// `cargo test -p codegpt-runner --bin codegpt-runner real_typescript -- --ignored --nocapture`
#[cfg(unix)]
#[test]
...
fn real_typescript_document_symbols_end_to_end() {
    let Some(server) = real_language_server(
        "CODEGPT_TYPESCRIPT_LANGUAGE_SERVER",
        "typescript-language-server",
    ) else {
        panic!("typescript-language-server not found; set CODEGPT_TYPESCRIPT_LANGUAGE_SERVER");
    };
...
/// only runs when a preinstalled binary is selected via CODEGPT_GOPLS/PATH.
/// Run with:
/// `cargo test -p codegpt-runner --bin codegpt-runner real_gopls -- --ignored --nocapture`
#[test]
#[ignore = "requires a preinstalled gopls; CodeGPT never installs it automatically"]
fn real_gopls_navigation_and_call_hierarchy_end_to_end() {
    let Some(gopls) = real_language_server("CODEGPT_GOPLS", "gopls") else {
        panic!("gopls not found; set CODEGPT_GOPLS or preinstall gopls manually");
    };
