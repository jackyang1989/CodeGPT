...
impl ShutdownPhaseResult {
    ...
    fn log_line(&self) -> String {
        let mut line = format!(
            "codegpt-runner shutdown phase {} phase={} elapsed_ms={} resources={}",
            self.status.label(),
            self.phase,
            self.elapsed_ms,
            self.resources
...
impl ShutdownReport {
    ...
    pub(crate) fn log_lines(&self) -> Vec<String> {
        let mut lines = self
            ...
            .collect::<Vec<_>>();
        lines.push(format!(
            "codegpt-runner shutdown complete elapsed_ms={} timed_out_phases={} failed_phases={}",
            self.elapsed_ms,
...
impl ShutdownCoordinator {
    ...
    pub(crate) fn request_signal(&self) -> bool {
        ...
        if first {
            eprintln!("codegpt-runner shutdown signal received");
        }
        first
    }
...
mod tests {
    ...
    fn shutdown_coordinator_runs_once_and_completion_log_is_last() {
        ...
        assert!(lines
            .last()
            .unwrap()
            .starts_with("codegpt-runner shutdown complete elapsed_ms="));
