...
use crate::codegpt_runner::detached_job::{
...
fn detached_job_request(
    ...
    env.push((
        "CODEGPT_DETACHED_JOB_MANAGER_SCENARIO".to_string(),
        scenario.to_string(),
    ));
    DetachedStartRequest {
...
fn detached_job_payload_subprocess_entrypoint() {
    let Ok(scenario) = std::env::var("CODEGPT_DETACHED_JOB_MANAGER_SCENARIO") else {
        return;
    };
...
fn detached_recovery_uses_same_inventory_and_observes_terminal_output() {
    ...
    assert!(matches!(
        outcome,
        crate::codegpt_runner::detached_job::DetachedHandoffOutcome::Accepted { .. }
    ));
    ...
    STRUCTURED_PROCESS_HELPER
        .get_or_init(|| {
            ...
            let result = Command::new(rustc)
                .arg("--edition=2021")
                .arg("--crate-name=codegpt_structured_process_helper")
                .arg(source)
                .arg("-o")
                .arg(&output)
                .output()
...
fn cargo_test_terminal_count_evidence_survives_runner_stream_retention() {
    ...
    assert_eq!(
        evidence.status,
        codegpt_core::validation_evidence::CargoTestCountEvidenceStatus::CompleteSummary
    );
...
fn arbitrary_process_output_cannot_forge_cargo_activity() {
    ...
    assert_eq!(
        cargo_activity_from_stderr(&cargo_step, "Compiling codegpt-core\n"),
...
fn cargo_activity_returns_to_validation_plan_after_fine_phase_ends() {
    ...
    let compiling = cargo_activity_from_stderr(&step, "Compiling codegpt v0.3.9\n").unwrap();
    manager.record_update(
...
// `codegpt-process` (compiled at test time with rustc, exactly like the LSP
...
fn job_tree_helper() -> Arc<JobTreeHelper> {
    JOB_TREE_HELPER
        .get_or_init(|| {
            let source = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
                .join("../codegpt-process/src/bin/process_tree_helper.rs");
            let temp = tempfile::tempdir().unwrap();
            let output = temp.path().join(format!(
                "process-tree-helper{}",
                std::env::consts::EXE_SUFFIX
            ...
            let result = Command::new(rustc)
                .arg("--edition=2021")
                .arg("--crate-name=codegpt_job_tree_helper")
                .arg(&source)
                .arg("-o")
                .arg(&output)
                .output()
