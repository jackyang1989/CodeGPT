...
fn initialize_failure_detail(code: &str) -> &'static str {
    match code {
        "plugin_protocol_version_mismatch" => {
            "Plugin initialize response did not confirm codegpt-plugin-v1"
        }
...
impl PluginManager {
    ...
    fn project_catalog_for_root(&self, project_root: &Path) -> ProjectPluginCatalog {
        ...
        hasher.update(b"codegpt.project-plugin-catalog.v1\0");
        for provider in committed.providers.values() {
            ...
            if !codegpt_runner_config::paths::paths_equal(&provider_root, project_root) {
                continue;
            }
...
impl ProviderConnection {
    fn initialize(&mut self, timeout: Duration) -> Result<(), ProviderFailure> {
        ...
        codegpt_core::plugin::validate_json_value(
            &result,
