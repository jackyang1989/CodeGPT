...
fn fake_binary() -> Arc<FakeBinary> {
    ...
    let output = temp
        .path()
        .join(format!("codegpt-plugin-fake{}", env::consts::EXE_SUFFIX));
    let source =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("src/codegpt_runner/fake_plugin.rs");
    let rustc = env::var_os("RUSTC").unwrap_or_else(|| "rustc".into());
    let result = Command::new(rustc)
        .arg("--edition=2021")
        .arg("--crate-name=codegpt_plugin_fake")
        .arg(source)
        .arg("-o")
        .arg(&output)
        .output()
...
fn project_catalog_bounds_aggregate_wire_response_without_losing_total() {
    let entries: Vec<_> = (0..codegpt_core::plugin::PLUGIN_MAX_PROJECT_CATALOG_ENTRIES)
    ...
    codegpt_core::plugin::validate_project_plugin_catalog(&full).unwrap();
    assert!(
        codegpt_core::plugin::validate_response(&PluginGatewayResponse::success(
    ...
    assert!(codegpt_core::plugin::validate_project_plugin_catalog(&invalid).is_err());
    invalid.total_count = codegpt_core::plugin::PLUGIN_MAX_PROJECT_CATALOG_ENTRIES + 1;
    assert!(codegpt_core::plugin::validate_project_plugin_catalog(&invalid).is_err());
    let response =
        PluginGatewayResponse::success(PluginGatewayResponsePayload::ProjectCatalog { catalog });
    codegpt_core::plugin::validate_response(&response).unwrap();
...
fn prepared_environment_reuses_shell_env_default_profile_and_clears_sensitive_values() {
    ...
    let _env = crate::tests::EnvGuard::new()
        .set("CODEGPT_AGENT_TOKEN", "must-not-leak")
        .set("CODEGPT_PAT", "inherited-pat-must-not-leak");
    let temp = tempfile::tempdir().unwrap();
    let marker = temp.path().join("marker.log");
    let fake = fake_binary();
    ...
    shell
        .env
        .insert("CODEGPT_PLUGIN_TEST_ENV".to_string(), "base".to_string());
    shell.env.insert(
        "CODEGPT_PAT".to_string(),
    ...
    shell.profiles = BTreeMap::from([(
        ...
        ShellProfileConfig {
            env: BTreeMap::from([
                (
                    "CODEGPT_PLUGIN_TEST_ENV".to_string(),
...
fn plugin_profile_init_script_is_captured_into_native_child_environment() {
    ...
    shell.profiles.insert(
        "native".to_string(),
        ShellProfileConfig {
            init_script: Some("export CODEGPT_PLUGIN_TEST_ENV=profile-ready".to_string()),
            ..ShellProfileConfig::default()
        },
    );
    let plugins = PluginConfig {
...
fn plugin_committed_environment_ignores_unrelated_shell_runtime_controls() {
    ...
    unused_profile.env.insert(
        "CODEGPT_UNUSED_PLUGIN_PROFILE".to_string(),
        "v1".to_string(),
    );
    unrelated_shell
        .profiles
    ...
    relevant_shell
        ...
        .unwrap()
        .env
        .insert("CODEGPT_PLUGIN_ENV_TEST".to_string(), "v2".to_string());
...
fn runner_config_reload_and_plugin_state_commit_as_one_active_generation() {
    ...
    assert_eq!(
        rejected.error_code,
        Some(codegpt_core::runner_protocol::RunnerConfigErrorCode::PluginReloadFailed)
    );
    assert_eq!(runtime.snapshot().generation, 2);
