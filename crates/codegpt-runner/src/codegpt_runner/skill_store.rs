...
use codegpt_core::skill_metadata::{
...
impl SkillStore {
    pub(super) fn for_runner(client_id: &str, server_url: &str) -> Result<Self, String> {
        ...
        hasher.update(b"codegpt-runner-skill-store-v1\0");
        hasher.update(client_id.as_bytes());
        hasher.update(b"\0");
        hasher.update(server_url.as_bytes());
        let storage_namespace = format!("{:x}", hasher.finalize());
        let root = codegpt_runner_config::paths::default_client_state_base_dir()?
            .join(STORE_DIR)
    ...
    fn replay_path(&self, idempotency_key: &str) -> PathBuf {
        let mut hasher = Sha256::new();
        hasher.update(b"codegpt-skill-store-replay-key-v1\0");
    ...
    fn skill_id(&self, skill_key: &str) -> String {
        let mut hasher = Sha256::new();
        hasher.update(b"codegpt-runner-skill-id-v1\0");
    ...
    fn state_revision(
        ...
        let mut hasher = Sha256::new();
        hasher.update(b"codegpt-skill-store-state-v1\0");
        hasher.update(self.namespace.as_bytes());
        hasher.update(b"\0");
        hasher.update(skill_key.as_bytes());
        hasher.update(b"\0");
    ...
    fn namespace_revision(&self, descriptors: &[RunnerSkillDescriptor]) -> String {
        let mut hasher = Sha256::new();
        hasher.update(b"codegpt-skill-store-namespace-v1\0");
        hasher.update(self.namespace.as_bytes());
    ...
    pub(super) fn prepare_execution_snapshot(
        ...
        if codegpt_core::sensitive_paths::is_secret_path(&path) {
...
fn compute_package_revision(files: &BTreeMap<String, Vec<u8>>) -> String {
    let mut hasher = Sha256::new();
    hasher.update(b"codegpt-skill-package-v1\0");
...
fn hash_simple_intent(operation: &str, values: &[&str]) -> String {
    let mut hasher = Sha256::new();
    hasher.update(b"codegpt-skill-store-intent-v1\0");
...
mod tests {
    ...
    fn replay_retention_gc_respects_claimed_prepared_and_completed_boundaries() {
        ...
        assert_eq!(
            codegpt_core::skill_store::MAX_SKILL_STORE_REPLAY_RETAINED_BYTES,
            64 * 1024 * 1024
        );
