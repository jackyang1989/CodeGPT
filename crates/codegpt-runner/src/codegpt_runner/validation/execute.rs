    ...
    if let Ok(value) = std::env::var(env_override) {
        ...
        if !trimmed.is_empty() {
            #[cfg(windows)]
            {
                ...
                return crate::codegpt_runner::util::resolve_program_in_path(trimmed, &path_var)
                    .map(|program| program.path().to_path_buf());
            }
            #[cfg(not(windows))]
            {
                let path = PathBuf::from(trimmed);
                return crate::codegpt_runner::util::is_executable_file(&path).then_some(path);
            }
        }
    }
    let path_var = std::env::var_os("PATH")?;
    crate::codegpt_runner::util::find_executable_in_path(executable_name, &path_var)
}
...
mod tests {
    ...
    fn env_override_requires_an_executable_file() {
        ...
        const ENV: &str = "CODEGPT_TEST_VALIDATION_EXECUTABLE";
        const MISSING_NAME: &str = "codegpt-validation-executable-that-does-not-exist";
        ...
        VALIDATION_TREE_HELPER
            .get_or_init(|| {
                ...
                let result = Command::new(rustc)
                    .arg("--edition=2021")
                    .arg("--crate-name=codegpt_validation_tree_helper")
                    .arg(&source)
                    .arg("-o")
                    .arg(&output)
                    .output()
    ...
    fn spawn_failure_reports_spawn_error_only() {
        let cwd = tempfile::tempdir().unwrap();
        let captured = run_bounded(
            Path::new("codegpt-validation-executable-that-does-not-exist"),
            &str_args(&["sleep", "0", "0"]),
            cwd.path(),
            30,
            None,
