...
fn inherited_environment_drops_windows_drive_current_directory_entries() {
    ...
    assert!(!should_inherit_env_key("CodeGPT_Token"));
}
    ...
    PROCESS_ARGV_HELPER
        .get_or_init(|| {
            ...
            let result = Command::new(rustc)
                .arg("--edition=2021")
                .arg("--crate-name=codegpt_process_argv_helper")
                .arg(source)
                .arg("-o")
                .arg(&output)
                .output()
...
fn compile_internal_posix_test_executable(path: &Path) {
    ...
    let compiled = Command::new(rustc)
        .arg("--edition=2021")
        .arg("--crate-name=codegpt_internal_posix_bash_helper")
        .arg(source)
        .arg("-o")
        .arg(path)
        .output()
...
fn phase_f_bounded_raw_tail_aligns_complete_utf8_before_windows_decode() {
    ...
    assert!(
        !crate::codegpt_runner::output_text::captured_windows_output_uses_oem_for_test(
            &captured.bytes,
            captured.encoding,
        )
    );
...
fn runner_real_process_typescript_probe_receives_eof_instead_of_runner_stdin() {
    const FIXTURE_ENV: &str = "CODEGPT_TEST_TYPESCRIPT_PROBE_FIXTURE";
...
fn execution_environment_inherits_path_filters_credentials_and_honors_overrides() {
    ...
    assert!(!env.contains_key("CODEGPT_TOKEN"));
    let shell = ShellConfig {
        environment_mode: ShellEnvironmentMode::Isolated,
        env: HashMap::from([
            ("PATH".into(), "shell-path".into()),
            ("CODEGPT_TOKEN".into(), "test-secret".into()),
        ]),
        ..ShellConfig::default()
    };
    ...
    .unwrap();
    assert!(!provider.env_snapshot.contains_key("CODEGPT_TOKEN"));
}
...
fn default_shell_preserves_non_unicode_environment_without_panicking() {
    ...
    assert!(
        !snapshot.contains_key("CODEGPT_OPAQUE_TOOLCHAIN_ENV"),
