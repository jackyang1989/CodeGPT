...
fn fake_binary() -> Arc<FakeBinary> {
    ...
    let output = temp.path().join(format!(
        "codegpt-claude-mcp-fake{}",
        env::consts::EXE_SUFFIX
    ));
    let source =
        PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("src/codegpt_runner/fake_claude_mcp.rs");
    let rustc = env::var_os("RUSTC").unwrap_or_else(|| "rustc".into());
    let result = Command::new(rustc)
        .arg("--edition=2021")
        .arg("--crate-name=codegpt_claude_mcp_fake")
        .arg(source)
        .arg("-o")
        .arg(&output)
        .output()
...
fn native_search_preflight_reports_only_proven_missing_paths() {
    ...
    assert_eq!(marker["codegpt_search"]["path_status"], "not_found");
    assert_eq!(marker["codegpt_search"]["backend"], "native");
...
fn opt_in_real_claude_mcp_probe() {
    let _serial = serialize_fake_mcp_test();
    if env::var("CODEGPT_PROBE_CLAUDE_PROVIDER").as_deref() != Ok("1") {
        return;
    }
    let root = tempfile::tempdir().unwrap();
    let config = ClaudeCodeMcpConfig {
...
fn opt_in_real_claude_mcp_smoke() {
    let _serial = serialize_fake_mcp_test();
    if env::var("CODEGPT_TEST_CLAUDE_MCP").as_deref() != Ok("1") {
        return;
    }
    let temp = tempfile::tempdir().unwrap();
...
fn normalized_search_exit_status_distinguishes_matches_from_no_match() {
    let marker = r#"{"codegpt_search":{"backend":"claude_code","feature_unavailable":false}}"#;
