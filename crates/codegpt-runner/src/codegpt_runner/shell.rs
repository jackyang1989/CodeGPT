...
/// `CodeGPT_Token` and `CodeGPT_Pat` must be filtered too; Unix stays case-sensitive.
...
fn should_inherit_env_key(key: &str) -> bool {
    ...
    // entries such as `=E:=E:\\git\\codegpt` to the native environment
...
fn capture_profile_env_snapshot(
    ...
    let marker = format!("__CODEGPT_ENV_START_{}__", uuid::Uuid::new_v4().simple());
...
pub(crate) fn cwd_allowed(policy: &RunnerPolicy, cwd: &Path) -> Result<(), String> {
    ...
    for root in &policy.allowed_roots {
        if let Ok(root) = canonicalize_existing(root) {
            ...
            if codegpt_runner_config::paths::path_is_within(&cwd, &root) {
                return Ok(());
            }
        }
    }
...
fn run_internal_posix_script_impl(
    ...
    #[cfg(windows)]
    {
        // CodeGPT-generated POSIX programs never inherit the configured shell
