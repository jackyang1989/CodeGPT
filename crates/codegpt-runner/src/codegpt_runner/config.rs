...
const DEFAULT_SYSTEM_CONFIG_DIR: &str = "/etc/codegpt";
...
pub(crate) struct RunnerConfig {
    ...
    /// Static Runner-owned stdio MCP providers exposed through CodeGPT's
...
pub(crate) struct QuicClientConfig {
    ...
    /// ALPN protocol; must match the server's `CODEGPT_QUIC_ALPN`.
        ...
        let candidate = match self.load_candidate() {
            ...
            Err(error) => {
                ...
                if let (Some(field), Some(reason)) = (error_field, error_reason) {
                    eprintln!(
                        "codegpt-runner config reload failed: {} field={} reason={}",
                    ...
                    eprintln!(
                        "codegpt-runner config reload failed: {}",
                        config_wire_atom(code)
                    );
                }
                return (
        ...
        match self
            ...
            .apply_config_candidate_and_then(&candidate, || {
                ...
                tracing::debug!(
                    ...
                    "codegpt-runner MCP provider config activated"
                );
                {
                    let mut routers = lock_unpoison(&self.external_routers);
                    routers.retain(|router| router.strong_count() > 0);
...
fn default_client_base_dir() -> Result<PathBuf, String> {
    codegpt_runner_config::paths::default_client_config_base_dir()
}
...
pub(crate) fn client_profile_runner_config(profile: &str) -> Result<PathBuf, String> {
    codegpt_runner_config::paths::resolve_runner_config_path(
...
pub(crate) fn default_config_path() -> Result<PathBuf, String> {
    ...
    if let Some(path) = codegpt_runner_config::paths::existing_runner_config_path(&user_dir)? {
        return Ok(path);
    }
    #[cfg(not(windows))]
    {
        ...
        if system_dir != user_dir {
            if let Some(path) =
                codegpt_runner_config::paths::existing_runner_config_path(&system_dir)?
            {
                return Ok(path);
            }
        }
    }
    Ok(user_dir.join(codegpt_runner_config::paths::RUNNER_CONFIG_FILE))
}
...
fn validate_ssh_resource_name(name: &str) -> Result<(), String> {
    codegpt_core::ssh_resource::validate_ssh_resource_name(name)
...
fn validate_acp_config(config: &AcpConfig) -> Result<(), String> {
    ...
    for agent in &config.agents {
        ...
        for (destination, source) in &agent.env_from_env {
            ...
            {
                return Err(format!(
                    "ACP agent '{}' env_from_env may not map CodeGPT-sensitive environment variables",
                    agent.id
                ));
            }
            if destinations
...
fn validate_mcp_gateway_config(config: &McpGatewayConfig) -> Result<(), String> {
    ...
    for provider in &config.providers {
        ...
        for (destination, source) in &provider.env_from_env {
            ...
            {
                return Err(format!(
                    "mcp provider '{}' env_from_env may not map CodeGPT-sensitive environment variables",
                    provider.id
                ));
            }
            if destinations
...
mod acp_config_tests {
    ...
    #[test]
    fn acp_env_mapping_rejects_codegpt_pat() {
        for (destination, source) in [("CODEGPT_PAT", "SOURCE"), ("DEST", "CODEGPT_PAT")] {
        ...
        #[cfg(windows)]
        {
            ...
            mixed_case
                .env_from_env
                .insert("DEST".to_string(), "CodeGPT_Pat".to_string());
...
mod mcp_gateway_config_tests {
    ...
    fn mcp_gateway_execution_context_rejects_sensitive_and_platform_duplicate_names() {
        for (destination, source) in [
            ("CODEGPT_TOKEN", "SOURCE"),
            ("CODEGPT_PAT", "SOURCE"),
            ("DEST", "CODEGPT_PAT"),
            ("DEST", "CODEGPT_AGENT_TOKEN"),
            ("CODEGPT_USER_TOKEN", "SOURCE"),
            ("DEST", "AUTHORIZATION"),
        ] {
