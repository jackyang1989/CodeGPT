...
//!
//! This is deliberately separate from `codegpt-process::ManagedChild`'s
...
const DETACHED_INTERNAL_SUPERVISOR: &str = "--codegpt-internal-detached-supervisor";
const DETACHED_INTERNAL_WATCHDOG: &str = "--codegpt-internal-detached-watchdog";
...
impl DetachedJobStore {
    ...
    pub(crate) fn default_root_for_runner(
        ...
        hasher.update(b"codegpt-detached-store-runner-v1\0");
    ...
    #[cfg(any(unix, windows))]
    {
        Some(match run_internal_platform_mode(&args) {
            ...
            Err(error) => {
                ...
                eprintln!("codegpt-runner detached internal mode failed: {error}");
                1
            }
        })
    }
...
fn execution_id_for(request: &DetachedStartRequest) -> String {
    let mut hasher = Sha256::new();
    hasher.update(b"codegpt-detached-job-v1\0");
...
fn internal_mode_command(mode: &str, args: &[String]) -> Result<Command, String> {
    #[cfg(test)]
    {
        ...
        command
            .arg("--exact")
            .arg("codegpt_runner::detached_job::tests::internal_mode_subprocess_entrypoint")
            .arg("--nocapture")
            .env_clear()
            .env("CODEGPT_DETACHED_TEST_INTERNAL_MODE", mode)
            .env(
                "CODEGPT_DETACHED_TEST_INTERNAL_ARGS",
