...
fn internal_mode_subprocess_entrypoint() {
    let Some(mode) = std::env::var_os("CODEGPT_DETACHED_TEST_INTERNAL_MODE") else {
        return;
    };
    let args: Vec<String> = serde_json::from_str(
        &std::env::var("CODEGPT_DETACHED_TEST_INTERNAL_ARGS").expect("internal args"),
    )
...
fn payload_command(
    ...
    let args = vec![
        "--exact".to_string(),
        "codegpt_runner::detached_job::tests::payload_subprocess_entrypoint".to_string(),
        "--nocapture".to_string(),
    ];
    let mut all_env = env;
    all_env.push((
        "CODEGPT_DETACHED_PAYLOAD_SCENARIO".to_string(),
        scenario.to_string(),
    ));
...
fn payload_subprocess_entrypoint() {
    let Some(scenario) = std::env::var_os("CODEGPT_DETACHED_PAYLOAD_SCENARIO") else {
        return;
    };
...
fn payload_descendant_subprocess_entrypoint() {
    let Some(marker) = std::env::var_os("CODEGPT_DETACHED_DESCENDANT_MARKER") else {
        return;
    };
...
fn run_accept_then_exit_owner(temp: &Path, state_root: &Path, request: &DetachedStartRequest) {
    ...
    owner
        .arg("--exact")
        .arg("codegpt_runner::detached_job::tests::accept_then_exit_owner_subprocess_entrypoint")
        .arg("--nocapture")
        .env_clear()
        .env("CODEGPT_DETACHED_ACCEPT_EXIT_INSTRUCTION", &instruction)
        .stdout(Stdio::null())
        .stderr(Stdio::null());
...
fn runner_real_process_accepted_handoff_keeps_payload_alive_after_owner_process_exits() {
    ...
    owner
        .arg("--exact")
        .arg("codegpt_runner::detached_job::tests::owner_subprocess_entrypoint")
        .arg("--nocapture")
        .env_clear()
        .env("CODEGPT_DETACHED_OWNER_INSTRUCTION", &instruction)
        .stdout(Stdio::null())
        .stderr(Stdio::null());
...
fn owner_subprocess_entrypoint() {
    let Some(path) = std::env::var_os("CODEGPT_DETACHED_OWNER_INSTRUCTION") else {
        return;
    };
...
fn accept_then_exit_owner_subprocess_entrypoint() {
    let Some(path) = std::env::var_os("CODEGPT_DETACHED_ACCEPT_EXIT_INSTRUCTION") else {
        return;
    };
