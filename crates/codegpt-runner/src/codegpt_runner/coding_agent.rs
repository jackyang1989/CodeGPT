...
impl DurableRunStore {
    ...
    fn default_root(client_id: &str, server_url: &str) -> Result<PathBuf, String> {
        ...
        hasher.update(b"codegpt-coding-agent-store-runner-v1\0");
        hasher.update(client_id.as_bytes());
        hasher.update(b"\0");
        hasher.update(server_url.as_bytes());
        let namespace = format!("{:x}", hasher.finalize());
        Ok(
            codegpt_runner_config::paths::default_client_state_base_dir()?
...
impl CodingAgentManager {
    ...
    fn start(
        self: &Arc<Self>,
        request: codegpt_core::coding_agent::CodingAgentStartRequest,
        project_registry_dir: &Path,
    ) -> CodingAgentResponse {
        #[cfg(test)]
        {
        ...
        if !self.write_pre_prompt_frame(
            ...
            request_frame(
                ...
                json!({
                    "protocolVersion": 1,
                    "clientCapabilities": {},
                    "clientInfo": {"name":"codegpt-runner","version":env!("CARGO_PKG_VERSION")}
                }),
            ),
            run_deadline,
            ...
            match rx.recv_timeout(ACP_POLL) {
                Ok(ReaderEvent::Message(message)) => {
                        ...
                        == Some("session/request_permission")
                    {
                        ...
                        // the only fail-closed ACP outcome emitted by CodeGPT.
                        if !self.write_post_prompt_frame(
                            &request.run_id,
                            &entry,
                            &mut child,
...
fn bounded_text(value: &str) -> String {
    const MAX: usize = codegpt_core::coding_agent::CODING_AGENT_MAX_EVENT_TEXT_BYTES;
    const SUFFIX: &str = "…";
    if value.len() <= MAX {
...
log({'startup_pid':os.getpid(),'env_keys':sorted(k for k in os.environ if k.startswith('CODEGPT_TEST_ACP_') or k=='ACP_VISIBLE')})
...
"#;
    ...
    fn start_request(
        ...
        CodingAgentRequest::Start(codegpt_core::coding_agent::CodingAgentStartRequest {
    ...
    fn max_instruction_request(
        ...
        start.instruction =
            "x".repeat(codegpt_core::coding_agent::CODING_AGENT_MAX_INSTRUCTION_BYTES);
    ...
    fn bounded_text_never_exceeds_utf8_byte_budget() {
        let max = codegpt_core::coding_agent::CODING_AGENT_MAX_EVENT_TEXT_BYTES;
        let cases = [
            "a".repeat(max),
            "a".repeat(max + 1),
    ...
    fn event_ring_capacity_and_continuation_are_bounded() {
        ...
        let response = manager.handle(
            CodingAgentRequest::Observe(codegpt_core::coding_agent::CodingAgentObserveRequest {
                run_id: run.to_string(),
    ...
    fn child_environment_is_cleared_and_missing_mapping_never_spawns() {
        ...
        let _env = crate::tests::EnvGuard::new()
            .set("CODEGPT_TEST_ACP_VISIBLE", "visible-value")
            .set("CODEGPT_TEST_ACP_HIDDEN", "must-not-reach-child");
        let temp = TempDir::new().unwrap();
        let (exe, args) = fake_agent(&temp, "end");
        let mut cfg = fake_config(exe, args);
        cfg.agents[0].env_from_env = BTreeMap::from([(
            "ACP_VISIBLE".to_string(),
            "CODEGPT_TEST_ACP_VISIBLE".to_string(),
        )]);
        let projects = project_fixture(&temp);
        let root = temp.path().join("repo");
        let manager = CodingAgentManager::with_store(&cfg, temp.path().join("store")).unwrap();
        ...
        cfg.agents[0].env_from_env = BTreeMap::from([(
            "ACP_VISIBLE".to_string(),
            "CODEGPT_TEST_ACP_MISSING".to_string(),
        )]);
        let projects = project_fixture(&temp);
        let root = temp.path().join("repo");
        let manager = CodingAgentManager::with_store(&cfg, temp.path().join("store")).unwrap();
    ...
    fn real_codex_acp_opt_in_dogfood() {
        ...
        let request = CodingAgentRequest::Start(codegpt_core::coding_agent::CodingAgentStartRequest {
            ...
            provider_id: "codex".to_string(),
            provider_instance_id: provider.provider_instance_id,
            instruction: "Read Cargo.toml only and reply with the CodeGPT package version in one short sentence. Do not modify files, run builds, install dependencies, or request elevated permissions.".to_string(),
            config: BTreeMap::new(),
            timeout_secs: 180,
        });
