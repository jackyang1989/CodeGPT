...
impl PersistentShellManager {
    ...
    fn open_ssh(
        ...
        let transport_box: Box<dyn codegpt_persistent_shell::ShellTransport> = Box::new(transport);
...
mod windows_tests {
    ...
    fn windows_named_ssh_persistent_shell_real_transport_opt_in() {
        let Ok(host) = std::env::var("CODEGPT_TEST_WINDOWS_SSH_HOST") else {
            eprintln!("skipping Windows SSH persistent-shell integration test; CODEGPT_TEST_WINDOWS_SSH_HOST is unset");
            return;
        };
        let (_temp, _project, projects, mut policy) = fixture();
        let shell = ShellConfig::default();
