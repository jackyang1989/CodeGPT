...
use codegpt_runner::is_checkpoint_request_kind;
use codegpt_runner::output_text::{OutputTextDecoder, OutputTextSource};
#[cfg(test)]
use codegpt_runner::QuicClientConfig;
#[cfg(test)]
use codegpt_runner::{
...
use codegpt_runner::{is_transport_failure, SshConfig, SshConnectionPool};
use codegpt_runner::{
...
struct PendingJobStart {
    ...
    shell: ShellConfig,
    ssh: SshConfig,
    skills: codegpt_runner::config::SkillsConfig,
    client_id: String,
    server_url: String,
...
impl PendingJobStart {
    fn from_wire(
        ...
        Self {
            ...
            shell,
            ssh,
            skills: codegpt_runner::config::SkillsConfig::default(),
            client_id,
...
fn usage() -> &'static str {
    "Usage: codegpt-runner [--config PATH] [--once] [--stop-on-stdin-eof]\n\n\
     Options:\n\
     ...
     ~/.config/codegpt/clients/<profile> for non-root users. Explicit\n\
     --config overrides the profile-derived default.\n\n\
     Environment:\n\
       CODEGPT_RUNNER_CONFIG     default config path override\n\
       CODEGPT_AGENT_CONFIG      legacy alias for CODEGPT_RUNNER_CONFIG\n\
     Example runner.toml:\n\
       ...
       display_name = \"XRH\"\n\
       owner = \"yyjeqhc\"\n\
       project_registry_dir = \"/root/.config/codegpt/project-registry\"\n\
    ...
    S: AsRef<str>,
{
    ...
    if args.len() == 1 {
        match args[0].as_str() {
            ...
            "--version" | "-V" => {
                return Ok(RunnerCliAction::Exit {
                    code: 0,
                    stdout: build_info::version_output("codegpt-runner"),
                    stderr: String::new(),
                });
            }
            _ => {}
    ...
    let runner_config_env = std::env::var("CODEGPT_RUNNER_CONFIG").ok();
    let legacy_agent_config_env = std::env::var("CODEGPT_AGENT_CONFIG").ok();
    ...
    let config_path = if let Some(config_path) = config_path {
        ...
        if let Some(profile) = profile {
            ...
            if runner_config_env.is_some() && legacy_agent_config_env.is_some() {
                return Err(
                    "CODEGPT_RUNNER_CONFIG and legacy CODEGPT_AGENT_CONFIG cannot both be set"
                        .to_string(),
                );
            }
            runner_config_env
...
struct PollingDispatch {
    ...
    jobs: JobManager,
    persistent_shells: codegpt_runner::PersistentShellManager,
    project_registry_dir: PathBuf,
    lsp: codegpt_runner::LspSupervisor,
    browser: codegpt_browser::BrowserSupervisor,
    request: RunnerRequest,
}
...
impl PollingDispatchSupervisor {
    ...
    fn spawn(&mut self, dispatch: PollingDispatch) -> Result<(), PollError> {
        ...
        let handle = std::thread::Builder::new()
            .name("codegpt-poll-dispatch".to_string())
            .spawn(move || {
                let mut completion =
...
/// Hidden, test/ops-only knob: parse `CODEGPT_RUNNER_DISABLE_JOB_STATE_RECONCILIATION`
...
fn disable_job_state_reconciliation_for_test() -> bool {
    matches!(
        std::env::var("CODEGPT_RUNNER_DISABLE_JOB_STATE_RECONCILIATION")
            .ok()
...
fn runner_register_capabilities(cfg: &RunnerConfig) -> RunnerCapabilities {
    ...
    // CodeGPT 0.4 requires every successful patch to expose the complete bounded
    ...
    capabilities.persistent_shell = codegpt_persistent_shell::local_shell_supported();
    ...
    let browser_available = codegpt_browser::discover_chromium_executable().is_some();
...
fn build_register_request_with_provider_status(
    ...
    RunnerRegisterRequest,
    Arc<codegpt_runner::external_tools::ExternalToolRouter>,
    u64,
) {
...
/// `TerminateJobObject` on Windows) stays inside `codegpt-process`.
...
fn observe_cargo_test_count_chunks(
    accumulator: &mut Option<codegpt_core::cargo_test_count::CargoTestRunMetadataAccumulator>,
    stdout: &str,
    stderr: &str,
) {
...
fn finish_cargo_test_count_evidence(
    accumulator: Option<codegpt_core::cargo_test_count::CargoTestRunMetadataAccumulator>,
...
fn validate_detached_recovery_context(
    ...
    if let Some(session_id) = context.workflow_session_id.as_deref() {
        if context.runtime_project_id.is_none()
            || !codegpt_core::workflow_session_contract::is_valid_session_id(session_id)
        {
...
impl JobManager {
    ...
    fn spawn_detached_observer(
        ...
        std::thread::Builder::new()
            .name("codegpt-detached-job-observer".to_string())
            .spawn(move || {
                ...
                loop {
                    ...
                    let terminal = record.phase == codegpt_runner::detached_job::DetachedJobPhase::Terminal;
    ...
    fn sync_detached_record(
        &self,
        job_id: &str,
        record: &codegpt_runner::detached_job::DetachedJobRecord,
    ) -> Result<bool, String> {
        let snapshot = snapshot_from_detached_record(record)?;
    ...
    fn stop_all(&self) {
        ...
        if outcome.timed_out > 0 || outcome.failures > 0 {
            eprintln!(
                "codegpt-runner shutdown job cleanup incomplete resources={} timed_out={} failures={}",
    ...
    fn start_shell_job(&self, start: PendingJobStart) {
        ...
        std::thread::spawn(move || {
            ...
            let mut test_count_accumulator = capture_cargo_test_count
                .then(codegpt_core::cargo_test_count::CargoTestRunMetadataAccumulator::default);
                ...
                let step_status = loop {
                    ...
                    match wait_result {
                        ...
                        Err(e) => {
                            ...
                            eprintln!("codegpt-runner failed to wait job {job_id}: {e}");
                            break (
                                "failed".to_string(),
                                None,
...
fn handle_one_poll(
    ...
    let handle = std::thread::Builder::new()
        .name("codegpt-poll-dispatch-once".to_string())
        .spawn(move || {
            let _dispatch_guard = dispatch_guard;
...
fn main() {
    if let Some(code) =
        codegpt_runner::detached_job::maybe_run_internal_mode(std::env::args().skip(1))
    {
    ...
    if cfg.token.trim().is_empty() {
        eprintln!(
            "codegpt-runner warning: agent token is empty; connecting without Authorization; the server must be started with --open"
        );
    }
    if let Err(e) = run_runner(cfg, config_path, once, stop_on_stdin_eof) {
        eprintln!("codegpt-runner failed: {}", e);
        std::process::exit(1);
    }
}
