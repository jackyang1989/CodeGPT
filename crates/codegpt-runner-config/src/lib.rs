...
//! `codegpt-runner`, and `codegpt-cli`.
//!
...
use codegpt_core::runner_protocol::RunnerCapabilities;

pub mod paths;

/// Default Runner project registry selected for a new system-level install.
pub const DEFAULT_INIT_PROJECT_REGISTRY_DIR: &str = "/etc/codegpt/project-registry";
...
pub fn resolve_runner_init_token(opts: &RunnerInitOptions) -> Result<String, String> {
    ...
    let token = std::env::var("CODEGPT_AGENT_TOKEN")
        .map_err(|_| "--token, --token-file, or CODEGPT_AGENT_TOKEN is required".to_string())?
        .trim()
        .to_string();
    if token.is_empty() {
        return Err("CODEGPT_AGENT_TOKEN cannot be empty".to_string());
    }
    Ok(token)
}
...
mod tests {
    ...
    fn init_opts(output: PathBuf) -> RunnerInitOptions {
        RunnerInitOptions {
            ...
            project_registry_dir: PathBuf::from("/etc/codegpt/project-registry"),
            output,
