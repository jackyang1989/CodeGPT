//! Central platform directory policy shared by `codegpt-cli` and
//! `codegpt-runner`.
//!
...
//!
//! - **Unix**: XDG-style layout rooted at `$HOME` (`~/.config/codegpt`,
//!   `~/.local/state/codegpt`), with `/etc/codegpt` for effective-root.
//!   Existing behavior is preserved exactly.
//! - **Windows**: configuration and credentials live in `%APPDATA%\codegpt`
//!   (Roaming profile, follows the user) with `%USERPROFILE%\.config\codegpt`
//!   as the fallback; runner state/logs live in `%LOCALAPPDATA%\codegpt`
//!   (machine-local) with `%USERPROFILE%\.local\state\codegpt` and finally
//!   `%TEMP%\codegpt` as fallbacks. `HOME` and the XDG variables are
...
/// Canonical Runner configuration filename for CodeGPT 0.4 and later.
...
/// `false` (there is no `/etc/codegpt` system scope).
...
/// Base directory for per-user CodeGPT configuration and credentials
/// (client profiles, `runner.toml`, Runner project registry, token files).
///
/// - Unix (root): `/etc/codegpt`
/// - Unix (user): `$XDG_CONFIG_HOME/codegpt`, else `$HOME/.config/codegpt`.
///   When `HOME` is also missing the caller gets an error (never `.`).
/// - Windows: `%APPDATA%\codegpt`, else `%USERPROFILE%\.config\codegpt`,
...
pub fn default_client_config_base_dir() -> Result<PathBuf, String> {
    #[cfg(windows)]
    {
        ...
        if let Some(appdata) = std::env::var_os("APPDATA").filter(|value| !value.is_empty()) {
            return Ok(PathBuf::from(appdata).join("codegpt"));
        }
    ...
    #[cfg(not(windows))]
    {
        ...
        if let Some(config_home) = std::env::var_os("XDG_CONFIG_HOME").filter(|v| !v.is_empty()) {
            return Ok(PathBuf::from(config_home).join("codegpt"));
        }
        if is_effective_root() {
            return Ok(PathBuf::from("/etc/codegpt"));
        }
        let home = home_dir().ok_or_else(|| {
            "cannot determine user home: set HOME to derive the CodeGPT config directory"
                .to_string()
        })?;
        Ok(home.join(".config/codegpt"))
    }
}

/// Base directory for per-user CodeGPT state: hosted Runner state
/// (`runner.toml`), Runner logs, checkpoints and recovery data.
///
/// - Unix: `$XDG_STATE_HOME/codegpt`, else `$HOME/.local/state/codegpt`,
///   else `$TMPDIR/codegpt` (existing behavior preserved).
/// - Windows: `%LOCALAPPDATA%\codegpt`, else
///   `%USERPROFILE%\.local\state\codegpt`, else `%TEMP%\codegpt`. `HOME` and
...
pub fn default_client_state_base_dir() -> Result<PathBuf, String> {
    #[cfg(windows)]
    {
        ...
        if let Some(local_appdata) = std::env::var_os("LOCALAPPDATA").filter(|v| !v.is_empty()) {
            return Ok(PathBuf::from(local_appdata).join("codegpt"));
        }
        if let Some(profile) = std::env::var_os("USERPROFILE").filter(|value| !value.is_empty()) {
            return Ok(PathBuf::from(profile).join(".local/state/codegpt"));
        }
        // A volatile temp location is better than a relative path for state
        // that can be regenerated.
        Ok(std::env::temp_dir().join("codegpt"))
    }
    #[cfg(not(windows))]
    {
        if let Some(state_home) = std::env::var_os("XDG_STATE_HOME").filter(|v| !v.is_empty()) {
            return Ok(PathBuf::from(state_home).join("codegpt"));
        }
        if let Some(home) = home_dir() {
            return Ok(home.join(".local/state/codegpt"));
        }
...
mod tests {
    ...
    fn windows_config_base_uses_appdata_then_userprofile() {
        ...
        #[cfg(windows)]
        {
            ...
            assert_eq!(
                default_client_config_base_dir().unwrap(),
                PathBuf::from("C:\\Users\\alice\\AppData\\Roaming\\codegpt")
            );
        }
        ...
        assert_eq!(
            default_client_config_base_dir().unwrap(),
            PathBuf::from("C:\\Users\\alice\\.config\\codegpt")
        );
    }
    ...
    fn windows_state_base_uses_localappdata_then_userprofile() {
        ...
        #[cfg(windows)]
        {
            ...
            assert_eq!(
                ...
                PathBuf::from("C:\\Users\\alice\\AppData\\Local\\codegpt")
            );
        }
        ...
        assert_eq!(
            default_client_state_base_dir().unwrap(),
            PathBuf::from("C:\\Users\\alice\\.local\\state\\codegpt")
        );
    }
    ...
    fn config_base_honors_xdg_on_unix_and_ignores_it_on_windows() {
        ...
        if is_effective_root() {
            ...
            assert_eq!(
                default_client_config_base_dir().unwrap(),
                PathBuf::from("/home/alice/.config/codegpt")
            );
        }
        #[cfg(windows)]
        assert_eq!(
    ...
    fn state_base_honors_xdg_on_unix_and_ignores_it_on_windows() {
        ...
        assert_eq!(
            default_client_state_base_dir().unwrap(),
            PathBuf::from("/tmp/state/codegpt")
        );
        #[cfg(windows)]
        assert_eq!(
            default_client_state_base_dir().unwrap(),
        ...
        assert_eq!(
            default_client_state_base_dir().unwrap(),
            PathBuf::from("/home/alice/.local/state/codegpt")
        );
        #[cfg(windows)]
        assert_eq!(
            default_client_state_base_dir().unwrap(),
    ...
    fn project_path_policy_keeps_dangerous_roots_fail_closed_under_cwd_anywhere() {
        ...
        validate_project_path_policy(Path::new("/tmp/codegpt-project"), &[], true)
    ...
    fn state_base_falls_back_to_temp_only_when_no_home_anywhere() {
        ...
        assert_eq!(base, std::env::temp_dir().join("codegpt"));
    }
