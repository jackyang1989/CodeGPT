...
use codegpt_core::authority::SCOPE_JOB_DETACH;
...
pub(super) const EXECUTION_DEFINITIONS: &[ToolDefinition] = &[
    ...
    requires_explicit_business_session(model_spec(
            ...
            "Open one bounded long-lived shell for an explicit Workflow Session. Primary use: one shell for repeated commands on the active named SSH resource in execution_context.resource, preserving remote cwd/env/exports/functions/umask. Local sh/bash or Windows PowerShell remains supported only when same local shell-process state is actually required, not merely for several commands. New SSH targets use ssh_resource list/register, Runner restart, list again, then update_session_context; no per-shell host/resource parameter. The SSH target does not need CodeGPT Runner.",
    )),
    requires_explicit_business_session(model_spec(
            def(
                "session_shell_exec",
    ...
    permission_risk(
        model_spec(
            ...
            "Stop one existing CodeGPT Job by job_id. Requires confirm=true and preserves project/session ownership; log bodies are not returned.",
        ),
        PERMISSION_RISK_JOB,
    ),
    adaptive_runtime_direct(
