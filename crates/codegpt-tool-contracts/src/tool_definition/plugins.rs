...
pub(super) const DEFINITIONS: &[ToolDefinition] = &[adaptive_runtime_direct(
    require_any_scopes(
        model_spec(
            ...
            "Stable gateway for Runner-owned native Tool Plugins. Provider tools are never outer CodeGPT MCP tools. Discovery begins at an exact caller-visible Runner; describe observes one exact Runner/provider/tool schema and returns an opaque binding; call accepts only binding + arguments, never retargets, relists, reloads, or blindly retries. Gateway visibility requires any Plugin scope, while each action separately enforces plugin:inspect, plugin:invoke, or plugin:manage before provider dispatch.",
