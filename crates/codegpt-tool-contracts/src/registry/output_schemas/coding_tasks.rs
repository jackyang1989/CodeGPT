...
fn startup_extensions_schema() -> Value {
    ...
    json!({
        ...
        "properties": {
            "skills": family(skill_entry, "^cg_skillcat_[A-Za-z0-9_-]{43}$"),
            "plugins": family(plugin_entry, "^cg_plugcat_[A-Za-z0-9_-]{43}$")
        },
...
fn startup_workflow_schema() -> Value {
    json!({
        "type": "object",
        "description": "CodeGPT-owned workflow defaults and optional named coding/review roles. Separate from project instructions and Session authority.",
        "properties": {
...
fn startup_instructions_schema() -> Value {
    json!({
        "type": "object",
        "description": "Project-local repository instructions discovered from fixed sources such as AGENTS.md or CLAUDE.md. Separate from the CodeGPT built-in workflow.",
        "properties": {
            "status": {
                "type": "string",
...
fn work_on_project_output_schema() -> Value {
    ...
    let compact_instructions = json!({
        "type": "object",
        "description": "Compact project-local repository instruction projection, separate from the CodeGPT built-in workflow. status reports repository/Workflow Session delta; content_included reports this call's caller-explicit model-facing body projection. False/null/empty body-projection defaults are omitted.",
        "properties": {
            "status": {
                "type": "string",
                "enum": ["loaded", "reused", "changed", "not_found", "unavailable"]
    ...
    let output_properties = vec![
        ...
        (
            "workflow",
            {
                ...
                schema["description"] = json!("Canonical static built-in CodeGPT coding-workflow guidance. Included on every work_on_project call with include_workflow_guidance=true and omitted only when the caller explicitly passes false; Workflow Session or transport identity never suppresses it automatically.");
                schema
            },
        ),
