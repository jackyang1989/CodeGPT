...
fn goal_summary_schema() -> Value {
    json!({
        ...
        "properties": {
            "goal_id": {"type": "string", "pattern": "^cg_goal_[A-Za-z0-9_-]{16}$", "description": "Canonical durable Goal id; identity only, never authority."},
...
fn correlation_schema() -> Value {
    json!({
        ...
        "properties": {
            ...
            "reference_id": {"type": "string", "pattern": "^(cg_agent_task_[A-Za-z0-9_-]{16}|cg_sess_([A-Za-z0-9_-]{16}|[0-9a-f]{32}))$", "maxLength": 46, "description": "Exact correlated durable identity. It is not a credential and cannot be dereferenced without that domain's normal authorization."},
...
fn goal_activity_schema() -> Value {
    json!({
        ...
        "properties": {
            ...
            "last_seen_at_ms": nullable_integer("Latest caller-visible CodeGPT Window activity, including non-meaningful Host/App control traffic."),
            "last_meaningful_activity_at_ms": nullable_integer("Latest caller-visible meaningful CodeGPT business activity across correlated Windows."),
            ...
            "active_meaningful_request_count": {"anyOf": [{"type": "integer", "minimum": 0, "maximum": 64}, {"type": "null"}], "description": "Visible in-flight meaningful CodeGPT requests across candidate Windows, or null when unavailable/not applicable."},
...
fn goal_plan_schema() -> Value {
    json!({
        ...
        "properties": {
            ...
            "goal_id": {"type": "string", "pattern": "^cg_goal_[A-Za-z0-9_-]{16}$", "description": "Exact durable Goal identity used for refresh/rehydration and app-only polling. Identity is never authority."},
            "title": {"type": "string", "minLength": 1, "maxLength": 200, "description": "Bounded Goal title."},
