    ...
    let job_observation = json!({
        ...
        "properties": {
            ...
            "observation_token": {
                "type": "string",
                "minLength": 1,
                "maxLength": codegpt_core::job_observation::MAX_JOB_OBSERVATION_TOKEN_LEN,
    ...
    match name {
        "run_detached_process" => {
            let mut schema = wrapped_output_schema(vec![
                ...
                ("tool_failure", schema_type("boolean", "True for CodeGPT initiation/runtime failures.")),
        ...
        "run_process" => {
            let mut properties = vec![
                ...
                (
                    ...
                    schema_type(
                        "boolean",
                        "True for CodeGPT tool/runtime failures; false for child exit status failures. Omitted when false on ordinary synchronous terminal success.",
                    ),
                ),
        ...
        "run_script" => {
            let mut properties = vec![
                ...
                (
                    ...
                    schema_type(
                        "boolean",
                        "True for CodeGPT tool/runtime failures; false for interpreter exit status failures. Omitted when false on ordinary synchronous terminal success.",
                    ),
                ),
                ("purpose", schema_type("string", "Declared execution purpose. Audit metadata: omitted on ordinary synchronous terminal success and from the default model-facing failure projection.")),
                (
        ...
        "run_shell" => {
            let mut properties = vec![
                ...
                (
                    ...
                    schema_type(
                        "boolean",
                        "True for CodeGPT tool/runtime failures; false for command exit status failures or a healthy durable handoff.",
                    ),
                ),
                ("purpose", schema_type("string", "Declared execution purpose.")),
                (
...
fn persistent_shell_output_schema() -> Value {
    wrapped_output_schema(vec![
        ...
        (
            ...
            schema_type(
                "boolean",
                "Whether CodeGPT rejected or lost the operation.",
            ),
        ),
    ])
}
