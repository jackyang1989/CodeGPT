...
use codegpt_core::runner_protocol::ShellScriptLanguage;
...
pub struct PluginToolCall {
    ...
    /// Logical provider-local Plugin tool name. Provider tools never become outer CodeGPT MCP tool
    /// names.
    #[schemars(length(min = 1, max = 128))]
    #[serde(default)]
...
pub enum ToolCall {
    ...
    WorkOnProject {
        ...
        /// applicable repository instructions; keep true for a fresh or uncertain model context. CodeGPT
        ...
        #[schemars(extend("default" = true))]
        /// Whether this bootstrap response should include the static built-in CodeGPT coding-workflow
        ...
        /// guidance; keep true for a fresh or uncertain model context. CodeGPT never infers retention from
    ...
    PostSessionMessage {
        ...
        /// system/platform/CodeGPT safety policy.
    ...
    PostPeerMessage {
        ...
        priority: SessionMessagePriority,
        #[schemars(extend("default" = false))]
        /// When false, CodeGPT attempts one ambient projection on the recipient's next model-facing
    ...
    CodingAgentStart {
        ...
        #[serde(default)]
        config: Option<BTreeMap<String, codegpt_core::coding_agent::CodingAgentConfigValue>>,
    ...
    CargoFmt {
        ...
        /// coding to ensure formatting: CodeGPT first checks, then runs mutating `cargo fmt` only when a
    ...
    /// Register an existing directory as a CodeGPT project on a selected
    ...
    /// observed from `list_projects`. This removes only CodeGPT registration
