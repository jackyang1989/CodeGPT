...
fn git_diff_hunks_recovery_schema_accepts_only_sparse_actionable_lanes() {
    ...
    let arguments = json!({
        "project": "agent:special:codegpt", "paths": ["a.txt"],
...
fn key_tool_output_schemas_include_expected_fields() {
    ...
    for name in ["cargo_fmt", "cargo_check", "cargo_test", "go_test"] {
        ...
        assert_eq!(
            ...
            codegpt_core::runtime_contract::MAX_JOB_OBSERVATION_WAIT_SECS
        );
        assert_eq!(
...
fn validation_summary_schema_exposes_optional_recoverable_assertion_label_only() {
    ...
    let expected_purposes = codegpt_core::workflow_session_contract::EXECUTION_PURPOSE_VALUES
        .iter()
        .copied()
        .filter(|purpose| {
            codegpt_core::workflow_session_contract::is_validation_like_execution_purpose(purpose)
        })
        .map(Value::from)
        .collect::<Vec<_>>();
    assert_eq!(
    ...
    assert_eq!(
        assertion["maxLength"],
        codegpt_core::workflow_session_contract::MAX_MODEL_VALIDATION_ASSERTION_NAME_CHARS
    );
