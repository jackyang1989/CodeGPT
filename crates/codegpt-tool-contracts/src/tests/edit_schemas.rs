...
fn apply_text_edits_input_schema_encodes_structural_wire_contracts() {
    ...
    assert!(matches!(
        ...
        ToolCall::ApplyTextEdits { changes, .. }
            if changes.len() == 1
                && changes[0].kind == codegpt_core::apply_edits_shared::ApplyFileChangeKind::Edit
                && changes[0].edits.len() == 1
                && changes[0].edits[0].kind == codegpt_core::apply_edits_shared::ApplyTextEditKind::ReplaceExact
