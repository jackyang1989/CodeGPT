...
export function formatWorkspaceBreadcrumb(
  project: any,
...
export function formatSelectedProjectIdentity(
  project: any,
  language?: RuntimeLanguage,
): string {
  if (language !== "zh-CN") {
...
export function formatSessionWorkspaceIdentity(
  project: any,
  language?: RuntimeLanguage,
): string {
  return formatProjectIdentity(project, language);
...
export function formatDeviceStatusText(
  devicesCount: number,
  filter: string,
  language?: RuntimeLanguage,
): string {
...
export function formatRunnerCountText(count: number, language?: RuntimeLanguage): string {
  return localizedCountLabel(count, "Runner", "Runners", language);
}

export function formatRecentSessionStatusText(
  meta: any,
  language?: RuntimeLanguage,
): string {
  if (!meta) return "";
  ...
  for (const clientId of visibleDevices) {
    ...
    for (const project of deviceProjects) {
      ...
      workspace.addEventListener("toggle", () => {
        if (!workspace.isConnected || project.id !== options.selectedProject) return;
      ...
      if (projectAttention > 0) {
        addSignal(
          options.language === "zh-CN" ? "待处理 " + projectAttention : projectAttention + " attention",
          "tone-warn",
      ...
      if (project.sessions) {
        ...
        if (project.sessions.sessions_truncated) metaParts.push(options.language === "zh-CN" ? "扫描不完整" : "scan partial");
      }
      meta.textContent = metaParts.join(" · ");
      if (metaParts.length) main.appendChild(meta);
      row.appendChild(projectIcon); row.appendChild(main);
      const select = (): void => options.onSelectProject(String(project.client_id || ""), String(project.id || ""));
      row.addEventListener("click", (event) => {
        if (project.id === options.selectedProject) return;
...
export interface RenderRunnerFleetOptions {
  selectedDevice: string;
  language?: RuntimeLanguage;
  onSelectRunner: (clientId: string) => void;
}

export function renderRunnerFleetRows(
  node: HTMLElement,
  runners: any[],
  ...
  for (const runner of runners) {
    ...
    if (clientId === options.selectedDevice) row.setAttribute("aria-current", "true");
    ...
    if (runner.build_git_commit) metaParts.push((options.language === "zh-CN" ? "构建 " : "build ") + String(runner.build_git_commit));
    meta.textContent = metaParts.join(" · ");
    ...
    if (runner.projects_scan_partial) factParts.push(options.language === "zh-CN" ? "设备群扫描不完整" : "fleet scan partial");
    if (runner.sessions?.sessions_truncated) factParts.push(options.language === "zh-CN" ? "会话扫描不完整" : "Session scan partial");
    facts.textContent = factParts.join(" · ");
    row.appendChild(main); row.appendChild(signals); row.appendChild(facts);
    const select = (): void => options.onSelectRunner(clientId);
    row.addEventListener("click", select);
    node.appendChild(row);
  }
}

export interface RenderRecentSessionsOptions {
  selectedProject: string;
...
export function renderRecentSessionRows(
  node: HTMLElement,
  sessions: any[],
  ...
  for (const session of sessions) {
    ...
    const select = (): void => options.onSelectSession(session);
    row.addEventListener("click", select);
    node.appendChild(row);
  }
}
