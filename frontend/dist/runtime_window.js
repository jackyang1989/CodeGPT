...
export function windowDateTimeLabel(timestampMs, language) {
...
export function renderWindowActivityRows(node, activities, options = {}) {
    if (!node)
        return;
...
export function createWindowCard(row, selectedWindowKey, onSelect, now = Date.now(), language) {
...
export function renderWindowActiveRequests(activeNode, activeRequests, options = {}) {
    if (!activeNode)
        return;
...
export function formatWindowDetailFields(detail, fallbackKey = "", now = Date.now(), language) {
    if (!detail)
        return null;
...
export function renderProjectWindowCards(node, windowRows, onSelect, now = Date.now(), language) {
    if (!node)
        return;
    while (node.firstChild)
        node.removeChild(node.firstChild);
