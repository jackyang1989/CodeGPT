...
test("collaboration delta replaces message state by id and completion renders todo resolution plus answer", () => {
  ...
  adoptRuntimeCollaborationList(state, request, [
    { message_id: "cg_msg_todo", kind: "todo", status: "open", created_at: 1, message: "do work" },
  ]);
  assert.equal(adoptRuntimeCollaborationObservation(state, request, {
    observation_token: "opaque-1",
    messages: [
      { message_id: "cg_msg_todo", kind: "todo", status: "resolved", created_at: 1, message: "do work", resolved_by_message_id: "cg_msg_answer" },
      { message_id: "cg_msg_answer", kind: "answer", status: "open", created_at: 2, message: "done", reply_to: "cg_msg_todo", author_session_id: "cg_sess_worker" },
    ],
  }), true);
  assert.equal(state.collaboration.messages.length, 2);
  assert.equal(state.collaboration.messages[0].status, "resolved");
  assert.equal(state.collaboration.messages[1].reply_to, "cg_msg_todo");
...
test("project-read-only degradation keeps project selection while collaboration is marked unavailable", () => {
  ...
  assert.equal(state.workflow.selectedSessionId, "cg_sess_a");
...
test("collaboration Edit and Reply are mutually exclusive and context switches clear edit state", () => {
  ...
  adoptRuntimeCollaborationList(state, request, [
    { message_id: "cg_msg_edit", kind: "guidance", status: "open", priority: "high", requires_ack: true, first_ack_observed_at: 10, created_at: 1, message: "old" },
  ]);
  assert.equal(setRuntimeCollaborationEditTarget(state, "cg_msg_edit"), true);
  assert.equal(runtimeCollaborationEditTarget(state).message_id, "cg_msg_edit");
  assert.equal(state.collaboration.replyTargetId, "");
  setRuntimeCollaborationReplyTarget(state, "cg_msg_edit");
  assert.equal(runtimeCollaborationEditTarget(state), null);
  assert.equal(state.collaboration.replyTargetId, "cg_msg_edit");
  assert.equal(setRuntimeCollaborationEditTarget(state, "cg_msg_edit"), true);
  assert.equal(state.collaboration.replyTargetId, "");
  ...
  adoptRuntimeCollaborationList(state, request, [
    { message_id: "cg_msg_b", kind: "note", status: "open", created_at: 2, message: "b" },
  ]);
  assert.equal(setRuntimeCollaborationEditTarget(state, "cg_msg_b"), true);
...
test("incoming authoritative closure cancels edit while preserving refreshed state", () => {
  ...
  const request = runtimeCollaborationRequest(state);
  adoptRuntimeCollaborationList(state, request, [
    { message_id: "cg_msg_todo", kind: "todo", status: "open", created_at: 1, message: "work" },
  ]);
  assert.equal(setRuntimeCollaborationEditTarget(state, "cg_msg_todo"), true);
  adoptRuntimeCollaborationObservation(state, request, {
    messages: [{ message_id: "cg_msg_todo", kind: "todo", status: "resolved", resolved_at: 2, created_at: 1, message: "work" }],
  });
  assert.equal(runtimeCollaborationEditTarget(state), null);
...
test("withdraw and replacement responses merge by message id without duplicate history", () => {
  ...
  const request = runtimeCollaborationRequest(state);
  adoptRuntimeCollaborationList(state, request, [
    { message_id: "cg_msg_old", kind: "note", status: "open", created_at: 1, message: "wrong" },
  ]);
  adoptRuntimeCollaborationObservation(state, request, {
    messages: [
      { message_id: "cg_msg_old", kind: "note", status: "resolved", closure_kind: "superseded", superseded_by_message_id: "cg_msg_new", created_at: 1, message: "wrong" },
      { message_id: "cg_msg_new", kind: "note", status: "open", supersedes_message_id: "cg_msg_old", created_at: 2, message: "right" },
    ],
  });
  assert.deepEqual(state.collaboration.messages.map((message) => message.message_id), ["cg_msg_old", "cg_msg_new"]);
  adoptRuntimeCollaborationObservation(state, request, {
    messages: [{ message_id: "cg_msg_new", kind: "note", status: "resolved", closure_kind: "withdrawn", created_at: 2, message: "right" }],
  });
  assert.equal(state.collaboration.messages.length, 2);
...
test("unknown mutation outcome stays fenced until exact replay confirms durability", () => {
  ...
  const request = runtimeCollaborationRequest(state);
  adoptRuntimeCollaborationList(state, request, [
    { message_id: "cg_msg_old", kind: "guidance", status: "open", priority: "high", requires_ack: true, created_at: 1, message: "wrong" },
  ]);
  assert.equal(setRuntimeCollaborationEditTarget(state, "cg_msg_old"), true);
  assert.equal(markRuntimeCollaborationMutationUncertain(state, request, { kind: "replace", messageId: "cg_msg_old", message: "right" }), true);
  assert.equal(state.collaboration.messages.length, 1);
  assert.equal(state.collaboration.uncertainMutation.messageId, "cg_msg_old");

  adoptRuntimeCollaborationList(state, request, [
    { message_id: "cg_msg_old", kind: "guidance", status: "resolved", closure_kind: "superseded", superseded_by_message_id: "cg_msg_new", priority: "high", requires_ack: true, created_at: 1, message: "wrong" },
    { message_id: "cg_msg_new", kind: "guidance", status: "open", supersedes_message_id: "cg_msg_old", priority: "high", requires_ack: true, created_at: 2, message: "right" },
  ]);
  assert.equal(state.collaboration.uncertainMutation.messageId, "cg_msg_old");
  assert.equal(runtimeCollaborationEditTarget(state), null);
  assert.equal(
...
test("conversation presentation never infers authorship from reply topology or message kind", () => {
  const sides = runtimeCollaborationMessageSides([
    ...
    { message_id: "trusted-agent", kind: "progress", author_session_id: "cg_sess_worker", message: "working" },
