...
test("runtimeWindowActivityLabel formats relative time in English and Chinese", () => {
  ...
  assert.equal(runtimeWindowActivityLabel(null, now, "en"), "No CodeGPT activity");
  assert.equal(runtimeWindowActivityLabel(null, now, "zh-CN"), "无 CodeGPT 活动");
  assert.equal(runtimeWindowActivityLabel(0, now, "en"), "No CodeGPT activity");
  assert.equal(runtimeWindowActivityLabel(0, now, "zh-CN"), "无 CodeGPT 活动");
});
...
test("formatWindowDetailFields translates fallbacks and bounded indicators", () => {
  ...
  assert.equal(enFields.lastMeaningful, "No meaningful CodeGPT work recorded");
  ...
  assert.equal(zhFields.lastMeaningful, "未记录到有效 CodeGPT 工作");
...
test("renderWindowActiveRequests renders active request items or localized empty placeholder", () => {
  withMockDom(() => {
    ...
    assert.equal(emptyContainer.children[0].textContent, "当前没有活跃的 CodeGPT 请求。");
